...
try {
    ...
    // 策略1：加正则项（λI，λ>0小量）
    var regularized = SPD.addScalar(1e-6 * IMatrix.identity(SPD.rows()));
    // 策略2：改用 LU 分解
...
for (int i = 0; i < eigenvalues.length(); i++) {
    ...
    var Avi = sym.mmul(vi);
    double lambda = eigenvalues.get(i).doubleValue();
    var expected = vi.multiplyScalar(lambda);
    // Avi 应与 expected 近似相等
}
...
// 条件数 = σ_max / σ_min
double cond = S.get(0).doubleValue() / S.get(S.length() - 1).doubleValue();
...
> **⚠️ 常见误区**
> - **混淆矩阵的左奇异向量和右奇异向量**：$\mathbf{A} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}^\top$ 中，$\mathbf{U}$ 的列是 $\mathbf{AA}^\top$ 的特征向量（左奇异向量，对应行空间）；$\mathbf{V}$ 的列是 $\mathbf{A}^\top\mathbf{A}$ 的特征向量（右奇异向量，对应列空间）。说到「SVD 的基向量」时，要明确是哪个。
> - **认为 SVD 第三分量是 V 的第三列**：SVD 分解 $\mathbf{A} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}^\top$ 中，与奇异值 $\sigma_3$ 对应的方向是 $\mathbf{V}$ 的第 3 列，不是 $\mathbf{U}$ 的第 3 列。
...
| 场景 | API | 说明 |
|------|-----|------|
| SVD分解 | `Decomps.svd(A)` | 返回 U/Σ/V |
| 特征分解 | `Decomps.eigen(A)` | 方阵特征值/向量 |
| QR分解 | `Decomps.qr(A)` | 正交三角分解 |
| Cholesky | `Decomps.cholesky(A)` | 对称正定矩阵 |
| 低秩重构 | `Decomps.svd(A).reconstruct(k)` | 前k项截断近似 |
| PCA | `Decomps.pca(A, k)` | 等价于SVD→投影 |

> **SVD是PCA的推广**：不要求方阵、不要求对称、不要求正定；SVD存在性对任何矩阵都成立。
