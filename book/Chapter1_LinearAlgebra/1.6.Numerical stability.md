...
double cond = H.cond().doubleValue();
System.out.println("条件数: " + cond);  // 约 1.55×10^4

// 接近奇异的矩阵
var nearlySingular = Linalg.matrix(new double[][]{{1, 1}, {1, 1.0001}});
double cond2 = nearlySingular.cond().doubleValue();
System.out.println("接近奇异矩阵的条件数: " + cond2);  // 约 40002
```
...
// ❌ 错误：显式求逆
var xWrong = A.inv().mmul(b.asColumnMatrix()).getColumn(0);
...
var diff = a.sub(b);
double relError = diff.norm2().doubleValue() / (a.norm2().doubleValue() + 1e-15);
...
| 场景 | API | 说明 |
|------|-----|------|
| 条件数 | `Decomps.svd(A).conditionNumber()` | $\sigma_{\max}/\sigma_{\min}$ |
| 矩阵求逆稳定性 | `Decomps.inverse(A)` | SVD正则化求逆 |
| QR稳定性 | `Decomps.qr(A)` | 数值稳定QR分解 |
| 病态检测 | `Stats.condTest(A, threshold)` | 自动判断病态 |
| Cholesky前置检查 | `Decomps.choleskyPrecondition(A)` | 对称正定性检验 |

> **数值陷阱**：条件数 $>10^{10}$ 时直接求逆会溢出；先用SVD截断或正则化。
