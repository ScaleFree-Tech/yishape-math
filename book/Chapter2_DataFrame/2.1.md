...
> - 掌握 `DataFrame.of()` 创建与 `DataFrame.describe()` 基本统计
> - 能用 `toMatrix()` 将表格转为 `IMatrix<Double>` 进行数值计算
...
DataFrame df = DataFrame.readCsv("employees.csv");
    // ⚠️ 注意：CSV含空值时，readCsv默认会跳过空值行；用 .dropNulls() 或 .fill(0.0)显式处理前先查看 .describe() 了解空值情况
...
// ❌ 错：想把第3列（salary）读出来
IVector col = df.getColumn(3);   // 实际拿到的是第4列（department）

// ✅ 对：salary 是第3列 → 索引是 2
IVector col = df.getColumn(2);

// ✅ 更安全：按列名访问（推荐）
IVector salary = df.column("salary");
```
...
tempCol.setData(List.of(12.5f, 15.3f, 22.1f, 16.8f));

Column popCol = new Column();
popCol.setName("population");
popCol.setColumnType(ColumnType.Numeric);
popCol.setData(List.of(2154.0f, 2487.0f, 1252.0f, 987.0f));
...
idCol.setData(List.of(1.0f, 2.0f, 3.0f));
...
scoreCol.setData(List.of(85.0f, 92.0f, 78.0f));
...
wrongCol.setData(List.of(85.0f, 92.0f));  // 只有 2 行！
...
| 场景 | API | 说明 |
|------|-----|------|
| 标准 CSV（有表头）| `DataFrame.readCsv(path)` | 逗号分隔，有表头 |
| 自定义分隔符 | `DataFrame.readCsv(path, sep, hasHeader)` | `sep` 如 `"\t"`、`";"` |
| 手动建 DataFrame | `new DataFrame(List<Column>)` | 列必须等长 |
| 动态添加列 | `df.addColumn(col)` | 第一列确立行数，后续必须等长 |
