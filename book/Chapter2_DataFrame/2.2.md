...
> **📌 学习目标**
> - 掌握按位置 `df.get(int)` 和按名称 `df.getColumnByName(String)` 选取列
> - 能用列类型检查 + `column.toVec()` 串接到 `IVector` 的统计接口
> - 理解 `getColumn` 返回的是 `Column`（容器），还需 `.toVec()` / `.toStringList()` 转为可计算对象
...
### 快速概览：info() 和 describe()

如果你不想手动遍历列名和类型，DataFrame 提供了两个「一键概览」方法：

```java
// info()：数据结构概览（类似 pandas 的 df.info()）
df.info();
// 输出：
// DataFrame: 1000 rows × 10 columns
// Column 0: name (String) — 1000 valid, 0 missing
// Column 1: age (Numeric) — 995 valid, 5 missing
// Column 2: salary (Numeric) — 980 valid, 20 missing
// ...

// describe()：数值列的统计摘要（返回 IMatrix，可进一步计算）
IMatrix stats = df.describe();
// 行：[count, mean, std, min, 25%, 50%, 75%, max]
// 列：每个数值列一个
System.out.println("统计摘要矩阵: " + stats.getRowNum() + " × " + stats.getColNum());
```

> **describe() 返回的是 IMatrix**，不是打印！这意味着你可以用第 1 章的所有矩阵运算来处理它——比如 `stats.getRow(1)` 取出所有列的均值，`stats.getColumn(0)` 取出第一列的完整统计摘要。
...
> ⚠️ **常见误区**：在空 DataFrame 上调用 `df.get(-1)` 会抛 `IndexOutOfBoundsException`，而**不是**返回空列。访问前先用 `df.isEmpty()` 或 `df.rows() > 0` 判断。
...
按位置取列（`df.get(int)` / `df.getColumn(int)`）适合已知列序号时；按名称取列（`df.getColumnByName(String)`）适合列顺序可能变化的场景。两种方式返回的都是 `Column` 容器，需要再调 `toVec()` / `toDoubleArray()` / `toStringArray()` 进入数值/字符串计算路径。

**核心要点**：`get`/`getColumn` 用整数下标，`getColumnByName` 用列名字符串；负数索引从末尾计数（-1 = 最后一列）；任何取列后想做统计都要走 `Column.toVec()` 拿到 `IVector<Double>`。
## 章节练习 / Exercises
