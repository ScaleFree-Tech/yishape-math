...
> **📌 学习目标**
> - 掌握 `df.toMatrix()` 把数值列转为 `IMatrix<Double>` 喂给模型
> - 掌握 `Column.toVec()` / `toDoubleArray()` / `toStringList()` 三种列转换路径
> - 能用 `df.toCsv(path)` 持久化清洗后的数据集
...
然后报错了：`IClassifier.fit(X, y)` 等接口只接受 `IMatrix<Double>` + `IVector<Double>`，不接受 `DataFrame`。
...
System.out.printf("矩阵形状: %d 行 × %d 列%n", X.rows(), X.cols());
```
...
// 解决方法：先确认 df.getColumnTypes() 里有 ColumnType.Numeric
...
```java
// 方式1：toVec()（最简洁，直接返回 IVector<Double>）
var v1 = ageCol.toVec();

// 方式2：toDoubleArray() 再用 Linalg.vector()（拿到原始 double[]，便于跟 Java 库交互）
double[] arr = ageCol.toDoubleArray();
...
// 机器学习模型（见第 5 章；ML.clf 是分类器子工厂）
IClassifier clf = ML.clf.logisticRegression(0.01, 0.1);  // l1Weight=0.01, l2Weight=0.1
clf.fit(X, y);
```
