...
|--------|------|------------------|
| 样本均值 | $\bar{X} = \frac{1}{n}\sum_{i=1}^{n}X_i$ | `IDoubleVector.mean()` |
| 样本方差 | $S^2 = \frac{1}{n-1}\sum_{i=1}^{n}(X_i - \bar{X})^2$ | `IDoubleVector.var()` |
| 样本标准差 | $S = \sqrt{S^2}$ | `IDoubleVector.std()` |
| 样本偏度 | $Skew = \frac{n}{(n-1)(n-2)}\frac{\sum(X_i-\bar{X})^3}{S^3}$ | `IDoubleVector.skewness()` |
| 样本峰度 | $Kurt = \frac{n(n+1)}{(n-1)(n-2)(n-3)}\frac{\sum(X_i-\bar{X})^4}{S^4} - \frac{3(n-1)^2}{(n-2)(n-3)}$ | `IDoubleVector.kurtosis()` |
...
public class SampleStatistics {
    public static void main(String[] args) {
        ...
        // 创建向量对象
        IVector sample = Linalg.vector(heights);
...
public class LawOfLargeNumbers {
    public static void main(String[] args) {
        ...
        var sampleSizes = {10, 50, 100, 500, 1000, 5000, 10000};
        
        for (int n : sampleSizes) {
            // 生成n个样本点
            var sample = normal.sample(n);
            IVector vec = Linalg.vector(sample);
...
public class CentralLimitTheorem {
    public static void main(String[] args) {
        ...
        var sampleMeans = new double[m];
        for (int i = 0; i < m; i++) {
            IVector sample = Linalg.vector(uniform.sample(n));
            sampleMeans[i] = sample.mean();
        }
        
        IVector means = Linalg.vector(sampleMeans);
        ...
        for (int i = 0; i < m; i++) {
            IVector sample = Linalg.vector(exp.sample(n));
            sampleMeans[i] = sample.mean();
        }
...
public class SamplingDistributions {
    public static void main(String[] args) {
        ...
        // 计算分位数值
        var quantiles = {0.25, 0.5, 0.75, 0.9, 0.95, 0.975, 0.99};
        ...
        System.out.println("\nF(5, 20) 分位数:");
        var fQuantiles = {0.5, 0.75, 0.9, 0.95, 0.975};
...
| 场景 | API | 说明 |
|------|-----|------|
| 样本均值 | `Stats.mean(data)` 或 `data.mean()` | 算术平均 |
| 样本方差 | `Stats.variance(data)` | $s^2 = \frac{1}{n-1}\sum(x_i-\bar{x})^2$ |
| 样本标准差 | `data.std()` | 方差的平方根 |
| 置信区间 | `Stats.confInterval(data, alpha)` | 返回均值置信区间 |
| 假设检验 | `Stats.hTest(data, testType)` | t检验等 |
| CLT验证 | `Stats.cltCI(n, alpha, trials)` | 演示CLT置信区间覆盖 |
| 抽样分布 | `Stats.tDist(df)`, `Stats.chiSq(df)`, `Stats.fDist(d1,d2)` | 三大抽样分布PDF/CDF |

> **⚠️ 注意**：`Stats.variance(data)` 默认使用 $n-1$ 无偏估计；若需总体方差用 `Stats.variancePop(data)`。
...
| 文件 | 说明 |
|------|------|
| `Stats.java` | 统计工厂类，提供 `norm()`, `t()`, `chi2()`, `f()` 等工厂方法 |
| `NormalDistribution.java` | 正态分布实现 |
| `StudentDistribution.java` | t分布实现 |
| `Chi2Distribution.java` | 卡方分布实现 |
| `FDistribution.java` | F分布实现 |
| `IDoubleVector.java` | 向量接口，提供 `mean()`, `var()`, `skewness()`, `kurtosis()` 等方法 |
