...
public class BayesianNormalMeanExample {
    public static void main(String[] args) {
        // 数据
        var weights = {10.2, 9.8, 10.1, 10.3, 9.9};
...
public class BayesianVIExample {
    public static void main(String[] args) {
        // 示例：使用变分推断估计正态均值
        // 真实后验无法解析求解，使用平均场VI近似

        var data = {10.2, 9.8, 10.1, 10.3, 9.9};
...
| 功能 | API | 说明 |
|------|-----|------|
| 贝叶斯定理 | `Stats.bayesPosterior(prior, likelihood, evidence)` | 计算后验概率 |
| Beta-Bernoulli共轭 | `Stats.betaBernoulli(priorAlpha, priorBeta, successes, trials)` | 返回后验参数 |
| 共轭先验 | `Stats.conjugatePrior(family, alpha, beta, data)` | 自动识别分布族 |
| 变分推断 | `Stats.variationalBayes(data, family, iterations)` | 近似后验分布 |
| 先验选择 | `Stats.jeffreysPrior(distType)` | Jeffreys无信息先验 |

> **⚠️ 注意**：贝叶斯计算涉及数值稳定性，大型数据用变分推断（VI）近似而非精确后验。

## 4.5.6 章节练习 / Exercises
...
贝叶斯定理 P(H|D) = P(D|H)·P(H) / P(D) 将先验知识（先验概率）与新数据（似然）结合，更新我们对假设的信念。后验概率是贝叶斯学习的核心输出。

**核心要点**：频率学派只信任数据，贝叶斯学派融合先验和数据；共轭先验让后验计算变得简单；在数据稀缺时，贝叶斯方法尤其有价值。
...
```java
// 负二项分布
var nb = Stats.negativeBinomial(r = 5, p = 0.3);  // r=成功次数, p=单次成功概率
double prob = nb.pmf(8);  // P(K=8)

// Gamma-Gamma 共轭（计数的贝叶斯估计）
var gammaPrior = Stats.gammaPrior(alpha = 2, beta = 1);  // 先验
var gammaPost = gammaPrior.update(counts = new int[]{3, 5, 2, 8});  // 更新
...
```java
// 比较公平硬币 vs 偏斜硬币
var bf = Stats.bayes().bayesFactor(
    model1 = "H1: p = 0.5",              // 公平硬币
    model0 = "H0: p ~ Uniform(0,1)",     // 先验均匀
    data = successes = 8, trials = 10    // 8次成功/10次
);
System.out.println("BF₁₀ = " + bf.value());
