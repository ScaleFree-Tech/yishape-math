...
public class DrugEfficacyCase {
    public static void main(String[] args) {
        ...
        // 3. 方差齐性检验
        System.out.println("\n【3. 方差齐性检验】");
        boolean equalVar = ANOVA.testHomogeneityOfVariance(control, treatment);
...
| 场景 | API | 说明 |
|------|-----|------|
| 质量控制图 | `Stats.controlChart(data, UCL, LCL)` | X-bar控制图 |
| Cpk工序能力 | `Stats.cpk(data, USL, LSL)` | 工序能力指数 |
| 事件率比较 | `Stats.rateTest(r1, n1, r2, n2)` | 两组事件率差异检验 |
| 置信区间 | `Stats.confInterval(data, alpha)` | 率/均值CI |
| 效应量Cohen's d | `Stats.cohensD(group1, group2)` | 两组标准化差异 |

> **应用场景**：质量控制用控制图；药物实验用率检验+效应量。
...
统计质量控制用控制图监控生产过程：若数据点超出 UCL/LCL（3σ 控制限）或呈现趋势/连续 7 点在中心线同一侧，说明过程失控，需要查找特殊原因。

**核心要点**：控制图区分普通原因变异和特殊原因变异；Cpk 工序能力指数衡量过程满足规格的能力；休哈特控制图是统计质量控制的基石。