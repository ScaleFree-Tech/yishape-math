...
// 降到2维
var Z = pca.dimensionReduction(X, 2);
...
tsne.setPerplexity(30.0);
var Z = tsne.dimensionReduction(X, 2);  // 降至2维
```
...
umap.setNNeighbors(15);
umap.setMinDist(0.1);
var Z = umap.dimensionReduction(X, 2);
```
...
| 场景 | API | 说明 |
|------|-----|------|
| PCA降维 | `Decomps.pca(X, k)` 或 `Decomps.svd(X).project(k)` | 返回k维投影矩阵 |
| 方差解释比例 | `pca.explainedVarianceRatio()` | 各主成分方差占比 |
| 累计方差 | `Arrays.stream(pca.explainedVarianceRatio()).sum()` | 前k主成分累计 |
| t-SNE | `ML.tsne(X, k, perplexity)` | 非线性降维，k=2或3用于可视化 |
| SVD→PCA等价 | `Decomps.svd(X).U.times(Decomps.svd(X).Sigma).project(k)` | 等价于PCA |
