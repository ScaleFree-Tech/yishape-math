...
// 获取特征重要性
var importance = rf.getFeatureImportance();
...
// 使用 DataFrame 划分训练集和验证集（7:3）
var df = DataFrame.fromMatrix(X, labels);
var splits = df.trainTestSplit(0.7);
var XTrain = splits[0].toMatrix();
var XVal = splits[1].toMatrix();
var yTrain = splits[0].getColumnByName("label").toVec();
var yVal = splits[1].getColumnByName("label").toVec();
...
System.out.println("训练准确率: " + xgb.getTrainAccuracy());

// 调参对比：不同 learningRate
System.out.println("\n=== 超参数调参建议 ===");
System.out.println("使用 ML.clf.xGboost() 创建默认 XGBoost 后，可通过以下方式调参：");
System.out.println("- learningRate：越小精度越高，但需配合更多树");
System.out.println("- maxDepth：3~8，太深容易过拟合");
System.out.println("- lambda/alpha：L2/L1 正则化，防止权重过大");
```

**调参经验**：
- 使用 `ML.clf.xGboost()` 创建默认配置
- 通过交叉验证选择最佳超参数
- 配合 `ML.clf.kFoldCrossValidation()` 评估不同配置
