...
**拿到一个业务问题，判断它属于哪类 ML 任务，然后选择合适的算法，从数据清洗到模型评估独立完成。**
...
|------|------|-------------|-----------------|
| **回归** | 线性回归 + 正则化 | 多少？（广告预算 → 订单量） | `ML.linearRegression()` |
| **二分类** | 逻辑回归 | 是还是否？（贷款违约/不违约） | `ML.logisticRegression(l1, l2)` |
| **二分类** | K 近邻 | 像不像？（用户行为相似 → 同类推荐） | `ML.kNN(k)` |
| **多分类** | 逻辑回归扩展 | 哪个？（图片识别：猫/狗/车） | 同上 |
| **聚类** | K-Means++ | 怎么分群？（用户行为分群） | `new KMeansPlusPlus()` |
| **降维** | PCA | 能不能压缩？（50个特征 → 5个主成分） | `new RerePCA()` |
| **降维** | t-SNE/UMAP | 高维数据怎么可视化？（2D 散点图） | `new RereTSNE()` / `new RereUMAP()` |
| **集成** | 随机森林 | 怎么减少过拟合？（多棵树投票） | `ML.randomForest()` |
| **集成** | XGBoost | 精度怎么再提升？（梯度提升） | `ML.xGboost()` |
