...
YiShape-Math 提供：
- `Opts.onlineSGD(double lr)` → 随机梯度下降（学习率需手动设置）
- `Opts.onlineAdam(double lr, double beta1, double beta2)` → Adam（推荐配置 lr=0.001, β₁=0.9, β₂=0.999）
- `Opts.onlineAdamW(...)` → AdamW（带权重衰减）
- `Opts.onlineRMSprop(...)` → RMSprop
- `Opts.onlineAdagrad(...)` → Adagrad
- `Opts.onlineAdadelta(...)` → Adadelta
- `Opts.onlineLAMB(...)` → LAMB（大 batch 训练）

```java
import com.yishape.lab.math.optimize.Opts;

var sgd = Opts.onlineSGD(0.01);                                    // 学习率 0.01
var adam = Opts.onlineAdam(0.001, 0.9, 0.999);                    // 推荐默认配置
var adamW = Opts.onlineAdamW(0.001, 0.9, 0.999, 0.01);           // AdamW（weightDecay=0.01）
...
var adam = Opts.onlineAdam(0.5, 0.9, 0.999);
...
除了 SGD 和 Adam，还有几个实用的优化器：

```java
// Adagrad：适合稀疏特征（如 NLP 中的词向量）
// 特点：频繁更新的参数学习率自动减小，罕见特征保持较大学习率
var adagrad = Opts.onlineAdagrad(0.01);

// Adadelta：无需手动设置初始学习率
// 特点：用梯度的指数移动平均自适应调整，比 Adagrad 更稳定
var adadelta = Opts.onlineAdadelta(0.95);  // rho=0.95

// LAMB：大 batch 训练专用（如 BERT 预训练）
// 特点：Layer-wise Adaptive Moments，支持 batch_size > 8192
var lamb = Opts.onlineLAMB(0.001, 0.9, 0.999, 0.01);
```
...
| 优化器 | 最佳场景 | 关键超参数 |
|--------|---------|-----------|
| SGD | 简单凸问题、需要可解释性 | 学习率（最重要） |
| Adam | 深度学习、默认首选 | lr=0.001, β₁=0.9, β₂=0.999 |
| AdamW | Transformer、需要权重衰减 | weightDecay=0.01 |
| RMSprop | RNN、非平稳目标 | alpha=0.99 |
| Adagrad | 稀疏特征、NLP | lr=0.01 |
| Adadelta | 不想调学习率 | rho=0.95 |
| LAMB | 大 batch 分布式训练 | batch ≥ 8192 |
...
| 场景 | API | 说明 |
|------|-----|------|
| L-BFGS（默认） | `Opts.lbfgs()` | 限内存拟牛顿，无约束光滑优化主力 |
| L-BFGS（自定义） | `Opts.lbfgs(double tol, int maxSteps)` | 收敛容差、最大迭代步 |
| L-BFGS（Rust 加速） | `Opts.lbfgs(LBFGSType.JAVA)` 或 `LBFGSType.RUST` | 选择 Java 或 Rust 后端 |
| OWL-QN（L1 正则） | `Opts.owlqn()` | L-BFGS 的次梯度变体，L1 正则化优化 |
| 共轭梯度 | `Opts.conjugateGradient()` | 大规模正定二次型 |
| DFP | `Opts.dfp()` | 早期拟牛顿法 |
| 最速下降 | `Opts.steepestDescent()` | 最基础的梯度下降 |
| 在线 SGD | `Opts.onlineSGD(lr)` | 流数据 / 大规模 |
| 在线 Adam | `Opts.onlineAdam(lr, beta1, beta2)` | 自适应学习率，深度学习常用 |
| 在线 AdamW | `Opts.onlineAdamW(lr, beta1, beta2, weightDecay)` | 带权重衰减的 Adam |
| 在线 RMSprop | `Opts.onlineRMSprop(lr, alpha)` | 自适应学习率 |
| 在线 Adagrad | `Opts.onlineAdagrad(lr)` | 稀疏特征友好 |
| 在线 Adadelta | `Opts.onlineAdadelta(rho)` | 无需手动设置学习率 |
| 在线 LAMB | `Opts.onlineLAMB(lr, beta1, beta2, weightDecay)` | 大 batch 训练 |
| 启动优化 | `optimizer.optimize(initX, objectiveFn, gradientFn)` | 返回 `OptResult` |
| 最优点 / 最优值 | `result.getOptimalPoint()` / `result.getOptimalValue()` | `IVector<Double>` / `double` |
| 收敛标志 / 迭代次数 | `result.isConverged()` / `result.getIterations()` | 用于判断是否收敛与诊断 |
| 函数值轨迹 | `result.getFunctionValueHistory()` | `List<Double>`，可用于绘图 |
| 执行耗时 | `result.getExecutionTimeMs()` | 毫秒计 |

> ⚠️ **不存在的 API**：`Opts.gradientDescent(lr)`（用 `Opts.steepestDescent()` 或 `Opts.onlineSGD()`）、`result.getNormGradient()`（未提供，请用最后一步梯度自行计算）。
>
> **选择指南**：L-BFGS（二阶近似）收敛快但每次迭代贵；Adam/AdamW（自适应学习率）适合深度学习；SGD 适合大数据。
