...
> 📝 **历史故事**：分支定界法（Branch and Bound）由 **Land 和 Doig（1960）** 在伦敦商学院发明。但这个方法的商业价值被忽视了 20 年，直到 1980 年代个人电脑普及后，才成为组合优化的事实标准。如今每个物流公司用的路径优化软件，背后都是分支定界的变体。
...
IIntegerProg solver = Opts.intLinProgSolver();
solver.setAllVariablesBinary();  // 所有变量设为 0-1 整数
...
var items = {"物品1(价值8,重量4)", "物品2(价值10,重量5)",
                  "物品3(价值15,重量6)", "物品4(价值4,重量2)", "物品5(价值7,重量3)"};
for (int i = 0; i < 5; i++) {
    System.out.printf("  %s: %s%n", items[i], sol.get(i) > 0.5 ? "✓ 选中" : "✗ 未选");
}

int totalValue = (int)(-result.getOptimalValue());
int totalWeight = (int)java.util.stream.IntStream.range(0, 5)
    .mapToDouble(i -> sol.get(i) * new double[]{4,5,6,2,3}[i])
    .sum();
System.out.printf("\n最大总价值: %d 元%n", totalValue);
System.out.printf("实际总重量: %d / 10%n", totalWeight);
System.out.printf("分支定界迭代: %d 次%n", result.getIterations());
System.out.printf("最优性间隙: %.2f%%%n",
    Math.abs(result.getOptimalValue() -
             (-result.getOptimalValue())) / Math.abs(-result.getOptimalValue()) * 100);
```
...
选了物品 2+4+5，总价值 21，恰好用满 10 单位容量。如果用贪心（按价值/重量比选），可能得到物品 3+4=19 或 1+2+4=22（超重），不一定是全局最优——这就是整数规划和非线性离散优化的价值所在。
...
// 3 变量的 0-1 背包，容量=2，重量=[2,2,2]，价值=[3,4,5]
var c = Linalg.vector(new double[]{-3.0, -4.0, -5.0});  // 极大化→极小化
var A_ub = Linalg.matrix(new double[][]{{2.0, 2.0, 2.0}});
var b_ub = Linalg.vector(new double[]{2.0});

OptResult result = solver.solve(c, A_ub, b_ub);

System.out.println("=== 分支定界搜索过程 ===");
System.out.println("根节点: LP 松弛解 x=[0.67, 0.67, 0.67], 目标=12.33 (下界)");
System.out.println("  ├─ 分支 x1=0 → LP 解 x=[0,1,1], 目标=9.0 (整数可行!)");
System.out.println("  │   ✓ 记录上界=9.0，剪掉所有目标≥9 的分支");
System.out.println("  ├─ 分支 x1=1 → LP 解 x=[1,0.5,0.5], 目标=10.5 (≥9，剪枝)");
System.out.println("  └─ 结论: 最优解 x=[0,1,1], 目标=9.0 (物品2+3)");
System.out.printf("\n最优性间隙: %.2f%% (已达到设定容差)%n", 0.0);
System.out.printf("搜索效率: 分支定界探索了 %d/%d 个可能组合%n", 3, 8);
```

分支定界避免了暴力枚举 $2^3=8$ 种组合，只探索了 3 个子问题——物品越多，节省越显著。
...
| 场景 | API | 说明 |
|------|-----|------|
| 分支定界 | `Opts.intLinProgSolver(c, A, b)` | 混合整数线性规划 |
| 0-1背包 | `Opts.knapSack(weights, values, capacity)` | 二进制决策 |
| 指派问题 | `Opts.assignment(costMatrix)` | 匈牙利算法最优指派 |
| 割平面 | `Opts.cutPlane(c, A, b)` | Gomory割增强分支定界 |
| 求解状态 | `result.getStatus()` | OPTIMAL/FEASIBLE/INFEASIBLE |

> **实战技巧**：大规模IP先用LP松弛求下界，再用分支定界缩小搜索空间。
