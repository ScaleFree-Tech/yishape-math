...
// Adam - 自适应学习率，非凸优化常用
var adam = Opts.onlineAdam(0.001, 0.9, 0.999);  // lr=0.001, β₁=0.9, β₂=0.999

// AdamW - 带权重衰减的 Adam
var adamW = Opts.onlineAdamW(0.001, 0.9, 0.999, 0.01);  // weightDecay=0.01

// L-BFGS - 光滑非凸问题（如逻辑回归）
var lbfgs = Opts.lbfgs();
```
...
// 从 5 个随机起点运行
int nStarts = 5;
System.out.printf("=== 多起点局部优化（%d 起点）===%n", nStarts);
...
非凸优化在 YiShape-Math 中**没有专门的全局求解器**——多起点 + 局部优化是工程上的标准做法。

| 场景 | API | 说明 |
|------|-----|------|
| 局部光滑优化 | `Opts.lbfgs()` / `Opts.dfp()` / `Opts.steepestDescent()` | 各随机起点后接的局部方法 |
| 在线 SGD（带噪声，逃鞍点） | `Opts.onlineSGD(lr)` | 大规模非凸常用 |
| 在线 Adam（自适应学习率） | `Opts.onlineAdam(lr, beta1, beta2)` | 深度学习默认选择 |
| 在线 AdamW（权重衰减） | `Opts.onlineAdamW(lr, beta1, beta2, weightDecay)` | 正则化 + 自适应学习率 |
| 随机起点生成 | `RandomCache.seededRandom(seed).doubles(d, lo, hi)` | 固定 seed 便于复现 |
| 比较候选解 | `result.getOptimalValue()` 排序 | 自行遍历多次 `optimize` 的结果取最小 |

> ⚠️ **不存在的 API**：`Opts.multiStart(...)`、`Opts.simulatedAnnealing(...)`、`Opts.geneticAlg(...)`、`result.getAllLocalMinima()` 等「一键全局」接口都未提供。需要模拟退火/遗传算法时，请在用户代码层实现循环 + `Opts.lbfgs()`（或自定义启发式）。
>
