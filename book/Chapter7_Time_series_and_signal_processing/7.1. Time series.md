...
| 场景 | API | 说明 |
|------|-----|------|
| 差分 | `TimeSeries.diff(series, d)` | d阶差分使序列平稳 |
| ARIMA建模 | `TimeSeries.arima(data, p, d, q)` | 自动拟合最优参数 |
| ACF/PACF | `TimeSeries.acf(series, lags)` | 自相关/偏自相关函数 |
| ADF平稳检验 | `TimeSeries.adfTest(series)` | p<0.05则拒绝非平稳 |
| AIC/BIC模型选择 | `TimeSeries.autoArima(data, maxP, maxQ)` | 按信息准则选最优(p,d,q) |
| 滚动预测 | `result.forecast(h)` | 向前h步预测 |
...
```java
// STL 分解（加法模型）
var stl = TimeSeries.decompose(ts, model = "additive", period = 12);
// stl.trend() → 趋势成分
// stl.seasonal() → 季节成分
// stl.resid() → 残差

// 乘法模型（适合增长率相对稳定的数据）
var stlMulti = TimeSeries.decompose(ts, model = "multiplicative", period = 12);

// classical 分解（简单移动平均法）
var classical = TimeSeries.classicalDecompose(ts, model = "additive", period = 12);
```
