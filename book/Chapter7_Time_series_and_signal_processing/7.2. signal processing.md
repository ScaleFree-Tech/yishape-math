...
// FFT
Complex[] X = Signals.fft(x);
var magnitude = Signals.magnitudeSpectrum(X);
...
```java
Complex[] reconstructed = Signals.ifft(X);
...
| 场景 | API | 说明 |
|------|-----|------|
| FFT频谱 | `Signals.fft(signal)` | 快速傅里叶变换 |
| 逆FFT | `Signals.ifft(spectrum)` | 频域→时域 |
| 频谱幅度 | `Signals.magnitude(spectrum)` | |X[k]| |
| 滤波器设计 | `Signals.filter(b, a, signal)` | IIR/FIR滤波 |
| 频谱图 | `Signals.spectrogram(signal, windowSize, hop)` | STFT时频图 |
| 加窗 | `Signals.window(signal, "hann")` | 汉宁/汉明等窗函数 |

> **选择指南**：降噪用低通滤波器；提取特定频率成分用带通；时频联合用STFT。
...
var chirp = Signals.chirpSignal(f0, f1, duration, sampleRate);
...
```java
// 白噪声（所有频率成分功率相等）
var white = Signals.whiteNoise(duration = 1.0, sampleRate = 10000, amplitude = 1.0);
