...
- **`AD`** - 自动微分工厂类，创建变量与执行微分运算（**推荐使用**） / Autodiff factory for variables and differentiation (**recommended**)
- **`RereMathUtil`** - 数学工具类，提供常用数学函数 / Math utilities
- **`TSA`** - 时间序列分析入口，创建数据和调用分析功能（**推荐使用**） / Time series entry point for data creation and analysis (**recommended**)
- **`TimeSeriesAnalyzer`** - 时间序列分析工具类 / Time series analysis utilities
...
- **`IVariable`** / **`IMatrixVariable`** - 自动微分变量接口，支持反向模式梯度传播 / Autodiff variable interfaces for reverse-mode gradient propagation
- **`ISparseVariable`** / **`IComplexVariable`** - 稀疏/复数自动微分接口 / Sparse and complex autodiff variable interfaces
...
- **`LevinsonDurbinSolver`** - Yule-Walker + Toeplitz 求解器 / Yule-Walker + Toeplitz solver
- **`CohenClassDistribution`** - Cohen 类时频分布 / Cohen-class time-frequency distributions
...
- **计算工具类 / Compute helpers**: `CPUComputeFloatUtils`, `CPUComputeDoubleUtils`（GPU 工具类已于 v0.3.7 移除 / GPU helpers removed in v0.3.7）
...
- **ISparseMatrix**: 稀疏矩阵接口 (COO/CSR/CSC)，支持格式转换和高效稀疏运算 / Sparse matrix interface
- **ISparseLinearSolver**: 稀疏迭代求解器接口 (CG/BiCGStab/GMRES) / Sparse iterative solver interface
- **ISparsePreconditioner**: 稀疏预条件子接口 (ILU(0)) / Sparse preconditioner interface
- **Linalg.toeplitz()**: Toeplitz 矩阵构建器，支持对称/非对称 / Toeplitz matrix builder
...
- **`ML.reg`**: 回归工厂 — `linear(l1, l2)` 等 / Regression factory
- **`ML.clf`**: 分类工厂 — `logisticRegression(l1, l2)`、`randomForest()`、`xGboost()` 等 / Classification factory
- **`ML.dr`**: 降维工厂 — `pca(n)`、`svd(n)`、`tsne(n)`、`umap(n)` 等 / Dimensionality reduction factory
- **`ML.clu`**: 聚类工厂 — `kMeans(k)`、`gaussianMixture(k)` 等 / Clustering factory
- **`ML.dml`**: 距离度量学习工厂 — `diagDml(l1, l2)`、`ddml()` 等（含自主研发 DDML） / Distance metric learning factory
- **`ML.preproc`**: 数据预处理工厂 — `standardScaler()`、`minMaxScaler()` 等 / Preprocessing factory
- **IClassifier / IRegression / ITransform**: 分类、回归与变换接口 / Classification, regression, and transform interfaces
...
- **`Opts`**: 优化与规划统一入口（**推荐使用**） / Unified optimization entry point (**recommended**)
  - `Opts.lbfgs()`: L-BFGS 优化器 / L-BFGS optimizer
  - `Opts.onlineAdam()`: 在线 Adam 优化器 / Online Adam optimizer
  - `Opts.simplexLinProgSolver()`: 单纯形法线性规划求解器 / Simplex LP solver
  - `Opts.intLinProgSolver()`: 整数规划求解器 / Integer programming solver
- **`Opts.mclp`**: 多目标线性规划入口 / Multi-criteria linear programming entry
  - `weightedSumMclp(weights)`、`lexicographicMclp(order)`、`goalProgrammingMclp(goals, weights)` 等 7 种策略 / 7 solver strategies
- **`Opts.mcqp`**: 多目标二次规划入口 / Multi-criteria quadratic programming entry
  - `weightedSumQp(weights)`、`lexicographicQp(order)`、`solveBiObjective(Q1, c1, Q2, c2, ...)` 等 7 种策略 + 双目标快捷求解 / 7 strategies + bi-objective quick solve
- **IOptimizer**: 优化器接口 / Optimizer interface

### 6. 自动微分 (Automatic Differentiation)
- **`AD`**: 自动微分统一入口（**推荐使用**） / Unified autodiff entry point (**recommended**)
  - `AD.vector(data)`: 创建反向模式变量（`IDiffVector`） / Create reverse-mode variable
  - `AD.matrix(data)`: 创建矩阵变量（`IDiffMatrix`） / Create matrix variable
  - `AD.tangent(primal, tangent)`: 前向模式 Jacobian-vector product / Forward-mode JVP
  - `AD.jacobian(fn, x)`: 前向模式完整 Jacobian 矩阵 / Forward-mode full Jacobian
  - `AD.optimize(w0, loss, optimizer)`: 一行代码替代手写梯度 / One-liner gradient-free optimization
  - `AD.odeint(dynamics, z0, t0, t1, dt)`: Neural ODE 积分 / Neural ODE integration
  - `AD.sparse(data)` / `AD.complex(data)` / `AD.diffFloat(data)`: 稀疏/复数/混合精度 / Sparse, complex, mixed precision
  - `AD.checkGradient(fn, x, tol)`: 数值梯度校验 / Numerical gradient checking
  - `AD.render(root)`: 计算图 DOT 可视化 / Graph DOT visualization
- **`IVariable`**: 反向模式变量接口，32 种运算，`backward()` 拓扑排序传播 / Reverse-mode variable, 32 ops, topological backward
- **`IMatrixVariable`**: 矩阵变量接口，支持 matmul/transpose/flatten/reshape / Matrix variable with matmul/transpose
- **`ISparseVariable`** / **`IComplexVariable`**: 稀疏/复数自动微分接口（sum/mean/sin/cos/tan/grad 等） / Sparse and complex autodiff interfaces (sum, mean, sin, cos, tan, grad, etc.)
- **`MixedMode`**: 混合模式自动微分 — HVP (Hessian-vector product)、JVP (Jacobian-vector product)、全 Hessian / Mixed-mode AD: HVP, JVP, full Hessian via forward-over-reverse

### 7. 降维算法 (Dimensionality Reduction)
- **RerePCA**: 主成分分析 / PCA
- **RereSVD**: 奇异值分解 / SVD
...
### 9. 数学工具 (Mathematical Utilities)
- **RereMathUtil**: 数学工具类，提供类型转换、随机数生成、统计函数等 / Conversions, RNG, math helpers

### 10. 时间序列分析 (Time Series Analysis)
- **TSA**: 时间序列入口类，提供 `TSA.data()` 数据创建和 `TSA.forecast`/`TSA.filter`/`TSA.decompose` 子域 / Entry point: `TSA.data()` + subdomain groups
- **TimeSeriesData**: 时间序列数据类，支持单变量和多变量时间序列 / Uni- and multivariate series
- **TimeSeriesAnalyzer**: 统一时间序列分析工具类，提供完整分析功能 / Unified analysis
- **TimeSeriesForecasting**: 时间序列预测服务类，接受 `IVector` 直接预测 / Forecasting (IVector-based)
- **TimeSeriesFiltering**: 时间序列滤波服务类，接受 `IVector` 直接滤波 / Filtering (IVector-based)
- **TimeSeriesDecomposition**: 时间序列分解服务类，接受 `IVector` 直接分解 / Decomposition (IVector-based)
- **LevinsonDurbinSolver**: Yule-Walker 方程 + 通用 Toeplitz O(n²) 求解器 / Yule-Walker equations + general Toeplitz O(n²) solver
- **TimeSeriesUtils**: PACF 计算 (内部调用 Levinson-Durbin) / PACF computation (uses Levinson-Durbin internally)
...
- **SignalAnalysis**: 信号分析类，提供频谱分析、功率谱密度、时频分析等功能 / Spectral analysis, PSD, time-frequency analysis
...
- **CohenClassDistribution**: Cohen 类时频分布 (Choi-Williams/Born-Jordan/Margenau-Hill/Pseudo WVD) / Cohen-class time-frequency distributions
- **SignalVisualizer**: 信号可视化类，提供信号数据可视化功能 / Signal visualization
- **Complex**: 复数类，提供复数运算功能 / Complex numbers

### 12. 广播与稠密数组相关 API / Broadcasting and dense-array-related APIs
...
向量操作的核心接口，为一维数据结构提供全面的数学运算。实际泛型签名为 `IVector<T extends Number>`；以下示例使用 `T` 表示元素类型，实际调用时由 `IDoubleVector`（`T=Double`）或 `IFloatVector`（`T=Float`）特化。
The actual generic signature is `IVector<T extends Number>`; below, `T` represents the element type, specialized by `IDoubleVector` (`T=Double`) or `IFloatVector` (`T=Float`).

```java
public interface IVector<T extends Number> {
    // 工厂方法 / Factory methods
    static IVector<Float>  of(float[] array);       // Create vector from float array
    static IVector<Double> of(double[] array);      // Create vector from double array
    static IVector<Float>  of(int[] array);         // Create vector from int array
    static IVector<T> zeros(int length);             // Create zero vector
    static IVector<T> ones(int length);              // Create vector of ones
    static IVector<T> rand(int length);              // Create random vector
    static IVector<T> randn(int length);             // Create normal random vector
    ...
    IVector<T> set(int index, T value);              // Set element at index
    IVector<T> slice(int start, int end);            // Get slice of vector
    IVector<T> slice(int[] indices);                 // Get elements at specific indices
    
    // 基本数学运算 / Basic mathematical operations
    IVector<T> add(IVector<T> other);                // Vector addition
    IVector<T> add(T scalar);                        // Scalar addition
    IVector<T> subtract(IVector<T> other);          // Vector subtraction
    IVector<T> subtract(T scalar);                   // Scalar subtraction
    IVector<T> multiply(IVector<T> other);          // Element-wise multiplication
    IVector<T> multiply(T scalar);                   // Scalar multiplication
    IVector<T> divide(IVector<T> other);            // Element-wise division
    IVector<T> divide(T scalar);                     // Scalar division
    
    // 线性代数运算 / Linear algebra operations
    double dot(IVector<T> other);                    // Dot product (inner product)
    IVector<T> dot(IMatrix<T> matrix);               // row @ matrix; same as mmul(matrix); NumPy np.dot(v, M)
    double norm1();                                    // L1 norm
    double norm2();                                    // L2 norm
    double normInf();                                  // Infinity norm
    IVector<T> normalize();                          // Normalize vector
    double cosine(IVector<T> other);                  // Cosine similarity
    double euclideanDistance(IVector<T> other);       // Euclidean distance
    double manhattanDistance(IVector<T> other);       // Manhattan distance
    
    // 统计函数 / Statistical functions
    double sum();                                      // Sum of elements
    double mean();                                     // Mean of elements
    double variance();                                 // Variance of elements
    ...
    IVector<T> abs();                                 // Absolute value
    IVector<T> sqrt();                                // Square root
    IVector<T> exp();                                 // Exponential
    IVector<T> log();                                 // Natural logarithm
    IVector<T> sin();                                 // Sine
    IVector<T> cos();                                 // Cosine
    IVector<T> tan();                                 // Tangent
    IVector<T> pow(T exponent);                       // Power
    
    // 比较操作 / Comparison operations
    IVector<T> equals(IVector<T> other);             // Element-wise equality
    IVector<T> greater(IVector<T> other);            // Element-wise greater than
    IVector<T> less(IVector<T> other);               // Element-wise less than
    IVector<T> greaterEqual(IVector<T> other);       // Element-wise greater or equal
    IVector<T> lessEqual(IVector<T> other);          // Element-wise less or equal
    
    // 排序和查找 / Sorting and searching
    IVector<T> sort();                                // Sort in ascending order
    IVector<T> sortDescending();                      // Sort in descending order
    int[] argsort();                                   // Get sort indices (ascending)
    int[] argsortDescending();                         // Get sort indices (descending)
    boolean contains(T value);                         // Check if value exists
    int indexOf(T value);                              // Get index of value
}
```
...
矩阵操作的核心接口，为二维数据结构提供全面的数学运算。实际泛型签名为 `IMatrix<T extends Number>`；以下示例使用 `T` 表示元素类型，实际调用时由 `IDoubleMatrix`（`T=Double`）或 `IFloatMatrix`（`T=Float`）特化。
The actual generic signature is `IMatrix<T extends Number>`; below, `T` represents the element type, specialized by `IDoubleMatrix` (`T=Double`) or `IFloatMatrix` (`T=Float`).

```java
public interface IMatrix<T extends Number> {
    // 工厂方法 / Factory methods
    static IMatrix<T> of(float[][] array);           // Create matrix from 2D float array
    static IMatrix<T> of(double[][] array);          // Create matrix from 2D double array
    static IMatrix<T> of(int[][] array);             // Create matrix from 2D int array
    static IMatrix<T> zeros(int rows, int cols);     // Create zero matrix
    static IMatrix<T> ones(int rows, int cols);      // Create matrix of ones
    static IMatrix<T> eye(int size);                 // Create identity matrix
    static IMatrix<T> rand(int rows, int cols);      // Create random matrix
    static IMatrix<T> randn(int rows, int cols);     // Create normal random matrix
    static IMatrix<T> diag(IVector<T> diagonal);    // Create diagonal matrix
    ...
    // 元素访问 / Element access
    T get(int row, int col);                           // Get element at (row, col)
    IVector<T> set(int row, int col, T value);        // Set element at (row, col)
    IVector<T> getRow(int row);                       // Get row vector
    IVector<T> getColumn(int col);                    // Get column vector
    IVector<T> setRow(int row, IVector<T> vector);    // Set row vector
    IVector<T> setColumn(int col, IVector<T> vector); // Set column vector
    IMatrix<T> slice(int startRow, int endRow, int startCol, int endCol); // Get submatrix
    
    // 基本数学运算 / Basic mathematical operations
    IMatrix<T> add(IMatrix<T> other);                 // Matrix addition
    IMatrix<T> add(T scalar);                         // Scalar addition
    IMatrix<T> subtract(IMatrix<T> other);            // Matrix subtraction
    IMatrix<T> subtract(T scalar);                    // Scalar subtraction
    IMatrix<T> multiply(IMatrix<T> other);            // Element-wise multiplication
    IMatrix<T> multiply(T scalar);                    // Scalar multiplication
    IMatrix<T> divide(IMatrix<T> other);              // Element-wise division
    IMatrix<T> divide(T scalar);                      // Scalar division
    
    // 矩阵运算 / Matrix operations
    IMatrix<T> mmul(IMatrix<T> other);                // Matrix multiplication (NumPy np.dot(A,B) for 2D)
    T frobeniusInnerProduct(IMatrix<T> other);         // sum(A*B) same shape; not 2D matmul — use mmul
    IVector<T> mmul(IVector<T> vector);               // Matrix-vector multiplication
    IMatrix<T> transpose();                           // Matrix transpose
    IMatrix<T> inverse();                             // Matrix inverse
    double determinant();                              // Matrix determinant
    double trace();                                    // Matrix trace
    int rank();                                        // Matrix rank
    
    // 线性代数 / Linear algebra
    Tuple2<IVector<T>, IMatrix<T>> eigenDecomposition(); // Eigenvalue decomposition
    Tuple3<IMatrix<T>, IVector<T>, IMatrix<T>> svd();    // Singular value decomposition
    IMatrix<T> pseudoInverse();                        // Moore-Penrose pseudo-inverse
    double condition();                                // Condition number
    double norm1();                                    // L1 norm
    double norm2();                                    // L2 norm
    double normInf();                                  // Infinity norm
    ...
    double variance();                                 // Variance of all elements
    double std();                                      // Standard deviation
    IVector<T> rowSum();                              // Sum of each row
    IVector<T> rowMean();                             // Mean of each row
    IVector<T> rowVariance();                         // Variance of each row
    IVector<T> rowStd();                              // Standard deviation of each row
    IVector<T> columnSum();                           // Sum of each column
    IVector<T> columnMean();                          // Mean of each column
    IVector<T> columnVariance();                      // Variance of each column
    IVector<T> columnStd();                           // Standard deviation of each column
    
    // 数据变换 / Data transformation
    IMatrix<T> center();                              // Center matrix (subtract column means)
    IMatrix<T> standardize();                         // Standardize matrix
    IMatrix<T> normalize();                           // Normalize matrix
    IMatrix<T> covariance();                          // Compute covariance matrix
    
    // 通用函数 / Universal functions
    IMatrix<T> abs();                                 // Absolute value
    IMatrix<T> sqrt();                                // Square root
    IMatrix<T> exp();                                 // Exponential
    IMatrix<T> log();                                 // Natural logarithm
    IMatrix<T> sin();                                 // Sine
    IMatrix<T> cos();                                 // Cosine
    IMatrix<T> tan();                                 // Tangent
    IMatrix<T> pow(T exponent);                       // Power

    // 与 NumPy / 常见数值库名称对齐的补充 / NumPy-aligned extras
    IMatrix<T> mmul(Number scalar);                   // 同 multiplyScalar；mmul(IMatrix) 仍为矩阵乘 / like A*s in NumPy
    IMatrix<T> outer(IMatrix<T> other);              // np.outer(A.ravel(), B.ravel()) 行优先展平 / row-major flatten
    IMatrix<T> kron(IMatrix<T> other);               // np.kron(A, B)
    Tuple2<IVector<T>, IMatrix<T>> qrEigen();        // 与 eigen() 相同；短名 / same as eigen(), short name
    Tuple2<IVector<T>, IMatrix<T>> qrEigenDecomposition(); // 与 qrEigen()、eigen() 相同 / same as qrEigen()
}
```
...
public interface IRegression {
    ...
    IRegression fit(IMatrix features, IVector labels); // Train regression model; get result via getResult()
    RegressionResult getResult();                     // Get training result after fit()
...
> **注意**: 以下 CPU 计算工具类为内部实现类，用户通常不需要直接使用。这些类由框架内部自动调用。

> **Note**: The following CPU computing utility classes are internal implementation classes that users typically don't need to use directly. These classes are automatically called by the framework internally.

### CPU 计算工具类 / CPU Computing Utility Classes

- **CPUComputeFloatUtils**: Float 类型 CPU 计算工具（内部实现）/ Float type CPU compute helper (internal)
- **CPUComputeDoubleUtils**: Double 类型 CPU 计算工具（内部实现）/ Double type CPU compute helper (internal)

**GPU 支持状态 / GPU Support Status**: GPU 加速功能已于 v0.3.7 暂时移除。当前所有计算均为纯 CPU 实现，包含 SIMD（Vector API）和 ForkJoinPool 并行化路径。/ GPU acceleration was temporarily removed in v0.3.7. All computation is currently CPU-only, with SIMD (Vector API) and ForkJoinPool parallelism paths.
...
// 矩阵运算 / Matrix operations
IMatrix<Double> result = matrix.add(matrix).multiplyByScalar(2.0);
...
#### 3. 机器学习 / Machine Learning
```java
// 线性回归 / Linear regression（推荐工厂方法 / recommended factory method）
var lr = ML.reg.linear(0.0, 0.1);  // L1, L2 正则化系数 / regularization coefficients
lr.fit(X, y);
var regResult = lr.getResult();
IVector<Double> predictions = lr.predict(X_test);

// 逻辑回归 / Logistic regression
var logReg = ML.clf.logisticRegression(0.0, 0.0);
logReg.fit(X, labels);
var clfResult = logReg.getResult();
...
// ML 训练：用 matrix()/constant() 包装数据，一行代码替代手写梯度
...
var optimalW = result.getOptimalPoint();
...
var z1 = AD.odeint(
    z -> z.tanh(), z0, 0.0, 2.0, 0.1);
var odeLoss = z1.square().sum();
odeLoss.backward();  // adjoint method 自动反向传播
```
...
var pca = ML.dr.pca(2);
var reduced = pca.fitTransform(data); // 降到2维

// t-SNE降维 / t-SNE dimensionality reduction
var tsne = ML.dr.tsne(2);
var embedded = tsne.fitTransform(data);
```
...
4. **性能优化**: 对于大规模数据，框架会自动启用 ForkJoinPool 并行计算（如 BLAS-3 SVD 的 trailing update）；添加 `--add-modules jdk.incubator.vector` 可启用 SIMD 路径 / For large-scale data, the framework automatically enables ForkJoinPool parallelism (e.g., BLAS-3 SVD trailing updates); add `--add-modules jdk.incubator.vector` for SIMD paths
...
public class TimeSeriesData {
    ...
    public TimeSeriesData(List<LocalDateTime> timestamps, IVector<Double> values, String variableName);
    
    // 从时间戳和矩阵数据创建 / Create from timestamps and matrix data
    public TimeSeriesData(List<LocalDateTime> timestamps, IMatrix<Double> data, String[] variableNames);
    ...
    TimeSeriesData toUnivariate(String variableName);
...
public class TimeSeriesForecasting {
    ...
    static final double DEFAULT_CONFIDENCE = 0.95;  // 默认置信水平
    ...
    // 所有方法接受 IVector<Double>，默认 95% 置信度
    
    static ForecastResult movingAverage(IVector<Double> data, int windowSize, int steps, double confidence);
    static ForecastResult exponentialSmoothing(IVector<Double> data, double alpha, int steps, double confidence);
    static ForecastResult linearRegression(IVector<Double> data, int steps, double confidence);
    
    // ========== 高级预测方法 / Advanced Forecasting Methods ==========
    
    static ForecastResult arimaForecast(IVector<Double> data, int p, int d, int q, int steps, double confidence);
    static ForecastResult seasonalForecast(IVector<Double> data, int period, int steps, double confidence);
    static ForecastResult holtWintersForecast(IVector<Double> data, double alpha, double beta, double gamma,
                                              int period, int steps, double confidence);
    static ForecastResult garchForecast(IVector<Double> data, int p, int q, int steps, double confidence);
    static ForecastResult stateSpaceForecast(IVector<Double> data, double sigmaEta, double sigmaZeta,
                                             double sigmaEpsilon, int steps, double confidence);
    static ForecastResult autoForecast(IVector<Double> data, int steps, double confidence);
}
```

> **推荐使用 `TSA.forecast` 包装器**：方法名更简洁（`TSA.forecast.arima(y,1,0,1,10)`），默认 95% 置信度。每个方法提供两个重载——带/不带 `confidence` 参数。 / Prefer `TSA.forecast` wrapper: shorter names, defaults to 95% confidence, two overloads per method.
...
public class TimeSeriesFiltering {
    ...
    // 所有方法接受 IVector<Double>
    
    static FilterResult movingAverage(IVector<Double> data, int windowSize);
    static FilterResult exponentialSmoothing(IVector<Double> data, double alpha);
    static FilterResult gaussianFilter(IVector<Double> data, double sigma);
    static FilterResult medianFilter(IVector<Double> data, int windowSize);
    
    // ========== 频域滤波方法 / Frequency Domain Filtering Methods ==========
    
    static FilterResult lowPassFilter(IVector<Double> data, double cutoffFreq, double samplingRate, int order);
    static FilterResult highPassFilter(IVector<Double> data, double cutoffFreq, double samplingRate, int order);
    static FilterResult bandPassFilter(IVector<Double> data, double lowFreq, double highFreq,
                                       double samplingRate, int order);
    static FilterResult adaptiveFilter(IVector<Double> data, double learningRate);
}
```
...
public class TimeSeriesDecomposition {
    ...
    static DecompositionResult classicalDecomposition(IVector<Double> data, int period, DecompositionModel model);
    
    // X-13ARIMA-SEATS分解 / X-13ARIMA-SEATS decomposition
    static DecompositionResult x13Decomposition(IVector<Double> data, int period);
    
    // STL分解 / STL decomposition
    static DecompositionResult stlDecomposition(IVector<Double> data, int period, int seasonalWindow, int trendWindow);
    
    // 小波分解 / Wavelet decomposition
    static DecompositionResult waveletDecomposition(IVector<Double> data, String wavelet, int levels);
}
```
