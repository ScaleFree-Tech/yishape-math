...
public class MLDataPreparationExample {
    public static void main(String[] args) {
        try {
            ...
            // 这里可以使用 ML.dr.pca() 进行主成分分析
            // ML.dr.pca() can be used here for principal component analysis
...
1. **数据类型** / **Data Types**: 支持String、Numeric(Double)、Integer、Boolean、DateTime五种数据类型 / Supports String, Numeric(Double), Integer, Boolean, DateTime data types
...
```java
// 只取数值列（排除字符串列）
var numericDf = df.selectNumeric();
