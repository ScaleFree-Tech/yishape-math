...
public interface IOnlineOptimizer {
    ...
    default <T> IVector step(T[] samples, BiFunction<IVector, T, Double> lossFunction) {
        ...
        IVector avgGradient = totalGradient.multiplyByScalar(1.0 / samples.length);
...
IVector newPoint = currentPoint.add(searchDirection.multiplyByScalar(optimalStep));
...
```java
public class RereSimplexLinProgSolver implements ILinProgSolver {
...
RereIntegerProg solver2 = new RereIntegerProg(new RereSimplexLinProgSolver());
...
- GPU加速优化 / GPU-accelerated optimization（暂时移除，未来版本可能回归 / temporarily removed in v0.3.7, may return in future versions）
...
## 自动微分集成 / Autodiff Integration

对于需要手写梯度的优化问题，可以使用 `Autodiff.optimize()` 自动推导梯度，消除手写梯度代码：

```java
// 传统方式：需手写 objFun + grdFun（~30 行代码）
// Autodiff 方式：只需定义 loss（~5 行代码）
var result = Autodiff.optimize(w0,
    w -> Xc.matmul(w).sub(y).square().mean(),  // MSE loss
    Opts.lbfgs()
);
var optimal = result.getOptimalPoint();
```

`Autodiff.optimize()` 支持所有 `IOptimizer` 优化器（L-BFGS、DFP、共轭梯度、最速下降），也支持在线优化器（SGD、Adam）通过 `Autodiff.autogradOptimizer()` 和 `Autodiff.onlineLearn()`。详见 [Autodiff-Reference.md](Autodiff-Reference.md)。
