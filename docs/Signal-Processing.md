...
IMatrix<Double> stft = Signals.analyze.shortTimeFourierTransform(
    signal, 256, 64, SignalUtilities.WindowType.HANNING
);
```

#### 3.1.1 频谱图 (Spectrogram) / Spectrogram

```java
// 幅度频谱图 / Magnitude spectrogram
IMatrix<Double> spec = Signals.analyze.spectrogram(
    signal, 256, 128, SignalUtilities.WindowType.HANNING
);

// 功率频谱图 / Power spectrogram
IMatrix<Double> pSpec = Signals.analyze.powerSpectrogram(
    signal, 256, 128, SignalUtilities.WindowType.HAMMING
);

// 对数频谱图 (dB) / Log spectrogram (dB)
IMatrix<Double> logSpec = Signals.analyze.logSpectrogram(
    signal, 256, 128, SignalUtilities.WindowType.BLACKMAN, -120.0
);
```

#### 3.1.2 Wigner-Ville 分布 / Wigner-Ville Distribution

```java
// Wigner-Ville 分布 (默认最大频率 bins) / Wigner-Ville distribution (default max freq bins)
IMatrix<Double> wvd = Signals.analyze.wignerVilleDistribution(signal);

// 指定频率分辨率 / With specified frequency bins
IMatrix<Double> wvdCustom = Signals.analyze.wignerVilleDistribution(signal, 128);
```

Wigner-Ville 分布提供最高的时频分辨率，但存在交叉项干扰。对于多分量信号，推荐使用 Cohen 类分布抑制交叉项。

The Wigner-Ville distribution provides the highest time-frequency resolution but suffers from cross-term interference. For multi-component signals, Cohen-class distributions are recommended to suppress cross-terms.

#### 3.1.3 Cohen 类时频分布 / Cohen-Class Time-Frequency Distributions

```java
// Choi-Williams 分布 (指数核，抑制交叉项) / Choi-Williams distribution (exponential kernel)
IMatrix<Double> cw = Signals.analyze.choiWilliams(signal, 1.0);  // sigma 控制平滑程度

// Born-Jordan 分布 (sinc 核) / Born-Jordan distribution (sinc kernel)
IMatrix<Double> bj = Signals.analyze.bornJordan(signal);

// Margenau-Hill 分布 (cos 核) / Margenau-Hill distribution (cosine kernel)
IMatrix<Double> mh = Signals.analyze.margenauHill(signal);

// 伪 Wigner-Ville 分布 (时域加窗) / Pseudo Wigner-Ville distribution (lag windowing)
IMatrix<Double> pwvd = Signals.analyze.pseudoWignerVille(signal, 32);  // 平滑窗口大小

// 通用 Cohen 类框架 / Generic Cohen-class framework
IMatrix<Double> custom = Signals.analyze.cohenClass(signal, kernelMatrix, maxLag);

// 获取幅度谱 (复数矩阵 → 幅度) / Get magnitude from complex TF matrix
IMatrix<Double> mag = CohenClassDistribution.magnitude(tfMatrix);
```

**核函数选择指南 / Kernel Selection Guide**:

| 分布 | 核函数 | 特点 |
|------|--------|------|
| Wigner-Ville | 1 (无核) | 最高分辨率，交叉项严重 |
| Choi-Williams | exp(-(ξτ)²/σ) | 指数抑制交叉项，σ 越大抑制越强 |
| Born-Jordan | sinc(ξτ) | 中等平滑，保留自项 |
| Margenau-Hill | cos(ξτ/2) | Rihaczek 分布的实部 |
| Pseudo WVD | 时域窗函数 | 频域平滑，结构简单 |
...
| 短时傅里叶变换 / STFT | `Signals.analyze.shortTimeFourierTransform()` | `scipy.signal.stft()` | 短时傅里叶变换 / Short-time Fourier transform |
| 频谱图 / Spectrogram | `Signals.analyze.spectrogram()` | `scipy.signal.spectrogram()` | 幅度频谱图 / Magnitude spectrogram |
| 功率频谱图 / Power spectrogram | `Signals.analyze.powerSpectrogram()` | `scipy.signal.spectrogram()` | 功率频谱图 / Power spectrogram |
| 对数频谱图 / Log spectrogram | `Signals.analyze.logSpectrogram()` | `librosa.power_to_db()` | 对数(dB)频谱图 / Log-scale spectrogram |
| Wigner-Ville 分布 / WVD | `Signals.analyze.wignerVilleDistribution()` | `tftb.WignerVilleDistribution()` | Wigner-Ville 时频分布 / Wigner-Ville distribution |
| Choi-Williams 分布 | `Signals.analyze.choiWilliams()` | `tftb.ChoiWilliamsDistribution()` | Choi-Williams 时频分布 |
| Born-Jordan 分布 | `Signals.analyze.bornJordan()` | `tftb.BornJordanDistribution()` | Born-Jordan 时频分布 |
| 伪 Wigner-Ville / Pseudo WVD | `Signals.analyze.pseudoWignerVille()` | `tftb.SmoothedPseudoWignerVilleDistribution()` | 平滑伪 Wigner-Ville 分布 |
...
IVector<Double> normalized = filtered.multiplyByScalar(1.0 / maxAmp);
```
