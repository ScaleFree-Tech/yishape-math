...
Double mean = v.multiplyByScalar(2.0).addScalar(1.0).mean();
...
IVector<Double> result3 = v1.multiplyByScalar(3.0);
...
// 元素设置 / Element setting (set() returns void, modifies in place)
v1.set(2, 10.0);                                    // 设置指定位置的值
v1.setFromTo(1, 4, new double[]{10.0, 20.0, 30.0}); // 范围设置

// 数据填充 / Data filling (fill() returns void, modifies in place)
v1.fill(0.0);                                       // 填充所有元素为0
...
- 稀疏向量 / Sparse vectors
- GPU加速向量 / GPU-accelerated vectors（暂时移除，未来版本可能回归 / temporarily removed in v0.3.7, may return in future versions）
...
| `v1.multiplyByScalar(s)` | `v1 * s` | 标量乘法 / Scalar multiplication |
...
```java
IVector<Double> a = Linalg.vector(new double[]{1.0, 2.0, 3.0});
IVector<Double> b = a.multiplyByScalar(2.0);  // a 不变，b 是新向量
System.out.println(a);  // [1.0, 2.0, 3.0]
System.out.println(b);  // [2.0, 4.0, 6.0]
```

若发现原向量被改变，检查是否误用了带 `Into` 后缀的方法（如 `a.multiplyByScalarInto(target, scalar)`），这类方法会将结果写入目标向量。
