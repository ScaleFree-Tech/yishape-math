...
public class SolverPerformanceComparisonExample {
    public static void main(String[] args) {
        ...
        testSolver("单纯形法 / Simplex Method",
                new RereIntegerProg(new RereSimplexLinProgSolver()),
                c, A, b, integerVars);
...
## 自动微分优化示例 / Autodiff Optimization Examples

### 使用 Autodiff 替代手写梯度 / Replace Hand-Written Gradients with Autodiff
...
public class AutodiffOptimizationExample {
    public static void main(String[] args) {
        System.out.println("=== 自动微分优化示例 / Autodiff Optimization Example ===");

        // 定义目标：minimize f(w) = Σ(w_i - target_i)²
        // 无需手写梯度！AD.optimize 自动推导
        var target = AD.constant((IDoubleVector) Linalg.vector(new double[]{1.0, 1.0}));
        var initX = Linalg.vector(new double[]{-1.0, -1.0});
        var result = AD.optimize(initX,
            w -> w.sub(target).square().sum(),
            Opts.lbfgs()
        );
...
public class OnlineAutodiffExample {
    public static void main(String[] args) {
        System.out.println("=== 在线学习 + Autodiff / Online Learning with Autodiff ===");

        // 使用 AD 包装在线 Adam 优化器
        // 无需手写梯度，loss 定义即梯度推导
        var baseOptimizer = Opts.onlineAdam(0.001);
        var opt = AD.autogradOptimizer(baseOptimizer,
            (w, target) -> w.sub(target).square().sum()
        );

        // 初始化参数 / Initialize params
        var params = Linalg.zeros(5);
        opt.initialize(params);

        // 在线学习：每个目标自动计算梯度 / Each step auto-computes gradients
        var target1 = AD.constant(Linalg.vector(new double[]{1, 2, 3, 4, 5}));
        opt.step(target1);

        var target2 = AD.constant(Linalg.vector(new double[]{2, 3, 4, 5, 6}));
        opt.step(target2);

        System.out.println("学到的参数: " + opt.getCurrentParams());
    }
}
```
...
- **大规模优化**：处理高维优化问题
- **线搜索**：精确控制步长选择
- **自动微分优化**：使用 `Autodiff.optimize()` 替代手写梯度，在线学习 + autodiff 自动梯度
