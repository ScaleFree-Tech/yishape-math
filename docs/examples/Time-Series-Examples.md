...
public class BasicTimeSeriesExample {
    public static void main(String[] args) {
        ...
        // 使用TSA工厂方法 / Using TSA factory method
        TimeSeriesData timeSeries3 = TSA.data(data, "temperature3");
...
public class BasicStatisticalAnalysisExample {
    public static void main(String[] args) {
        ...
        TimeSeriesData timeSeries = TSA.data(data, "sales");
        
        // 使用TimeSeriesAnalyzer进行分析 / Using TimeSeriesAnalyzer for analysis
        TimeSeriesAnalyzer analyzer = new TimeSeriesAnalyzer(data, "sales");
    ...
    private static IVector<Double> generateSampleTimeSeries(int length) {
        ...
        IVector<Double> trend = Linalg.range(length).multiplyByScalar(0.01);
        IVector<Double> seasonal = generateSeasonalComponent(length, 12);
        IVector<Double> noise = Linalg.randn(length).multiplyByScalar(0.1);
...
public class SimpleForecastingExample {
    public static void main(String[] args) {
        ...
        TimeSeriesData timeSeries = TSA.data(data, "value");
        
        // 使用TSA.forecast子门面进行预测 / Using TSA.forecast sub-facade for forecasting

        // 简单移动平均预测 / Simple moving average forecasting
        System.out.println("=== 简单移动平均预测 / Simple Moving Average Forecasting ===");
        ForecastResult smaResult = TSA.forecast.movingAverage(
            data, 5, 10, 0.95);
    ...
    private static IVector<Double> generateSampleTimeSeries(int length) {
        ...
        IVector<Double> trend = Linalg.range(length).multiplyByScalar(0.02);
        IVector<Double> noise = Linalg.randn(length).multiplyByScalar(0.1);
...
public class TimeSeriesDecompositionExample {
    public static void main(String[] args) {
        ...
        DecompositionResult stlResult =
            TSA.decompose.stl(data, 12, 7, 21);
    ...
    private static IVector<Double> generateSeasonalTimeSeries(int length) {
        ...
        IVector<Double> trend = Linalg.range(length).multiplyByScalar(0.02);
        IVector<Double> seasonal = generateSeasonalComponent(length, 12);
        IVector<Double> noise = Linalg.randn(length).multiplyByScalar(0.05);
...
public class TimeSeriesFilteringExample {
    public static void main(String[] args) {
        ...
        IVector<Double> noisyData = cleanData.add(Linalg.randn(100).multiplyByScalar(0.2));
        TimeSeriesData timeSeries = TSA.data(noisyData, "signal");
        ...
        System.out.println("\n=== 高斯滤波 / Gaussian Filtering ===");
        FilterResult gaussianResult = TSA.filter.gaussian(
            noisyData, 1.0);
        ...
        System.out.println("\n=== 中值滤波 / Median Filtering ===");
        FilterResult medianResult = TSA.filter.median(
            noisyData, 5);
...
public class ARIMAForecastingExample {
    public static void main(String[] args) {
        ...
        ForecastResult arimaResult = TSA.forecast.arima(
            data, 1, 1, 1, 20, 0.95);
        ...
        ForecastResult arima221 = TSA.forecast.arima(
            data, 2, 1, 2, 20, 0.95);
        System.out.println("ARIMA(2,1,2) MSE: " + arima221.mse);
        
        // ARIMA(0,1,1) / ARIMA(0,1,1)
        ForecastResult arima011 = TSA.forecast.arima(
            data, 0, 1, 1, 20, 0.95);
...
public class CompleteAnalysisWorkflowExample {
    public static void main(String[] args) {
        ...
        TimeSeriesData timeSeries = TSA.data(data, "complex_signal");
        ...
        FilterResult filtering =
            TSA.filter.movingAverage(preprocessed.getVariable(0), 3);
        ...
        ForecastResult forecast =
            TSA.forecast.auto(filtering.filtered, 30, 0.95);
        ...
        TimeSeriesAnalyzer analyzer = new TimeSeriesAnalyzer(data, "complex_signal");
    ...
    private static TimeSeriesData preprocessData(TimeSeriesData timeSeries) {
        ...
        return TimeSeriesData.of(cleanData, timeSeries.getVariableNames()[0]);
    }
...
public class MultivariateTimeSeriesExample {
    public static void main(String[] args) {
        ...
        TimeSeriesData multiTimeSeries = TSA.data(data, columnNames, timestamps);
        ...
        for (String columnName : columnNames) {
            ...
            // 预测 / Forecasting
            ForecastResult forecast =
                TSA.forecast.auto(variable, 10, 0.95);
...
public class AdvancedFilteringDecompositionExample {
    public static void main(String[] args) {
        ...
        DecompositionResult waveletResult =
            TSA.decompose.wavelet(data, "db4", 4);
