...
public class NewsvendorExample {
    ...
    public static void main(String[] args) {
        ...
        // 计算临界比率
        var criticalRatio = (sellingPrice - purchaseCost) / (sellingPrice - purchaseCost + shortageCost);
        System.out.printf("   临界比率: %.4f%n", criticalRatio);
        
        // 理论最优订货量
        var theoreticalOptimal = model.computeTheoreticalOptimalQuantity();
        System.out.printf("   理论最优订货量: %.2f 份%n", theoreticalOptimal);
        
        // 理论最大期望利润
        var theoreticalMaxProfit = model.computeExpectedProfit(theoreticalOptimal);
        System.out.printf("   理论最大期望利润: %.2f 元%n", theoreticalMaxProfit);
        
        // 4. 数值优化求解
        System.out.println("\n3. 数值优化求解:");
        var numericalResult = model.solveNumerically();
        
        var numericalOptimal = numericalResult.getOptimalPoint().get(0).doubleValue();
        var numericalMaxProfit = -numericalResult.getOptimalValue(); // 负号因为目标函数是负利润
        ...
        // 5. 结果比较
        System.out.println("\n4. 结果比较:");
        var quantityDifference = Math.abs(theoreticalOptimal - numericalOptimal);
        var profitDifference = Math.abs(theoreticalMaxProfit - numericalMaxProfit);
        ...
        for (double cost : costVariations) {
            var variantModel = new NewsvendorModel(
