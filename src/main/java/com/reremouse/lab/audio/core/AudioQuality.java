...
public enum AudioQuality {
    ...
    /**
     * 根据信噪比获取音频质量 / Get audio quality based on SNR
     *
     * @param snr 信噪比(dB) / Signal-to-noise ratio (dB)
     * @return 音频质量 / Audio quality
     */
    public static AudioQuality fromSNR(double snr) {
        if (snr < 0) {
            return POOR;
        } else if (snr < 20) {
            return POOR;
    ...
    /**
     * 根据动态范围获取音频质量 / Get audio quality based on dynamic range
     *
     * @param dynamicRange 动态范围(dB) / Dynamic range (dB)
     * @return 音频质量 / Audio quality
     */
    public static AudioQuality fromDynamicRange(double dynamicRange) {
        if (dynamicRange < 20) {
            return POOR;
    ...
    /**
     * 比较质量等级 / Compare quality levels
     *
     * @param other 另一个质量等级 / Another quality level
     * @return 比较结果 / Comparison result
     */
    public int compareQuality(AudioQuality other) {
