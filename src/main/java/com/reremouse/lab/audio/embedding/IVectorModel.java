 ...
 * @author lteb2
 */
public class IVectorModel implements IAudioEmbedding {
     ...
     */
    public IVectorModel(int len, int numComponents, int featureDim) {
        this.len = len;
        this.numComponents = numComponents;
        this.featureDim = featureDim;
        this.supervectorDim = numComponents * featureDim;
        this.random = new Random(42); // 固定种子以保证可重复性 / Fixed seed for reproducibility
    ...
    private Tuple2<IVector<Double>, IVector<Double>> computeBaumWelchStats(IVector<Double> features) {
        ...
        IVector<Double> F = IVector.zeros(numComponents * featureDim, Double.class);  // 一阶统计量
    ...
    private IMatrix<Double> computeTotalVariabilityMatrix(List<IVector<Double>> features) {
        IMatrix<Double> S = IMatrix.zeros(supervectorDim, supervectorDim);
