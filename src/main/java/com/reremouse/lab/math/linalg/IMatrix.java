...
public interface IMatrix {
 ...
 Creates a matrix instance with the given IVector array, each IVector representing a row of the matrix
    ...
    public static IMatrix randn(int rows, int cols, float mean, float std) {
        return LAs.randn(rows, cols, mean, std);
    }
 ...
 column
    ...
    public static IMatrix diag(float[] diagonal) {
        return LAs.diag(diagonal);
    }
    
    public static IMatrix diag(IVector diagonal) {
        return LAs.diag(diagonal.getData());
    }
    
    /**
     * 获取矩阵对角线元素 / Get matrix diagonal elements
     * <p>
     * 返回矩阵主对角线上的元素组成的向量 Returns a vector containing the main diagonal elements of the matrix
     * </p>
     *
    ...
    public static IMatrix rand(int rows, int cols, long seed) {
        return LAs.rand(rows, cols, seed);
    }
    ...
    public static IMatrix randn(int rows, int cols, long seed) {
        return LAs.randn(rows, cols, seed);
    }
    ...
    public static IMatrix fromArray(float[] data, int rows, int cols) {
        return LAs.fromArray(data, rows, cols);
    }
     ...
     * 设置子矩阵 / Set submatrix
     * <p>
     * 将指定子矩阵设置到当前矩阵的指定位置 Sets the specified submatrix to the current matrix at the specified position
     * </p>
     *
     * @param startRow 起始行索引 / Start row index
     * @param endRow 结束行索引（不包含） / End row index (exclusive)
