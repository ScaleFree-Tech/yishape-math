 ...
 * {@code
 ...
 IMatrix<Float> result = matrix.add(other).mmul(2.0f);
...
public class RereFloatMatrix implements IFloatMatrix {
     ...
     */
    private static final boolean GPU_ENABLED = GPUComputeFloatUtils.isGPUAvailable();
     ...
     */
    private RereFloatMatrix reset(int rows, int cols) {
    ...
    public static void shutdown() {
        ...
        GPUComputeFloatUtils.cleanup(); // 清理GPU资源
    }
    ...
    public static String getGPUInfo() {
        return GPUComputeFloatUtils.getGPUInfo();
    }
     ...
     */
    @Override
    public IMatrix<Float> sub(Float scalar) {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuMatrixScalarSub(this, scalar);
            } catch (Exception e) {
                // GPU失败时回退到CPU
        ...
        // 使用CPU计算工具类
        return CPUComputeFloatUtils.matrixScalarSub(data, scalar);
    }
     ...
     */
    @Override
    public IMatrix<Float> sub(IMatrix<Float> other1) {
        if (other1 == null) {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuMatrixSub(this, other0);
            } catch (Exception e) {
                // GPU失败时回退到CPU
        ...
        // 使用CPU计算工具类
        return CPUComputeFloatUtils.matrixSub(data, otherData);
    }
    ...
    public IMatrix<Float> multiplyScalar(Float scalar) {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuMatrixScalarMultiply(this, scalar);
            } catch (Exception e) {
                // GPU失败时回退到CPU
        ...
        // 使用CPU计算工具类
        return CPUComputeFloatUtils.matrixScalarMultiply(data, scalar);
    }
     ...
     */
    @Override
    public IMatrix<Float> add(IMatrix<Float> other1) {
        if (other1 == null) {
            throw new IllegalArgumentException("输入矩阵不能为null / Input matrix cannot be null");
        }
        IFloatMatrix other0 = (IFloatMatrix) other1;
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuMatrixAdd(this, other0);
            } catch (Exception e) {
                // GPU失败时回退到CPU，这里不需要额外日志，GPUComputeUtils已经处理了
            }
        }

        // 使用CPU计算工具类
        return CPUComputeFloatUtils.matrixAdd(data, otherData);
    }
     ...
     */
    @Override
    public IMatrix<Float> divide(IMatrix<Float> other1) {
        if (other1 == null) {
            throw new IllegalArgumentException("输入矩阵不能为null / Input matrix cannot be null");
        }
        IFloatMatrix other0 = (IFloatMatrix) other1;
     ...
     */
    @Override
    public Float dot(IMatrix<Float> other1) {
        if (other1 == null) {
            throw new IllegalArgumentException("输入矩阵不能为null / Input matrix cannot be null");
        }
        IFloatMatrix other0 = (IFloatMatrix) other1;
     ...
     */
    @Override
    public IVector<Float> getColumn(int colIndex) {
        int rows = data.length;
     ...
     */
    @Override
    public IMatrix<Float> transposeInPlace() {
        var m = this.transposeNew();
        IFloatMatrix other0 = (IFloatMatrix) m;
     ...
     */
    @Override
    public IMatrix<Float> transposeNew() {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuMatrixTranspose(this);
            } catch (Exception e) {
                // GPU失败时回退到CPU
        ...
        // 使用CPU计算工具类
        return CPUComputeFloatUtils.matrixTranspose(data);
    }
    ...
    public Tuple2<IVector<Float>, IMatrix<Float>> eigen() {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                // 尝试使用GPU进行特征分解计算
                tp2 = GPUComputeFloatUtils.gpuEigenDecomposition(this);
            } catch (Exception e) {
            ...
            // CPUComputeUtils会根据矩阵性质自动选择最优算法
            tp2 = CPUComputeFloatUtils.eigen(this);
        }
        Tuple2<IVector<Float>, IMatrix<Float>> tt = new Tuple2(tp2._1, tp2._2);
        return tt;
    }
    ...
    public Tuple3<IMatrix<Float>, IVector<Float>, IMatrix<Float>> svd() {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                // 尝试使用GPU进行奇异值分解计算
                tp3 = GPUComputeFloatUtils.gpuSVD(this);
            } catch (Exception e) {
                // GPU计算失败时的容错处理：自动回退到CPU计算
            ...
            // CPUComputeUtils会根据矩阵特性自动选择最优算法
            tp3 = CPUComputeFloatUtils.svd(this);
        }
        Tuple3<IMatrix<Float>, IVector<Float>, IMatrix<Float>> tt = new Tuple3(tp3._1, tp3._2, tp3._3);
        return tt;
    }
     ...
     */
    public Tuple3<IMatrix<Float>, IVector<Float>, IMatrix<Float>> optimizedSVD() {
        int m = data.length;    // 矩阵行数
        int n = data[0].length; // 矩阵列数
        ...
        Tuple3<float[][], float[][], float[][]> bidiagResult = bidiagonalization();
        float[][] B = bidiagResult._1;  // 双对角矩阵（只有主对角线和次对角线非零）
        float[][] U = bidiagResult._2;  // 左变换矩阵U₁
        float[][] V = bidiagResult._3;  // 右变换矩阵V₁

        // 步骤2：对双对角矩阵应用分治算法
        // 分治算法是处理双对角矩阵SVD的高效方法
        Tuple2<float[], float[][]> svdResult = divideAndConquerSVD(B);
        float[] singularValues = svdResult._1;  // 奇异值数组
        float[][] Q = svdResult._2;             // 分治算法得到的变换矩阵

        // 步骤3：计算最终的U和V矩阵，确保矩阵的完整性
        // 需要将中间结果组合成完整的m×m和n×n矩阵
        float[][] finalU = new float[m][m];  // 最终的左奇异向量矩阵
        float[][] finalV = new float[n][n];  // 最终的右奇异向量矩阵
        ...
        return new Tuple3<>(new RereFloatMatrix(finalU), IFloatVector.of(singularValues), new RereFloatMatrix(finalV).transposeNew());
    }

    /**
     * 双对角化SVD算法
     */
    public Tuple3<IMatrix<Float>, IVector<Float>, IMatrix<Float>> bidiagonalSVD() {
        int m = data.length;
        int n = data[0].length;

        // 双对角化
        Tuple3<float[][], float[][], float[][]> bidiagResult = bidiagonalization();
        float[][] B = bidiagResult._1;  // 双对角矩阵
        float[][] U = bidiagResult._2;  // 左变换矩阵
        float[][] V = bidiagResult._3;  // 右变换矩阵

        // 对双对角矩阵应用QR算法
        Tuple2<float[], float[][]> svdResult = qrAlgorithmForBidiagonal(B);
        float[] singularValues = svdResult._1;
        float[][] Q = svdResult._2;

        // 计算最终的U和V，确保完整的矩阵
        float[][] finalU = new float[m][m];
        float[][] finalV = new float[n][n];
     ...
     */
    private void orthogonalizeMatrix(float[][] matrix) {
     ...
     * 传统SVD算法（修复版本）
     */
    public Tuple3<IMatrix<Float>, IVector<Float>, IMatrix<Float>> traditionalSVD() {
        int m = data.length;    // 行数
        int n = data[0].length; // 列数
        int rank = Math.min(m, n);

        // 计算A^T * A
        IMatrix<Float> AT = this.transposeNew();
        IMatrix<Float> ATA = ((RereFloatMatrix) AT).mmul(this);

        // 对A^T * A进行特征分解得到V和奇异值的平方
        Tuple2<IVector<Float>, IMatrix<Float>> eigenResult = ((RereFloatMatrix) ATA).eigen();
        ...
        for (int i = 0; i < rank; i++) {
            ...
            singularValues[i] = (float) Math.sqrt(Math.max(0, eigenVal));
        }
        ...
        for (int i = 0; i < rank; i++) {
            IVector<Float> vi = V.getColumn(indices[i]);
            for (int j = 0; j < n; j++) {
                VData[j][i] = vi.get(j);
            }
        }
        ...
        IMatrix<Float> sortedV = new RereFloatMatrix(VData);

        // 计算U = A * V * S^(-1)，确保U的正交性
        float[][] UData = new float[m][m]; // 完整的m×m矩阵
        for (int i = 0; i < rank; i++) {
            if (singularValues[i] > 1e-10) { // 避免除零
                IVector<Float> vi = sortedV.getColumn(i);
                // 计算 A * vi
        ...
        for (int j = 0; j < rank; j++) {
            ...
            if (norm > 1e-10) {
                ...
                // 如果向量线性相关，设置为单位向量
                for (int i = 0; i < m; i++) {
                    UData[i][j] = (i == j) ? 1.0f : 0.0f;
                }
            }
        }

        IMatrix<Float> U = new RereFloatMatrix(UData);

        // 返回U, S, V^T
        return new Tuple3<>(U, IFloatVector.of(singularValues), sortedV.transposeNew());
    }
     ...
     * @return 简化的SVD分解结果
     */
    private Tuple3<IMatrix<Float>, IVector<Float>, IMatrix<Float>> simplifiedSVD() {
        int m = data.length;
        int n = data[0].length;
        int rank = Math.min(m, n);
        ...
        frobeniusNorm = (float) Math.sqrt(frobeniusNorm);

        // 创建简化的U矩阵（基于原矩阵的列向量）
        float[][] UData = new float[m][rank];
        for (int i = 0; i < rank; i++) {
            for (int j = 0; j < m; j++) {
                UData[j][i] = (i < n) ? data[j][i] / frobeniusNorm : 0.0f;
            }
        }

        // 创建简化的V矩阵（基于原矩阵的行向量）
        float[][] VData = new float[n][rank];
        for (int i = 0; i < rank; i++) {
            for (int j = 0; j < n; j++) {
                VData[j][i] = (i < m) ? data[i][j] / frobeniusNorm : 0.0f;
            }
        }

        // 计算简化的奇异值（基于Frobenius范数的分布）
        float[] singularValues = new float[rank];
        float baseValue = frobeniusNorm / (float) Math.sqrt(rank);
        for (int i = 0; i < rank; i++) {
            // 使用指数衰减模拟真实奇异值分布
            singularValues[i] = baseValue * (float) Math.exp(-i * 0.1);
        }

        IMatrix<Float> U = new RereFloatMatrix(UData);
        IMatrix<Float> V = new RereFloatMatrix(VData);

        return new Tuple3<>(U, IFloatVector.of(singularValues), V.transposeNew());
    }
     ...
     * @return 简化的伪逆矩阵
     */
    private IMatrix<Float> simplifiedPseudoInverse() {
        int m = data.length;
        int n = data[0].length;
        ...
        float[][] pseudoInverseData = new float[n][m];
        float scale = 1.0f / (frobeniusNorm * frobeniusNorm + 1e-10f); // 避免除零
        ...
        return new RereFloatMatrix(pseudoInverseData);
    }
     ...
     */
    private Tuple2<float[][], float[][]> blockedHessenbergReduction() {
        int n = data.length;
     ...
     */
    private void applyHouseholderToHessenberg(float[][] matrix, float[] v, int startRow, int endRow, int startCol, int endCol) {
        int n = v.length;

        // 计算 w = matrix * v
        float[] w = new float[endRow - startRow + 1];
        ...
        for (int i = 0; i < w.length; i++) {
            for (int j = 0; j < n; j++) {
                matrix[startRow + i][startCol + j] -= 2.0f * w[i] * v[j];
            }
        }
    }
     ...
     * @return 海森伯格矩阵和变换矩阵
     */
    private Tuple2<float[][], float[][]> hessenbergReduction() {
        int n = data.length;
        float[][] H = new float[n][n];
        float[][] Q = new float[n][n];
        ...
        for (int k = 0; k < n - 2; k++) {
            ...
            float[][] P = new float[n - k - 1][n - k - 1];
            for (int i = 0; i < n - k - 1; i++) {
                for (int j = 0; j < n - k - 1; j++) {
                    P[i][j] = (i == j ? 1.0f : 0.0f) - 2.0f * v[i] * v[j];
                }
            }

            // 应用变换到H的子矩阵
            float[][] subH = new float[n - k - 1][n - k];
            for (int i = 0; i < n - k - 1; i++) {
                for (int j = 0; j < n - k; j++) {
            ...
            // P * subH
            float[][] PsubH = new float[n - k - 1][n - k];
            for (int i = 0; i < n - k - 1; i++) {
                for (int j = 0; j < n - k; j++) {
     ...
     */
    @Override
    public Tuple2<IVector<Float>, IMatrix<Float>> qrEigenDecomposition() {
        int n = data.length;  // 矩阵维度
     ...
     */
    private Tuple2<IVector<Float>, IMatrix<Float>> symmetricEigenDecomposition() {
        int n = data.length;  // 矩阵维度

        // 步骤1：三对角化预处理
        // 对于对称矩阵，海森伯格化简退化为三对角化
        // 三对角化将对称矩阵A转换为三对角矩阵T，使得A = QTQ^T
        Tuple2<float[][], float[][]> tridiagResult = tridiagonalReduction();
        float[][] T = tridiagResult._1;  // 三对角矩阵（只有主对角线和次对角线非零）
        float[][] Q = tridiagResult._2;  // 正交变换矩阵Q
        ...
        Tuple2<IFloatVector, IFloatMatrix> eigenResult = qrAlgorithmForTridiagonal(T);
        IVector<Float> eigenvalues = eigenResult._1;    // 三对角矩阵T的特征值
        IFloatMatrix eigenvectors = eigenResult._2;   // 三对角矩阵T的特征向量

        // 步骤3：变换回原坐标系的特征向量
        // 数学关系：A = Q*T*Q^T, T = U*D*U^T
        // 因此：A = Q*U*D*U^T*Q^T = (Q*U)*D*(Q*U)^T
        // 所以A的特征向量是Q*U的列向量
        float[][] Q_matrix = Q;                    // 三对角化的变换矩阵
        float[][] U_matrix = eigenvectors.getData(); // 三对角矩阵的特征向量
        int matrixSize = Q_matrix.length;

        // 计算A的特征向量：Q * U^T
        // 注意：由于矩阵按行存储，U的行是特征向量，需要转置
        float[][] transformedCols = new float[matrixSize][matrixSize];
        for (int i = 0; i < matrixSize; i++) {
            for (int j = 0; j < matrixSize; j++) {
                transformedCols[i][j] = 0.0f;
        ...
        float[][] originalEigenvectorsData = new float[matrixSize][matrixSize];
        for (int i = 0; i < matrixSize; i++) {
            for (int j = 0; j < matrixSize; j++) {
        ...
        IMatrix<Float> originalEigenvectors = new RereFloatMatrix(originalEigenvectorsData);
     ...
     * 一般矩阵的特征分解 - 使用海森伯格化简
     */
    private Tuple2<IVector<Float>, IMatrix<Float>> generalEigenDecomposition() {
        int n = data.length;

        // 根据矩阵大小选择算法
        Tuple2<float[][], float[][]> hessResult;
        if (n > 100) {
            // 大矩阵使用分块算法
        ...
        float[][] H = hessResult._1;  // 海森伯格矩阵
        float[][] Q = hessResult._2;  // 变换矩阵

        // 对海森伯格矩阵应用QR算法
        Tuple2<IFloatVector, IFloatMatrix> eigenResult;
        if (n > 50) {
            // 大矩阵使用隐式QR算法
        ...
        IFloatVector eigenvalues = eigenResult._1;//此处正确的，别随意改动
        IFloatMatrix eigenvectors = eigenResult._2;//此处正确的，别随意改动
        ...
        float[][] Q_matrix = Q;
        float[][] U_matrix = eigenvectors.getData();
        int matrixSize = Q_matrix.length;
     ...
     * 三对角化 - 对称矩阵的特殊化简（修复版本）
     */
    public Tuple2<float[][], float[][]> tridiagonalReduction() {
        int n = data.length;
     ...
     */
    public Tuple2<IFloatVector, IFloatMatrix> qrAlgorithmForTridiagonal(float[][] T) {
        int n = T.length;
     ...
     */
    private void sortEigenvaluesAndVectors(float[] eigenvalues, float[][] eigenvectors) {
        ...
        for (int i = 0; i < n - 1; i++) {
            for (int j = 0; j < n - 1 - i; j++) {
                ...
                if (eigenvalues[j] < eigenvalues[j + 1]) {
                    // 交换特征值
                    float tempVal = eigenvalues[j];
                    eigenvalues[j] = eigenvalues[j + 1];
                    eigenvalues[j + 1] = tempVal;

                    // 交换对应的特征向量（行）
                    // 保持特征值和特征向量的对应关系
                    float[] tempVec = eigenvectors[j];
     ...
     * 小矩阵的QR算法 - 使用迭代QR分解
     */
    private Tuple2<IFloatVector, IFloatMatrix> qrAlgorithmForSmallMatrix(float[][] A) {
        int n = A.length;
        float[][] current = new float[n][n];
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            ...
            for (int i = 0; i < n - 1; i++) {
                for (int j = i + 1; j < n; j++) {
                    if (Math.abs(R[j][i]) > 1e-12f) {
                        ...
                        if (r < 1e-12f || Float.isNaN(r) || Float.isInfinite(r)) {
                            continue;
                        }
                        ...
                        // 检查结果
                        if (Float.isNaN(c) || Float.isNaN(s) || Float.isInfinite(c) || Float.isInfinite(s)) {
                            continue;
                        }
            ...
            float[][] tempEigenvectors = new float[n][n];
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    tempEigenvectors[i][j] = 0.0f;
                    for (int k = 0; k < n; k++) {
        ...
        for (int j = 0; j < n; j++) {
            ...
            if (norm > 1e-10f) {
                ...
                // 如果向量线性相关，设置为单位向量
                for (int i = 0; i < n; i++) {
                    eigenvectors[j][i] = (i == j) ? 1.0f : 0.0f;
                }
            }
        }
        ...
        float[][] eigenvectorsTransposed = new float[n][n];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
        ...
        return new Tuple2<>(IFloatVector.of(eigenvalues), new RereFloatMatrix(eigenvectorsTransposed));
    }

    /**
     * 计算Wilkinson位移 - 改进版本
     */
    private Float computeWilkinsonShift(float[] diagonal, float[] subdiagonal, int n) {
        if (n < 2) {
            return 0.0f;
        }

        float a = diagonal[n - 2];
        float b = subdiagonal[n - 2];
        float d = diagonal[n - 1];

        // 计算2x2矩阵的特征值
        float trace = a + d;
        float det = a * d - b * b; // 对称矩阵，b和c相同
        float discriminant = trace * trace - 4 * det;

        if (discriminant >= 0) {
            float sqrtDisc = (float) Math.sqrt(discriminant);
            float lambda1 = (trace + sqrtDisc) / 2.0f;
            float lambda2 = (trace - sqrtDisc) / 2.0f;

            // 选择更接近d的特征值，但避免数值不稳定
            float diff1 = Math.abs(lambda1 - d);
            float diff2 = Math.abs(lambda2 - d);
     ...
     * 从完整矩阵计算Wilkinson位移
     */
    private Float computeWilkinsonShiftFromMatrix(float[][] matrix, int n) {
        if (n < 2) {
            return 0.0f;
        }
     ...
     */
    private void performQRStep(float[][] matrix, float[][] eigenvectors, int n) {
        // 简化的QR步骤：直接处理三对角矩阵的Givens旋转
        // 只处理相邻的下对角元素

        for (int i = 0; i < n - 1; i++) {
            float a = matrix[i][i];
            float b = matrix[i + 1][i]; // 下对角元素
            ...
            if (Math.abs(b) > 1e-15f) {
                ...
                // 更新受影响的元素
                float temp_diag = matrix[i + 1][i + 1];
                float temp_super = matrix[i][i + 1];
                ...
                // 更新下一个下对角元素（如果存在）
                if (i + 1 < n - 1) {
                    float temp_next_sub = matrix[i + 2][i + 1];
                    matrix[i + 2][i + 1] = c * temp_next_sub;
                }

                // 应用右乘旋转恢复三对角形式
                float new_diag_i = matrix[i][i];
                float new_diag_i1 = matrix[i + 1][i + 1];
                ...
                // 更新特征向量（行式存储）
                for (int k = 0; k < n; k++) {
                    float v1 = eigenvectors[k][i];
                    float v2 = eigenvectors[k][i + 1];
     ...
     */
    private Tuple2<IFloatVector, IFloatMatrix> implicitQRAlgorithm(float[][] H) {
        int n = H.length;  // 矩阵维度
        float[][] A = new float[n][n];  // 工作矩阵，避免修改输入矩阵
        ...
        final float tolerance = 1e-10f;        // 收敛容差，提高数值精度
        final int minIterations = 5;           // 最小迭代次数要求，确保算法充分运行
        float previousOffDiagonalSum = Float.MAX_VALUE;  // 上次迭代的次对角线元素和
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            ...
            if (n >= 2) {
                // 提取右下角2×2子矩阵的元素
                float a = A[n - 2][n - 2];  // 主对角线元素
                float b = A[n - 2][n - 1];  // 次对角线元素
                float d = A[n - 1][n - 1];  // 右下角元素

                // 计算2×2子矩阵的特征值
                float trace = a + d;                    // 迹（trace）
                float det = a * d - b * b;             // 行列式（determinant）
                float discriminant = trace * trace - 4 * det;  // 判别式

                if (discriminant >= 0) {
                    // 实数特征值情况：使用二次公式
                    float sqrtDisc = (float) Math.sqrt(discriminant);
                    float lambda1 = (trace + sqrtDisc) / 2.0f;  // 较大特征值
                    float lambda2 = (trace - sqrtDisc) / 2.0f;  // 较小特征值
            ...
            for (int k = 0; k < n - 1; k++) {
                ...
                float a = A[k][k] - shift;    // 减去位移后的主对角线元素
                float b = A[k + 1][k];        // 次对角线元素
                float r = (float) Math.sqrt(a * a + b * b);  // 旋转半径

                if (r > 1e-10) {  // 避免除零错误，确保数值稳定性
                    float c = a / r;      // 余弦值（cosine）
                    float s = -b / r;     // 正弦值（sine），注意符号
        ...
        // 返回完整的特征分解结果
        return new Tuple2<>(IFloatVector.of(eigenvalues), new RereFloatMatrix(eigenvectors));
    }
     ...
     * @param j2 变换的结束列索引
     */
    private void applyGivensRotation(float[][] matrix, Float c, Float s, int i1, int i2, int j1, int j2) {
        // 对指定范围内的列进行Givens旋转变换
        for (int j = j1; j <= j2; j++) {
            // 保存原始值，避免在计算过程中被覆盖
            float temp1 = matrix[i1][j];  // 第i1行第j列的元素
            float temp2 = matrix[i2][j];  // 第i2行第j列的元素
     ...
     * 就地矩阵乘法 - 避免创建中间矩阵对象
     */
    private IMatrix<Float> multiplyMatricesInPlace(float[][] A, float[][] B) {
        int m = A.length;
        int n = B[0].length;
        int p = B.length;
     ...
     * 对海森伯格矩阵应用QR算法
     */
    private Tuple2<IFloatVector, IFloatMatrix> qrAlgorithmForHessenberg(float[][] H) {
        int n = H.length;
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            ...
            // 应用位移：A - shift * I
            float[][] shiftedA = new float[n][n];
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    shiftedA[i][j] = A[i][j] - (i == j ? shift : 0.0f);
                }
            }

            // 对位移后的矩阵进行QR分解
            Tuple2<IMatrix<Float>, IMatrix<Float>> qr = new RereFloatMatrix(shiftedA).qr();
            IMatrix<Float> Q = qr._1;
            IMatrix<Float> R = qr._2;

            // A = R * Q + shift * I
            IFloatMatrix newA = (IFloatMatrix) R.mmul(Q);
            ...
            // 更新特征向量
            IFloatMatrix currentEigenvectors = new RereFloatMatrix(eigenvectors);
            IFloatMatrix updatedEigenvectors = (IFloatMatrix) currentEigenvectors.mmul(Q);
     ...
     * 分块QR分解 - 适用于大矩阵
     */
    private Tuple2<IMatrix<Float>, IMatrix<Float>> blockedQRDecomposition(float[][] A) {
        int m = A.length;
        int n = A[0].length;
        ...
        return new Tuple2<>(new RereFloatMatrix(Q), new RereFloatMatrix(R));
    }

    /**
     * 应用Householder变换到QR分解
     */
    private void applyHouseholderToQR(float[][] matrix, float[] v, int startRow, int endRow, int startCol, int endCol) {
        int n = v.length;

        // 计算 w = matrix^T * v
        float[] w = new float[endCol - startCol + 1];
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < w.length; j++) {
                matrix[startRow + i][startCol + j] -= 2.0f * v[i] * w[j];
            }
        }
    }
     ...
     * 三对角矩阵的QR分解（优化版本）
     */
    private Tuple2<IMatrix<Float>, IMatrix<Float>> qrDecompositionTridiagonal(float[][] T) {
        int n = T.length;
        float[][] Q = new float[n][n];
        float[][] R = new float[n][n];
     ...
     */
    private void handleDuplicateEigenvalues(float[] eigenvalues, float[][] eigenvectors) {
     ...
     */
    private void orthogonalizeEigenvectors(float[][] eigenvectors, int startIdx, int count) {
     ...
     * 快速排序特征值和特征向量
     */
    private void quickSortEigen(float[] eigenvalues, float[][] eigenvectors, int low, int high) {
        if (low < high) {
     ...
     * 快速排序的分区函数
     */
    private int partitionEigen(float[] eigenvalues, float[][] eigenvectors, int low, int high) {
     ...
     * 交换特征值和对应的特征向量
     */
    private void swapEigen(float[] eigenvalues, float[][] eigenvectors, int i, int j) {
        // 交换特征值
        ...
        int n = eigenvectors.length;
        for (int k = 0; k < n; k++) {
            float tempVec = eigenvectors[i][k];
     ...
     */
    @Override
    public IMatrix<Float> mmul(IMatrix<Float> other1) {
        if (other1 == null) {
            throw new IllegalArgumentException("输入矩阵不能为null / Input matrix cannot be null");
        }
        IFloatMatrix other0 = (IFloatMatrix) other1;
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuMatrixMultiply(this, other0);
            } catch (Exception e) {
                // GPU失败时回退到CPU
     ...
     */
    private IMatrix<Float> cpuMatrixMultiply(IMatrix<Float> other1, int m, int n, int p) {
        IFloatMatrix other0 = (IFloatMatrix) other1;
        float[][] otherData = other0.getData();
        long complexity = (long) m * n * p;
     ...
     */
    private RereFloatMatrix naiveMultiply(float[][] otherData, int m, int n, int p) {
     ...
     */
    private RereFloatMatrix unrolledMultiply(float[][] otherData, int m, int n, int p) {
        float[][] result = new float[m][p];
     ...
     */
    private RereFloatMatrix blockMultiply(float[][] otherData, int m, int n, int p) {
        float[][] result = new float[m][p];
     ...
     */
    private RereFloatMatrix parallelBlockMultiply(float[][] otherData, int m, int n, int p) {
        float[][] result = new float[m][p];
     ...
     */
    private void strassenMultiply(float[][] otherData, RereFloatMatrix result, int n) {
        if (n <= 32) {
            // 对于小矩阵，使用基础算法
        ...
        float[][] M1 = basicMatrixMultiply(A11, strassenSub(B12, B22, half), half);
        float[][] M2 = basicMatrixMultiply(strassenAdd(A11, A12, half), B22, half);
        float[][] M3 = basicMatrixMultiply(strassenAdd(A21, A22, half), B11, half);
        float[][] M4 = basicMatrixMultiply(A22, strassenSub(B21, B11, half), half);
        float[][] M5 = basicMatrixMultiply(strassenAdd(A11, A22, half), strassenAdd(B11, B22, half), half);
        float[][] M6 = basicMatrixMultiply(strassenSub(A12, A22, half), strassenAdd(B21, B22, half), half);
        float[][] M7 = basicMatrixMultiply(strassenSub(A11, A21, half), strassenAdd(B11, B12, half), half);

        // 计算结果矩阵的四个子矩阵（正确的Strassen公式）
        float[][] C11 = strassenAdd(strassenSub(strassenAdd(M5, M4, half), M2, half), M6, half);
        float[][] C12 = strassenAdd(M1, M2, half);
        float[][] C21 = strassenAdd(M3, M4, half);
        float[][] C22 = strassenSub(strassenSub(strassenAdd(M5, M1, half), M3, half), M7, half);
     ...
     */
    private float[][] strassenAdd(float[][] A, float[][] B, int n) {
     ...
     */
    private float[][] strassenSub(float[][] A, float[][] B, int n) {
        float[][] C = new float[n][n];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                C[i][j] = A[i][j] - B[i][j];
     ...
     */
    private float[][] strassenMultiply(float[][] A, float[][] B, int n) {
        if (n <= 32) {
            // 对于小矩阵，使用基础算法
            float[][] C = new float[n][n];
            for (int i = 0; i < n; i++) {
        ...
        // 计算7个中间矩阵
        float[][] M1 = strassenAdd(strassenMultiply(A11, strassenAdd(B12, B22, half), half),
                strassenMultiply(A22, strassenAdd(B21, B11, half), half), half);
        float[][] M2 = strassenMultiply(strassenAdd(A11, A12, half), B22, half);
        float[][] M3 = strassenMultiply(strassenAdd(A21, A22, half), B11, half);
        float[][] M4 = strassenMultiply(A22, strassenSub(B21, B11, half), half);
        float[][] M5 = strassenMultiply(strassenAdd(A11, A22, half), strassenAdd(B11, B22, half), half);
        float[][] M6 = strassenMultiply(strassenSub(A12, A22, half), strassenAdd(B21, B22, half), half);
        float[][] M7 = strassenMultiply(strassenSub(A11, A21, half), strassenAdd(B11, B12, half), half);
     ...
     */
    private IMatrix<Float> basicOptimizedMultiply(IMatrix<Float> other1, int m, int n, int p) {
        IFloatMatrix other0 = (IFloatMatrix) other1;
        float[][] otherData = other0.getData();
        float[][] result = new float[m][p];
     ...
     */
    private IMatrix<Float> simdOptimizedMultiply(IMatrix<Float> other1, int m, int n, int p) {
        IFloatMatrix other0 = (IFloatMatrix) other1;
        float[][] otherData = other0.getData();
        float[][] result = new float[m][p];
     ...
     */
    private static float[][] basicMatrixMultiply(float[][] A, float[][] B, int n) {
        float[][] C = new float[n][n];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                float sum = 0;
     ...
     */
    private IMatrix<Float> cacheOptimizedBlockMultiply(IMatrix<Float> other1, int m, int n, int p) {
        IFloatMatrix other0 = (IFloatMatrix) other1;
        float[][] otherData = other0.getData();
        float[][] result = new float[m][p];
     ...
     */
    private void blockMicroKernel(float[][] otherData, float[][] result,
     ...
     */
    private IMatrix<Float> parallelBlockMultiplyOptimized(IMatrix<Float> other1, int m, int n, int p) {
        IFloatMatrix other0 = (IFloatMatrix) other1;
        float[][] otherData = other0.getData();
        float[][] result = new float[m][p];
     ...
     * matrices
     */
    private IMatrix<Float> optimizedParallelBlockMultiply(IMatrix<Float> other1, int m, int n, int p) {
        IFloatMatrix other0 = (IFloatMatrix) other1;
        float[][] otherData = other0.getData();
        float[][] result = new float[m][p];
     ...
     */
    private void optimizedMicroKernel(float[][] otherData, float[][] result,
            int iStart, int jStart, int kStart,
            int iEnd, int jEnd, int kEnd) {
        for (int i = iStart; i < iEnd; i++) {
            float[] thisRow = this.data[i];
     ...
     * 高性能微内核 / High performance micro-kernel
     */
    private void highPerformanceMicroKernel(float[][] otherData, float[][] result,
            int iStart, int jStart, int kStart,
            int iEnd, int jEnd, int kEnd) {
        // 使用更激进的优化策略
        for (int i = iStart; i < iEnd; i++) {
            ...
            int j = jStart;
            for (; j < jEnd - 1; j += 2) {
                float sum1 = resultRow[j];
                float sum2 = resultRow[j + 1];

                // 双列并行计算
                int k = kStart;
                for (; k < kEnd - 3; k += 4) {
                    float a0 = thisRow[k];
                    float a1 = thisRow[k + 1];
                    float a2 = thisRow[k + 2];
                    float a3 = thisRow[k + 3];
     ...
     */
    @Override
    public Float frobeniusDistance(IMatrix<Float> other) {
        if (other == null) {
            throw new IllegalArgumentException("输入矩阵不能为null / Input matrix cannot be null");
        }
     ...
     */
    @Override
    public IMatrix<Float> normalizeRows() {
        int rows = data.length;
        int cols = data[0].length;
        float[][] result = new float[rows][cols];

        for (int i = 0; i < rows; i++) {
            IVector<Float> row = this.getRow(i);
            float norm = row.norm2();

            if (norm > 0) {
                IFloatVector normalizedRow = (IFloatVector) row.divideByScalar(norm);
     ...
     */
    @Override
    public IMatrix<Float> normalizeColumns() {
        int rows = data.length;
        int cols = data[0].length;
        float[][] result = new float[rows][cols];
     ...
     */
    @Override
    public IMatrix<Float> center() {
        int rows = data.length;
        int cols = data[0].length;
        float[][] result = new float[rows][cols];

        // 计算每列的均值
        IVector<Float> columnMeans = this.rowMeans();
     ...
     */
    @Override
    public IMatrix<Float> covariance() {
        // 先进行数据中心化
        IMatrix<Float> centered = this.center();
        return centered.covarianceFromCentered();
    }

    @Override
    public IMatrix<Float> cov() {
     ...
     */
    @Override
    public IMatrix<Float> covarianceFromCentered() {
        int n = data.length; // 样本数

        // 计算 X^T * X
        IMatrix<Float> transposed = this.transposeNew();
    ...
    public IMatrix<Float> inv() {
        ...
        return new RereFloatMatrix(inverse);
    }
    ...
    public IMatrix<Float> pinv() {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuPseudoInverse(this);
            } catch (Exception e) {
                // GPU失败时回退到CPU，这里不需要额外日志，GPUComputeUtils已经处理了
            }
        }
        ...
        // 进行奇异值分解：A = U * S * V^T
        Tuple3<IMatrix<Float>, IVector<Float>, IMatrix<Float>> svdResult = this.svd();
        ...
        // 计算奇异值的伪逆
        IFloatVector pseudoSingularValues = IFloatVector.zeros(singularValuesLength);
        ...
        IMatrix<Float> V = VT.transposeNew();  // V = (V^T)^T

        // 创建结果矩阵：A⁺的维度应该是 originalCols x originalRows
        IMatrix<Float> pseudoInverse = IFloatMatrix.zeros(originalCols, originalRows);
     ...
     */
    @Override
    public IMatrix<Float> hstack(IMatrix<Float> other1) {
        if (other1 == null) {
            throw new IllegalArgumentException("输入矩阵不能为null / Input matrix cannot be null");
        }
     ...
     */
    @Override
    public IMatrix<Float> vstack(IMatrix<Float> other1) {
        if (other1 == null) {
            throw new IllegalArgumentException("输入矩阵不能为null / Input matrix cannot be null");
        }
     ...
     */
    @Override
    public IMatrix<Float>[] hsplit(int[] indices) {
        ...
        IMatrix<Float>[] result = new IFloatMatrix[numSplits];
        ...
        for (int i = 0; i < numSplits; i++) {
            ...
            result[i] = new RereFloatMatrix(subMatrix);
            startCol = endCol;
        }
     ...
     */
    @Override
    public IMatrix<Float>[] vsplit(int[] indices) {
        if (indices == null || indices.length == 0) {
            throw new IllegalArgumentException("分割索引不能为null或空 / Split indices cannot be null or empty");
        }
    ...
    public IMatrix<Float> copy() {
        ...
        return new RereFloatMatrix(copyData);
    }
    ...
    public Float det() {
        ...
        // 使用LU分解计算行列式
        try {
            Tuple2<IMatrix<Float>, IMatrix<Float>> lu = this.lu();
            IMatrix<Float> L = lu._1;
            IMatrix<Float> U = lu._2;
    ...
    public int rank() {
        try {
            // 使用SVD计算秩
            Tuple3<IMatrix<Float>, IVector<Float>, IMatrix<Float>> svdResult = this.svd();
            IVector<Float> singularValues = svdResult._2;
    ...
    public Tuple2<IMatrix<Float>, IMatrix<Float>> lu() {
        ...
        return new Tuple2<>(new RereFloatMatrix(L), new RereFloatMatrix(U));
    }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j <= i; j++) {
                ...
                if (j == i) {
                    ...
                    L[i][j] = (float) (((double) data[i][j] - sum) / (double) L[j][j]);
                }
            }
        }
     ...
     */
    @Override
    public IMatrix<Float> solve(IMatrix<Float> B) {
        if (data.length != data[0].length) {
            throw new IllegalArgumentException("只有方阵才能求解线性方程组 / Only square matrices can solve linear systems");
        }
        ...
        for (int col = 0; col < m; col++) {
            ...
            IVector<Float> x = this.solve(IFloatVector.of(b));
            for (int row = 0; row < n; row++) {
                X[row][col] = x.get(row);
            }
        }

        return new RereFloatMatrix(X);
    }
     ...
     */
    @Override
    public IMatrix<Float> slice(String rowSlice, String colSlice) {
        ...
        if (resultRows <= 0 || resultCols <= 0) {
            return new RereFloatMatrix(new float[0][0]);
        }
     ...
     */
    @Override
    public IMatrix<Float> sliceRows(String rowSlice) {
     ...
     */
    @Override
    public IMatrix<Float> sliceColumns(String colSlice) {
     ...
     */
    @Override
    public IMatrix<Float> fancyGet(int[] rowIndices, int[] colIndices) {
     ...
     */
    private Tuple3<float[][], float[][], float[][]> bidiagonalization() {
        int m = data.length;
        int n = data[0].length;
        int minDim = Math.min(m, n);
        ...
        for (int k = 0; k < minDim; k++) {
            ...
            if (k < n - 2) {
                ...
                if (norm > 1e-10) {
                    ...
                    // 应用变换到A
                    float[][] subA = new float[m - k][n - k - 1];
                    for (int i = 0; i < m - k; i++) {
                        for (int j = 0; j < n - k - 1; j++) {
                    ...
                    float[][] subAP = new float[m - k][n - k - 1];
                    for (int i = 0; i < m - k; i++) {
                        for (int j = 0; j < n - k - 1; j++) {
                            for (int l = 0; l < n - k - 1; l++) {
     ...
     * 对双对角矩阵应用分治算法
     */
    private Tuple2<float[], float[][]> divideAndConquerSVD(float[][] B) {
        int m = B.length;
        int n = B[0].length;
        int minDim = Math.min(m, n);

        // 提取双对角矩阵的对角线和次对角线
        float[] alpha = new float[minDim];
        float[] beta = new float[minDim - 1];
     ...
     * 分治算法处理双对角矩阵
     */
    private Tuple2<float[], float[][]> divideAndConquerBidiagonal(float[] alpha, float[] beta, int start, int end) {
        int n = end - start + 1;

        switch (n) {
            case 1:
                // 基本情况：1x1矩阵
                float[] singularValues = {Math.abs(alpha[start])};
                float[][] Q = {{1.0f}};
            ...
            default:
                ...
                // 分解为两个子问题
                Tuple2<float[], float[][]> leftResult = divideAndConquerBidiagonal(alpha, beta, start, mid - 1);
                Tuple2<float[], float[][]> rightResult = divideAndConquerBidiagonal(alpha, beta, mid, end);
     ...
     * 解决2x2双对角矩阵的SVD
     */
    private Tuple2<float[], float[][]> solve2x2Bidiagonal(Float a, Float b, Float c) {
        // 构造2x2矩阵 [a b; 0 c]
        float[][] M = {{a, b}, {0f, c}};
        ...
        float[] singularValues = {(float) Math.sqrt(Math.max(0, lambda1)), (float) Math.sqrt(Math.max(0, lambda2))};
     ...
     * 合并两个双对角SVD结果
     */
    private Tuple2<float[], float[][]> mergeBidiagonalResults(Tuple2<float[], float[][]> left, Tuple2<float[], float[][]> right) {
        float[] leftSV = left._1;
        float[] rightSV = right._1;
        float[][] leftQ = left._2;
        float[][] rightQ = right._2;
     ...
     * 对双对角矩阵应用QR算法
     */
    private Tuple2<float[], float[][]> qrAlgorithmForBidiagonal(float[][] B) {
        int m = B.length;
        int n = B[0].length;
        int minDim = Math.min(m, n);
        ...
        // 初始化Q为单位矩阵
        float[][] Q = new float[minDim][minDim];
        for (int i = 0; i < minDim; i++) {
            Q[i][i] = 1.0f;
        }
        ...
        final float tolerance = 1e-10f; // 提高收敛精度
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            ...
            if (previousOffDiagonalSum > 0) {
                float relativeChange = Math.abs(offDiagonalSum - previousOffDiagonalSum) / previousOffDiagonalSum;
            ...
            // 对双对角矩阵进行QR分解
            Tuple2<float[], float[]> qrResult = qrDecompositionBidiagonal(alpha, beta);
            float[] newAlpha = qrResult._1;
            float[] newBeta = qrResult._2;
        ...
        // 计算奇异值
        float[] singularValues = new float[minDim];
        for (int i = 0; i < minDim; i++) {
     ...
     */
    private Tuple2<float[], float[]> qrDecompositionBidiagonal(float[] alpha, float[] beta) {
        int n = alpha.length;
        float[] newAlpha = new float[n];
        float[] newBeta = new float[n - 1];
        ...
        // 找到最大的次对角线元素进行消除
        int maxIndex = 0;
        float maxValue = Math.abs(newBeta[0]);
        for (int i = 1; i < n - 1; i++) {
     ...
     * 快速排序奇异值
     */
    private void quickSortSingularValues(float[] values, int[] indices, int low, int high) {
        if (low < high) {
     ...
     * 快速排序的分区函数（奇异值）
     */
    private int partitionSingularValues(float[] values, int[] indices, int low, int high) {
     ...
     * 交换奇异值和对应的索引
     */
    private void swapSingularValues(float[] values, int[] indices, int i, int j) {
        // 交换奇异值
        float temp = values[i];
        values[i] = values[j];
        values[j] = temp;
     ...
     */
//    @Override
//    public void putColumn(int colIndex, IMatrix<Float> column) {
//        if (!(column instanceof IMatrix<Float>)) {
//            throw new IllegalArgumentException("参数必须是IMatrixGeneric<Float>类型 / Parameter must be IMatrix<Float> type");
//        }
//        putColumn(colIndex, (IMatrix<Float>) column);
//    }
    /**
     ...
     */
//    @Override
//    public IVector<Float> getColumn(int i) {
//        return getColunm(i);
//    }
    /**
     * 获取指定行向量 / Get specified row vector
...
//    }
     ...
     */
//    @Override
//    public IMatrix<Float>[] getColumns(int[] indices) {
//        IMatrix<Float>[] floatColumns = getColumns(indices);
//        @SuppressWarnings("unchecked")
//        IMatrix<Float>[] genericColumns = new IMatrix[floatColumns.length];
...
//    }
     ...
     */
    @Override
    public IMatrix<Float> pow(Float power) {
        int rows = data.length;
        int cols = data[0].length;
        float[][] result = new float[rows][cols];
        for (int i = 0; i < rows; i++) {
     ...
     */
    @Override
    public IMatrix<Float> getColumnMatrix(int colIndex) {
    ...
    public float[][] toFloatArray() {
        ...
        // 复制数据（RereFloatMatrix内部已经是float[][]，直接复制）
        for (int i = 0; i < rows; i++) {
            System.arraycopy(data[i], 0, result[i], 0, cols);
        }
