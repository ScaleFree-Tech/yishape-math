 ...
 * {@code
 ...
 IVector<Float> doubled = vector.multiplyScala(2.0f);
...
public class RereFloatVector implements IFloatVector {
     ...
     */
    @Override
    public IVector<Float> sub(IVector<Float> vec1) {
        // 参数验证：确保输入向量不为null
        if (vec1 == null) {
        ...
        IFloatVector vec0 = (IFloatVector)vec1;
        ...
        if (GPU_ENABLED && len > GPU_THRESHOLD) {
            try {
                // 尝试使用GPU进行向量减法计算
                return GPUComputeFloatUtils.gpuVectorSub(this, vec0);
            } catch (Exception e) {
        ...
        var vv = IFloatVector.of(v);  // 创建结果向量对象
        return vv;
    }
    ...
    public IVector<Float> add(IVector<Float> vec) {
        ...
        if (GPU_ENABLED && len > GPU_THRESHOLD) {
            try {
                // 尝试使用GPU进行向量加法计算
                return GPUComputeFloatUtils.gpuVectorAdd(this, (IFloatVector)vec);
            } catch (Exception e) {
                // GPU计算失败时的容错处理：自动回退到CPU计算
     ...
     */
    private IVector<Float> cpuVectorAdd(IVector<Float> vec1) {
        int len = this.data.length;        // 向量长度
        IFloatVector vec0 = (IFloatVector)vec1;
        float[] otherData = vec0.getData(); // 获取加数向量的数据
        ...
        return IFloatVector.of(result);  // 创建并返回结果向量
    }
    ...
    public Float innerProduct(IVector<Float> vec) {
        ...
        if (GPU_ENABLED && len > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuVectorDot(this, (IFloatVector)vec);
            } catch (Exception e) {
                // GPU失败时回退到CPU
     ...
     */
    private Float cpuInnerProduct(IVector<Float> vec1) {
        int len = this.data.length;
        IFloatVector vec0 = (IFloatVector)vec1;
        float[] otherData = vec0.getData();
    ...
    public IVector<Float> multiply(IVector<Float> vec1) {
        ...
        if (GPU_ENABLED && len > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuVectorMultiply(this, vec0);
            } catch (Exception e) {
                // GPU失败时回退到CPU
    ...
    public IVector<Float> subScalar(Float p) {
        ...
        if (GPU_ENABLED && len > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuVectorScalarSub(this, p);
            } catch (Exception e) {
                // GPU失败时回退到CPU
    ...
    public IVector<Float> addScalar(Float p) {
        ...
        if (GPU_ENABLED && len > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuVectorScalarAdd(this, p);
            } catch (Exception e) {
                // GPU失败时回退到CPU
    ...
    public IVector<Float> multiplyScalar(Float p) {
        ...
        if (GPU_ENABLED && len > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuVectorScalarMultiply(this, p);
            } catch (Exception e) {
                // GPU失败时回退到CPU
    ...
    public Float sum() {
        ...
        if (GPU_ENABLED && len > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuVectorSum(this);
            } catch (Exception e) {
                // GPU失败时回退到CPU
    ...
    @Override
    public Float var() {
        return this.subScalar(this.mean()).pow(2f).sum() / (float) this.length();
    }
     ...
     */
    @Override
    public IVector<Float> set(int position, Float value) {
        int actualPosition = position;
        if (position < 0) {
            actualPosition = data.length + position;
        }
     ...
     */
    @Override
    public IVector<Float> setFromTo(int start, int end, int step, Float[] values) {
        // 处理负数索引
     ...
     */
    @Override
    public IVector<Float> setFromTo(int start, int end, Float[] values) {
     ...
     */
    @Override
    public IVector<Float> fill(Float value) {
        for (int i = 0; i < this.length(); i++) {
            data[i] = value;
        }
        return this;
    ...
    public IVector<Float> sqrt() {
        ...
        if (GPU_ENABLED && len > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuVectorSqrt(this);
            } catch (Exception e) {
                // GPU失败时回退到CPU
    ...
    public IVector<Float> square() {
        ...
        if (GPU_ENABLED && len > GPU_THRESHOLD) {
            try {
                return GPUComputeFloatUtils.gpuVectorSquare(this);
            } catch (Exception e) {
                // GPU失败时回退到CPU
    ...
    public Float euclideanDistance(IVector<Float> other) {
        ...
        IVector<Float> diff = this.sub(other);
    ...
    @Override
    public IVector<Float> logicalAnd(IVector<Float> other) {
        if (other == null) {
            throw new IllegalArgumentException("输入向量不能为null / Input vector cannot be null");
        }
        if (this.data.length != other.length()) {
    ...
    @Override
    public IVector<Float> logicalOr(IVector<Float> other) {
        if (other == null) {
            throw new IllegalArgumentException("输入向量不能为null / Input vector cannot be null");
        }
        if (this.data.length != other.length()) {
    ...
    @Override
    public IVector<Float> logicalNot() {
        float[] result = new float[this.data.length];
        for (int i = 0; i < this.data.length; i++) {
    ...
    @Override
    public IVector<Float> logicalXor(IVector<Float> other) {
        if (other == null) {
            throw new IllegalArgumentException("输入向量不能为null / Input vector cannot be null");
        }
        if (this.data.length != other.length()) {
    ...
    public IVector<Float> diff(int n) {
        ...
        IVector<Float> current = this.copy();
        for (int i = 0; i < n; i++) {
    ...
    public Float percentile(double q) {
        ...
        Float index = ((float)q / 100.0f) * (sorted.length - 1);
    ...
    public IMatrix<Float> asColumnVector() {
        int len = this.data.length;
        float[][] columnMatrix = new float[len][1];
        ...
        return new RereFloatMatrix(columnMatrix);
    }
    ...
    public Float dtw(IVector<Float> other) {
        ...
        if (m == n) {
            ...
            for (int i = 0; i < m; i++) {
                if (Math.abs(this.data[i] - other.get(i)) > 1e-6f) {
                    identical = false;
                    break;
                }
            }
