...
public class RereMatrix implements IMatrix {
     ...
     */
    private static final Queue<RereMatrix> MATRIX_POOL = new ConcurrentLinkedQueue<>();
    ...
    private static int getOptimalBlockSize(int m, int n, int p) {
        int totalSize = m * n * p;
        if (totalSize < 10000) return 32;
        if (totalSize < 100000) return 64;
        if (totalSize < 1000000) return 128;
        return 256;
    }
     ...
     */
    public static RereMatrix borrowMatrix(int rows, int cols) {
        RereMatrix matrix = MATRIX_POOL.poll();
        if (matrix == null || matrix.data.length != rows || matrix.data[0].length != cols) {
            return new RereMatrix(rows, cols);
        }
        return matrix;
    }
     ...
     */
    private RereMatrix reset(int rows, int cols) {
     ...
     */
    public RereMatrix(int rows, int cols) {
    ...
    public IMatrix sub(float scalar) {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeUtils.gpuMatrixScalarSub(this, scalar);
            } catch (Exception e) {
                // GPU失败时回退到CPU
        ...
        // 使用CPU计算工具类
        return CPUComputeUtils.matrixScalarSub(data, scalar);
    }
    ...
    public IMatrix sub(IMatrix other) {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeUtils.gpuMatrixSub(this, other);
            } catch (Exception e) {
                // GPU失败时回退到CPU
        ...
        // 使用CPU计算工具类
        return CPUComputeUtils.matrixSub(data, otherData);
    }
    ...
    public IMatrix mmul(float scalar) {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeUtils.gpuMatrixScalarMultiply(this, scalar);
            } catch (Exception e) {
                // GPU失败时回退到CPU
        ...
        // 使用CPU计算工具类
        return CPUComputeUtils.matrixScalarMultiply(data, scalar);
    }
    ...
    public IMatrix add(IMatrix other) {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeUtils.gpuMatrixAdd(this, other);
            } catch (Exception e) {
        ...
        // 使用CPU计算工具类
        return CPUComputeUtils.matrixAdd(data, otherData);
    }
    ...
    /**
     * 矩阵转置（产生新对象） / Matrix transpose (create new object)，同transposeNew()
     * <p>
     ...
     * <p>
     * 将当前矩阵转置，修改原矩阵。方法名明确表示会修改原矩阵
     * Transposes the current matrix, modifying the original matrix. Method name clearly indicates modification.
     * </p>
     * 
     * @return 转置后的矩阵（原矩阵被修改）/ Transposed matrix (original matrix is modified)
     */
    @Override
    public IMatrix transposeInPlace() {
    ...
    public IMatrix transposeNew() {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeUtils.gpuMatrixTranspose(this);
            } catch (Exception e) {
                // GPU失败时回退到CPU
        ...
        // 使用CPU计算工具类
        return CPUComputeUtils.matrixTranspose(data);
    }
     ...
     * 
     * <p>特征分解是线性代数中的重要概念，将方阵A分解为A = QΛQ^(-1)的形式，
     * 其中Q是特征向量矩阵，Λ是对角矩阵（特征值在对角线上）。</p>
     * 
     * <p>数学原理：</p>
     * <ul>
    ...
    public Tuple2<IVector,IMatrix> eigen() {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                // 尝试使用GPU进行特征分解计算
                return GPUComputeUtils.gpuEigenDecomposition(this);
            } catch (Exception e) {
        ...
        return CPUComputeUtils.eigen(this);
    }
     ...
     * 
     * <p>奇异值分解是线性代数中的核心分解技术，将任意m×n矩阵A分解为A = UΣV^T的形式，
    ...
    public Tuple3<IMatrix, IVector, IMatrix> svd() {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                // 尝试使用GPU进行奇异值分解计算
                return GPUComputeUtils.gpuSVD(this);
            } catch (Exception e) {
                // GPU计算失败时的容错处理：自动回退到CPU计算
        ...
        return CPUComputeUtils.svd(this);
    }
    
    /**
     * 优化的奇异值分解算法 - 使用分治算法（Divide-and-Conquer SVD）
     * 
     * <p>这是高性能的SVD实现，采用双对角化预处理和分治算法相结合的策略，
     * 相比传统算法具有更好的数值稳定性和计算效率。</p>
     * 
     * <p>算法流程：</p>
     * <ol>
     ...
     * </ul>
     * 
     * @return 返回完整的SVD分解结果，包含：
     *         - U：完整的m×m左奇异向量矩阵
     *         - S：奇异值向量（按降序排列）
     *         - V^T：完整的n×n右奇异向量矩阵的转置
     */
    public Tuple3<IMatrix, IVector, IMatrix> optimizedSVD() {
        int m = data.length;    // 矩阵行数
        int n = data[0].length; // 矩阵列数
        ...
        return new Tuple3<>(new RereMatrix(finalU), IVector.of(singularValues), new RereMatrix(finalV).transposeNew());
    }
    
    /**
     * 双对角化SVD算法
     */
    public Tuple3<IMatrix, IVector, IMatrix> bidiagonalSVD() {
        int m = data.length;
        int n = data[0].length;
     ...
     * 
     * <p>正交化是线性代数中的重要操作，将一组线性无关的向量转换为正交向量组。
     * 在特征分解和奇异值分解中，正交化确保特征向量和奇异向量的正交性，
     * 这是数值稳定性和数学正确性的重要保证。</p>
     * 
     * <p>Gram-Schmidt正交化原理：</p>
     * <ul>
     *   <li>对于向量组{v₁, v₂, ..., vₙ}，构造正交向量组{u₁, u₂, ..., uₙ}</li>
     *   <li>u₁ = v₁ / ||v₁||（第一个向量标准化）</li>
     *   <li>uᵢ = (vᵢ - proj_u₁(vᵢ) - proj_u₂(vᵢ) - ... - proj_uᵢ₋₁(vᵢ)) / ||...||</li>
     ...
     * 传统SVD算法（修复版本）
     */
    public Tuple3<IMatrix, IVector, IMatrix> traditionalSVD() {
    ...
    /**
     * 简化的SVD算法，用于大矩阵避免卡死
     * 使用更高效的近似算法，基于矩阵的Frobenius范数分解
     * 
     * @return 简化的SVD分解结果
     */
    private Tuple3<IMatrix, IVector, IMatrix> simplifiedSVD() {
        int m = data.length;
        int n = data[0].length;
        int rank = Math.min(m, n);
        ...
        IMatrix U = new RereMatrix(UData);
        IMatrix V = new RereMatrix(VData);
        
        return new Tuple3<>(U, IVector.of(singularValues), V.transposeNew());
    }
    
    /**
     * 简化的伪逆算法，用于大矩阵避免卡死
     * 基于矩阵的Frobenius范数近似计算伪逆
     * 
     * @return 简化的伪逆矩阵
     */
    private IMatrix simplifiedPseudoInverse() {
        int m = data.length;
        int n = data[0].length;
        ...
        return new RereMatrix(pseudoInverseData);
    }
    
    /**
     * 分块海森伯格化简 - 将矩阵转换为海森伯格形式
     * 
     * <p>海森伯格化简是特征分解的关键预处理步骤，将任意矩阵A通过相似变换转换为海森伯格矩阵H，
    ...
    /**
     * 海森伯格化简 - 将矩阵转换为海森伯格形式
     * 海森伯格矩阵只有主对角线和次对角线非零，大大减少后续QR算法的计算量
     * 
     * @return 海森伯格矩阵和变换矩阵
     */
     ...
     * 
     * <p>QR算法是现代特征分解的核心算法，通过迭代QR分解来逼近特征值和特征向量。
     ...
     * 
     * <p>对称矩阵的特征分解是线性代数中的经典问题，具有特殊的数学性质和算法优势。
     ...
     * </ol>
     * 
     * @return 返回对称矩阵的特征分解结果，包含：
     *         - 特征值：按降序排列的实数特征值
     *         - 特征向量：与特征值对应的正交特征向量矩阵
     */
    private Tuple2<IVector, IMatrix> symmetricEigenDecomposition() {
        int n = data.length;  // 矩阵维度
        ...
        Tuple2<IVector, IMatrix> eigenResult = qrAlgorithmForTridiagonal(T);
        IVector eigenvalues = eigenResult._1;    // 三对角矩阵T的特征值
        IMatrix eigenvectors = eigenResult._2;   // 三对角矩阵T的特征向量
        ...
        IMatrix originalEigenvectors = new RereMatrix(originalEigenvectorsData);
     ...
     * 一般矩阵的特征分解 - 使用海森伯格化简
     */
    private Tuple2<IVector, IMatrix> generalEigenDecomposition() {
        int n = data.length;
        ...
        // 对海森伯格矩阵应用QR算法
        Tuple2<IVector, IMatrix> eigenResult;
        if (n > 50) {
            // 大矩阵使用隐式QR算法
        ...
        IVector eigenvalues = eigenResult._1;//此处正确的，别随意改动
        IMatrix eigenvectors = eigenResult._2;//此处正确的，别随意改动
    ...
    public Tuple2<float[][], float[][]> tridiagonalReduction() {
        ...
        for (int k = 0; k < n - 2; k++) {
            ...
            if (norm < 1e-12) continue; // 提高数值稳定性
    ...
    /**
     * 对三对角矩阵应用QR算法 - 优化版本
     * 使用类似于小矩阵的方法，但针对行式存储优化
     */
    public Tuple2<IVector, IMatrix> qrAlgorithmForTridiagonal(float[][] T) {
        int n = T.length;
     ...
     * 
     * <p>在特征分解完成后，需要将特征值和对应的特征向量按特征值大小进行排序。
     * 这确保了特征分解结果的一致性和可预测性，便于后续的数值计算和分析。</p>
     * 
     * <p>排序策略：</p>
     * <ul>
     ...
     * 小矩阵的QR算法 - 使用迭代QR分解
     */
    private Tuple2<IVector, IMatrix> qrAlgorithmForSmallMatrix(float[][] A) {
        int n = A.length;
        ...
        return new Tuple2<>(IVector.of(eigenvalues), new RereMatrix(eigenvectorsTransposed));
    }
     ...
     * 
     * <p>隐式QR算法是现代特征分解的核心算法，通过直接操作矩阵元素来避免显式的矩阵乘法，
     ...
     * </ol>
     * 
     * <p>Wilkinson位移：选择右下角2x2子矩阵的特征值中更接近H[n-1][n-1]的那个，
     * 这能显著加速收敛，特别是对于接近收敛的情况。</p>
     * 
     * <p>时间复杂度：O(n³)，但收敛速度比标准QR算法快2-3倍</p>
     * <p>空间复杂度：O(n²)</p>
     * 
     * @param H 海森伯格矩阵（输入）
     * @return 包含特征值和特征向量的元组
     */
    private Tuple2<IVector, IMatrix> implicitQRAlgorithm(float[][] H) {
        int n = H.length;  // 矩阵维度
     ...
     * 
     * <p>Givens旋转是数值线性代数中的基本变换，用于在矩阵的特定位置引入零元素。
     ...
     * 就地矩阵乘法 - 避免创建中间矩阵对象
     */
    private IMatrix multiplyMatricesInPlace(float[][] A, float[][] B) {
        int m = A.length;
        int n = B[0].length;
        int p = B.length;
     ...
     * 对海森伯格矩阵应用QR算法
     */
    private Tuple2<IVector, IMatrix> qrAlgorithmForHessenberg(float[][] H) {
        int n = H.length;
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            ...
            // 对位移后的矩阵进行QR分解
            Tuple2<IMatrix, IMatrix> qr = new RereMatrix(shiftedA).qr();
            IMatrix Q = qr._1;
            IMatrix R = qr._2;
     ...
     * 分块QR分解 - 适用于大矩阵
     */
    private Tuple2<IMatrix, IMatrix> blockedQRDecomposition(float[][] A) {
        int m = A.length;
        int n = A[0].length;
     ...
     * 三对角矩阵的QR分解（优化版本）
     */
    private Tuple2<IMatrix, IMatrix> qrDecompositionTridiagonal(float[][] T) {
        int n = T.length;
        float[][] Q = new float[n][n];
        float[][] R = new float[n][n];
     ...
     * 处理重复特征值 - 确保特征向量的正交性
     * 
     * <p>当矩阵具有重复特征值时，对应的特征向量空间可能不是一维的，
     * 这会导致特征向量不正交，影响后续计算的数值稳定性。</p>
     * 
     * <p>问题描述：</p>
     * <ul>
     ...
     * 
     * <p>算法原理：</p>
     * <p>对于重复特征值λ，设对应的特征向量为v₁, v₂, ..., vₖ，
     * 使用Gram-Schmidt过程构造正交向量组：</p>
     * <p>u₁ = v₁ / ||v₁||</p>
     * <p>uᵢ = (vᵢ - Σⱼ₌₁ⁱ⁻¹⟨vᵢ, uⱼ⟩uⱼ) / ||vᵢ - Σⱼ₌₁ⁱ⁻¹⟨vᵢ, uⱼ⟩uⱼ||</p>
     * 
    ...
    public IMatrix mmul(IMatrix other) {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeUtils.gpuMatrixMultiply(this, other);
            } catch (Exception e) {
                // GPU失败时回退到CPU
        ...
        // 使用原有的CPU优化算法
        return cpuMatrixMultiply(other, m, n, p);
    }
    
    /**
     * CPU矩阵乘法（原有算法） / CPU matrix multiplication (original algorithm)
     */
    private IMatrix cpuMatrixMultiply(IMatrix other, int m, int n, int p) {
        float[][] otherData = other.getData();
        long complexity = (long) m * n * p;
     ...
     */
    private RereMatrix naiveMultiply(float[][] otherData, int m, int n, int p) {
     ...
     */
    private RereMatrix unrolledMultiply(float[][] otherData, int m, int n, int p) {
        float[][] result = new float[m][p];
     ...
     */
    private RereMatrix blockMultiply(float[][] otherData, int m, int n, int p) {
        float[][] result = new float[m][p];
     ...
     */
    private RereMatrix parallelBlockMultiply(float[][] otherData, int m, int n, int p) {
        float[][] result = new float[m][p];
     ...
     */
    private void strassenMultiply(float[][] otherData, RereMatrix result, int n) {
        if (n <= 32) {
            // 对于小矩阵，使用基础算法
     ...
     */
    private IMatrix basicOptimizedMultiply(IMatrix other, int m, int n, int p) {
        float[][] otherData = other.getData();
        float[][] result = new float[m][p];
     ...
     */
    private IMatrix simdOptimizedMultiply(IMatrix other, int m, int n, int p) {
        float[][] otherData = other.getData();
        float[][] result = new float[m][p];
     ...
     */
    private IMatrix cacheOptimizedBlockMultiply(IMatrix other, int m, int n, int p) {
        float[][] otherData = other.getData();
        float[][] result = new float[m][p];
     ...
     */
    private IMatrix parallelBlockMultiplyOptimized(IMatrix other, int m, int n, int p) {
        float[][] otherData = other.getData();
        float[][] result = new float[m][p];
    ...
    /**
     * 超大矩阵的高度优化并行算法 / Highly optimized parallel algorithm for very large matrices
     */
    private IMatrix optimizedParallelBlockMultiply(IMatrix other, int m, int n, int p) {
        float[][] otherData = other.getData();
        float[][] result = new float[m][p];
    ...
    public IMatrix pinv() {
        ...
        if (GPU_ENABLED && complexity > GPU_THRESHOLD) {
            try {
                return GPUComputeUtils.gpuPseudoInverse(this);
            } catch (Exception e) {
                // GPU失败时回退到CPU，这里不需要额外日志，GPUComputeUtils已经处理了
            }
        }
    ...
    /**
     * 双对角化 - 将矩阵转换为双对角形式
     * 使用Householder变换将矩阵A转换为双对角矩阵B，使得A = U * B * V^T
     */
    ...
    public void set(int row, int col, float value) {
        if (row < 0 || row >= data.length || col < 0 || col >= data[0].length) {
            throw new IndexOutOfBoundsException("索引超出范围: row=" + row + ", col=" + col);
        }
        data[row][col] = value;
    }
    ...
    public IVector diag() {
        ...
        return LAs.vec(diagonal);
    }

    @Override
    public IMatrix subMatrix(int startRow, int endRow, int startCol, int endCol) {
        ...
        return new RereMatrix(subData);
    }

    @Override
    public void setSubMatrix(int startRow, int endRow, int startCol, int endCol, IMatrix subMatrix) {
        if (startRow < 0 || endRow > data.length || startCol < 0 || endCol > data[0].length) {
            throw new IndexOutOfBoundsException("索引超出范围");
        }
        if (startRow >= endRow || startCol >= endCol) {
        ...
        if (subMatrix.getRows() != rows || subMatrix.getColumns() != cols) {
