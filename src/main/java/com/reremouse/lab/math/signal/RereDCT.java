...
public class RereDCT {
    ...
    public static double[][] dct2D(double[][] matrix) {
        ...
        // 对每一行进行DCT / Apply DCT to each row
        ...
        // 对每一列进行DCT / Apply DCT to each column
        for (int j = 0; j < cols; j++) {
            double[] column = new double[rows];
            for (int i = 0; i < rows; i++) {
                column[i] = result[i][j];
            }
            IVector<Double> col = Linalg.vector(column);
    ...
    public static double[][] idct2D(double[][] dctMatrix) {
        ...
        // 对每一列进行IDCT / Apply IDCT to each column
        for (int j = 0; j < cols; j++) {
            double[] column = new double[rows];
            for (int i = 0; i < rows; i++) {
                column[i] = dctMatrix[i][j];
            }
            IVector<Double> col = Linalg.vector(column);
        ...
        // 对每一行进行IDCT / Apply IDCT to each row
        for (int i = 0; i < rows; i++) {
    ...
    public static double energyConcentration(IVector<Double> dctSignal, int k) {
        ...
        double totalEnergy = dctSignal.multiply(dctSignal).sum();
