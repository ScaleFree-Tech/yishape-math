...
public class WaveletAnalysis {
     ...
     */
    public static class WaveletCoefficients {
    ...
    /**
     * 连续小波变换 (CWT) / Continuous Wavelet Transform (CWT)
     * <p>
     * 计算信号的连续小波变换，提供时频分析能力。
     * Calculate continuous wavelet transform of signal, providing time-frequency analysis capability.
     * </p>
     *
    ...
    public static IVector<Double> waveletEnergyAnalysis(WaveletCoefficients coefficients) {
        ...
        energy.set(0, coefficients.approximation.multiply(coefficients.approximation).sum());
        
        // 细节系数能量 / Detail coefficient energy
        for (int i = 0; i < coefficients.levels; i++) {
            energy.set(i + 1, coefficients.details[i].multiply(coefficients.details[i]).sum());
        }
    ...
    public static IVector<Double> waveletFeatureExtraction(WaveletCoefficients coefficients) {
        ...
        features.add(coefficients.approximation.mean());
        features.add(coefficients.approximation.std());
        features.add(coefficients.approximation.max());
        features.add(coefficients.approximation.min());
        
        // 细节系数特征 / Detail coefficient features
        for (int i = 0; i < coefficients.levels; i++) {
            features.add(coefficients.details[i].mean());
            features.add(coefficients.details[i].std());
            features.add(coefficients.details[i].max());
            features.add(coefficients.details[i].min());
        }
    ...
    private static double[] getDaubechiesCoefficients(int order) {
        switch (order) {
            ...
            default:
                return new double[]{0.7071067811865476, 0.7071067811865476}; // 默认使用db2
        }
    }
