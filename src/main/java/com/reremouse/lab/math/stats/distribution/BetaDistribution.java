...
public class BetaDistribution implements IContinuousDistribution, Serializable {
    ...
    public double mode() {
        if (alpha <= 1.0f && beta <= 1.0f) {
            // 当α≤1且β≤1时，众数在端点处
            return alpha < beta ? 0.0f : (alpha > beta ? 1.0f : Double.NaN);
        }
    ...
    public double skewness() {
        double numerator = 2.0f * (beta - alpha) * (float) Math.sqrt(alpha + beta + 1.0f);
        double denominator = (alpha + beta + 2.0f) * (float) Math.sqrt(alpha * beta);
    ...
    public double kurtosis() {
        ...
        double numerator = 6.0f * ((alpha - beta) * (alpha - beta) * (aPlusB + 1.0f) - alpha * beta * (aPlusB + 2.0f));
        double denominator = alpha * beta * (aPlusB + 2.0f) * (aPlusB + 3.0f);
        return numerator / denominator;
    }
    ...
    public double sample() {
        ...
        double gamma1 = gammaSample(alpha, 1.0f);
        double gamma2 = gammaSample(beta, 1.0f);
