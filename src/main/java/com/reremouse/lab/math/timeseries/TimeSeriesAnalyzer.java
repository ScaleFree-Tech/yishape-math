 ...
 * <p>
 * 提供统一的时间序列分析功能，包括数据预处理、模型选择、预测、诊断等。
 * 整合各种时间序列模型，提供一致的分析接口。
 * </p>
 * <p>
 * Provides unified time series analysis functionality including data preprocessing,
 * model selection, forecasting, diagnostics, etc. Integrates various time series
 * models and provides consistent analysis interface.
 * </p>
 *
 * @author lteb2
 * @version 1.0
...
public class TimeSeriesAnalyzer {
     ...
     * <p>
     * 执行完整的时间序列分析，包括模型选择、拟合、预测和诊断。
     * Perform complete time series analysis including model selection, fitting, forecasting, and diagnostics.
     * </p>
     *
     ...
     * <p>
     * 使用默认配置进行快速分析。
     * Perform quick analysis using default configuration.
     * </p>
     *
     * @return 分析结果 / Analysis result
     */
     ...
     * <p>
     * 设置特定的时间序列模型。
     * Set specific time series model.
     * </p>
     *
    ...
    public Map<String, Object> getDataStatistics() {
        ...
        stats.put("mean", data.mean());
        stats.put("std", data.std());
        stats.put("min", data.min());
        stats.put("max", data.max());
     ...
     * <p>
     * 分析时间序列的趋势特征。
     * Analyze trend characteristics of time series.
     * </p>
     *
     ...
     * <p>
     * 分析时间序列的季节性特征。
     * Analyze seasonal characteristics of time series.
     * </p>
     *
    ...
    public IVector<Double> getData() { return data.copy(); }
    public String getName() { return name; }
    public LocalDateTime[] getTimestamps() { return timestamps.clone(); }
    public ITimeSeriesModel getCurrentModel() { return currentModel; }
    public ITimeSeriesForecastResult getLastForecast() { return lastForecast; }
    public ITimeSeriesDiagnostics getLastDiagnostics() { return lastDiagnostics; }
    ...
    private double calculateSeasonalStrength(IVector<Double> data, int period) {
        ...
        // 计算季节性强度 / Calculate seasonal strength
        double seasonalVar = seasonalPattern.var();
        double totalVar = data.var();
