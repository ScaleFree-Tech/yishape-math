 ...
 * <p>
 * 提供时间序列分解功能，包括趋势、季节性、周期性成分的分离。
 * 支持加法模型和乘法模型，使用项目现有的signal包和linalg包功能。
 * </p>
 * <p>
 * Provides time series decomposition functionality including trend, seasonal, and cyclical component separation.
 * Supports additive and multiplicative models, uses existing signal and linalg package functionality.
 * </p>
 *
 * @author lteb2
 * @version 1.0
...
public class TimeSeriesDecomposition {
    ...
    public static class DecompositionResult {
        ...
        public DecompositionResult(IVector<Double> trend, IVector<Double> seasonal, IVector<Double> residual,
                                 IVector<Double> original, DecompositionModel model, int period,
     ...
     * <p>
     * 使用X-13ARIMA-SEATS方法进行时间序列分解（简化版本）。
     * Use X-13ARIMA-SEATS method for time series decomposition (simplified version).
     * </p>
     *
     * @param timeSeries 输入时间序列 / Input time series
     * @param variableIndex 变量索引 / Variable index
