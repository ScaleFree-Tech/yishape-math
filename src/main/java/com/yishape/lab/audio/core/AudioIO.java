 ...
 * <p>
 * 提供音频文件的读取和写入功能，支持多种音频格式。 使用IVector接口存储音频数据，确保与现有代码库的兼容性。
 * </p>
 * <p>
 * Provides audio file reading and writing functionality, supporting multiple
 * audio formats. Uses IVector interface to store audio data, ensuring
...
public class AudioIO {
     ...
     * <p>
     * 根据文件扩展名自动识别音频格式并读取。 Automatically identifies audio format by file
     * extension and reads.
     * </p>
     *
     ...
     * @throws UnsupportedAudioFormatException 如果音频格式不支持 / If audio format is
     * not supported
     */
     ...
     * @return 音频数据对象 / Audio data object
     * @throws UnsupportedAudioFormatException 如果WAV格式不支持 / If WAV format is not
     * supported
     */
     ...
     * @return 音频数据对象 / Audio data object
     * @throws UnsupportedAudioFormatException 如果MP3格式不支持 / If MP3 format is not
     * supported
     */
    private static AudioData parseMP3(byte[] fileData) throws UnsupportedAudioFormatException {
        try {
            ...
            // 创建自定义AudioDevice来捕获数据
            CaptureAudioDevice device = new CaptureAudioDevice();

            // 创建Player并使用自定义device
            javazoom.jl.player.Player player = new javazoom.jl.player.Player(bais, device);
            ...
            // 获取解码后的数据
            short[] pcmSamples = device.getSamples();
            int sampleRate = device.getSampleRate();
            int channels = device.getChannels();
            int bitDepth = 16;

            // 转换为IVector
            IVector<Double> samples = Linalg.zeros(pcmSamples.length);
            for (int i = 0; i < pcmSamples.length; i++) {
                samples.set(i, pcmSamples[i] / 32768.0);
            }

            return new AudioData(samples, sampleRate, channels, bitDepth, AudioFormat.MP3);

        } catch (javazoom.jl.decoder.JavaLayerException e) {
            throw new UnsupportedAudioFormatException("Failed to decode MP3: " + e.getMessage(), e);
        }
    }
     ...
     * @return 音频数据对象 / Audio data object
     * @throws UnsupportedAudioFormatException 如果FLAC格式不支持 / If FLAC format is
     * not supported
     */
     ...
     * @return 音频数据对象 / Audio data object
     * @throws UnsupportedAudioFormatException 如果OGG格式不支持 / If OGG format is not
     * supported
     */
     ...
     * @return 音频数据对象 / Audio data object
     * @throws UnsupportedAudioFormatException 如果AAC格式不支持 / If AAC format is not
     * supported
     */
     ...
     * @return 音频数据对象 / Audio data object
     * @throws UnsupportedAudioFormatException 如果M4A格式不支持 / If M4A format is not
     * supported
     */
     ...
     * @return 音频数据对象 / Audio data object
     * @throws UnsupportedAudioFormatException 如果AIFF格式不支持 / If AIFF format is
     * not supported
     */
     ...
     * @return 音频数据对象 / Audio data object
     * @throws UnsupportedAudioFormatException 如果AU格式不支持 / If AU format is not
     * supported
     */
     ...
     * @throws UnsupportedAudioFormatException 如果格式块未找到 / If format chunk not
     * found
     */
     ...
     * @throws UnsupportedAudioFormatException 如果数据块未找到 / If data chunk not
     * found
     */
    ...
    private static class CaptureAudioDevice extends javazoom.jl.player.AudioDeviceBase {

        private final java.util.List<Short> samples = new java.util.ArrayList<>();
        ...
        protected void writeImpl(short[] samples, int offs, int len) {
            // Get format info on first write (openImpl is not called by JLayer)
            if (!formatInitialized) {
                javazoom.jl.decoder.Decoder decoder = getDecoder();
                if (decoder != null) {
