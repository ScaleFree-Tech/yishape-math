...
public class RereMathUtil {
    ...
    private static final YishapeLogger log = YishapeLogger.getLogger(RereMathUtil.class);
     ...
     */
    public static int[] generateRandomInts(long seed, int start, int end, int num) {
        if (start >= end) {
        ...
        RngProvider rng = RngFactory.createSeeded(seed);
        int[] result = new int[num];
        int range = end - start;

        for (int i = 0; i < num; i++) {
            result[i] = start + rng.nextInt(range);
        }

        return result;
    }

    /** @deprecated 使用 {@link #generateRandomInts(long, int, int, int)} */
    @Deprecated
    public static int[] generateRandomInts(int seed, int start, int end, int num) {
        return generateRandomInts((long) seed, start, end, num);
    }
    ...
    /**
     * ln Γ(x)，x &gt; 0。与 {@link #gamma(double)} 使用同一 Lanczos 系数，避免 Γ 上溢时仍可稳定计算。
     */
    public static double logGamma(double x) {
        if (x <= 0) {
            throw new IllegalArgumentException("logGamma requires x > 0");
        }
        if (x < 0.5) {
        ...
        for (int i = 0; i < coefficients.length; i++) {
            ser += coefficients[i] / (z + i + 1);
        }
        double t = z + coefficients.length - 0.5;
        return 0.5 * Math.log(2 * Math.PI) + (z + 0.5) * Math.log(t) - t + Math.log(ser);
    }
    ...
    /**
     * 下不完全伽马 γ(a,x)=∫₀ˣ t^{a-1}e^{-t}dt，a&gt;0, x≥0。
     * 使用级数（x 较小）与连分数 Q（x 较大）稳定计算，避免梯形积分在整数形状参数下的 ~1e-3 级误差。
     */
    public static double incompleteGamma(double a, double x) {
        ...
        if (a < 0) {
            throw new IllegalArgumentException("不完全伽马的形状参数 a 必须大于 0");
        }
        double p = incompleteGammaP(a, x);
        return p * Math.exp(logGamma(a));
    }

    /**
     * 正则化下不完全伽马 P(a,x)=γ(a,x)/Γ(a)。
     */
    public static double regularizedIncompleteGamma(double a, double x) {
        if (x < 0.0) {
            throw new IllegalArgumentException("x必须大于等于0 / x must be greater than or equal to 0");
        }
        if (x == 0.0) {
        ...
        if (a <= 0.0) {
            throw new IllegalArgumentException("a 必须为正 / a must be positive");
        }
        return incompleteGammaP(a, x);
    }

    /**
     * 正则化上不完全伽马 Q(a,x)=1−P(a,x)=Γ(a,x)/Γ(a)。Poisson 的 CDF 满足 F(k)=Q(k+1,λ)。
     */
    public static double regularizedIncompleteGammaQ(double a, double x) {
        ...
        return incompleteGammaQContinuedFraction(a, x);
    }
    ...
    private static double incompleteGammaP(double a, double x) {
        ...
        if (x < a + 1.0) {
            return incompleteGammaSeriesP(a, x);
        }
        return 1.0 - incompleteGammaQContinuedFraction(a, x);
    }

    /** 级数展开求 P(a,x)，适用于 x &lt; a+1（Press et al., Numerical Recipes） */
    private static double incompleteGammaSeriesP(double a, double x) {
    ...
    /** 连分数求 Q(a,x)，适用于 x ≥ a+1 */
    private static double incompleteGammaQContinuedFraction(double a, double x) {
        double gln = logGamma(a);
        return Math.exp(-x + a * Math.log(x) - gln) * gammaCF(a, x);
    }
    
    /**
     * 伽马函数的连分数展开（用于上不完全伽马 Q）
     */
     ...
     * @return 随机浮点数数组 / Random float array
     */
    public static float[] generateRandomFloats(long seed, int n) {
        if (n < 0) {
            throw new IllegalArgumentException("数组长度必须大于等于0 / Array length must be greater than or equal to 0");
        }
    ...
    /** @deprecated 使用 {@link #generateRandomFloats(long, int)} */
    @Deprecated
    public static float[] generateRandomFloats(int seed, int n) {
        return generateRandomFloats((long) seed, n);
    }
    ...
    /**
     * 正则化不完全Gamma函数 P(a,x)=γ(a,x)/Γ(a)（整数形状参数版本）。
     */
    public static double regularizedIncompleteGamma(int a, double x) {
        return regularizedIncompleteGamma((double) a, x);
    }
    ...
    /**
     * 安全地将Number对象转换为double值
     * Safely convert a Number object to a double value
     * 
     * @param number 要转换的Number对象 / Number object to convert
     * @return 转换后的double值 / Converted double value
     * @throws IllegalArgumentException 如果输入为null / If input is null
     */
    public static double safeDoubleValue(Number number) {
        if (number == null) {
    ...
    /**
     * 安全地将Number对象转换为double值，带有默认值
     * Safely convert a Number object to a double value with default value
     * 
     * @param number 要转换的Number对象 / Number object to convert
     * @param defaultValue 默认值 / Default value
     * @return 转换后的double值或默认值 / Converted double value or default value
     */
