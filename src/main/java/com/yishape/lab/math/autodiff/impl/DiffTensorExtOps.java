...
public static IDiffTensor diagEmbed(RereDiffTensor tensor, int offset, int dim1, int dim2) {
    ...
    if (!tensor.requiresGrad) return tensor.toNonDiff(new RereDoubleTensor(y, new int[]{outM, outN}));
    Consumer<RereDiffTensor> bw = self -> {
    ...
    return new RereDiffTensor(y, new int[]{outM, outN}, List.of(tensor), bw, "diagEmbed");
}
