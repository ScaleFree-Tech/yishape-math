 ...
 * @version 2.0 - 稳定性改进版本
 */
public class GPUDoubleComputer implements IDoubleVectorComputer {
...
//            Device[] devices = Device.getAvailableDevices();
            ...
            Kernel testKernel = new Kernel() {
                ...
                public void run() {
                    ...
                    if (i < testData.length) {
                        result[i] = testData[i] * 2.0; // 简单的双精度运算
                    }
                }
            };
     ...
     * CPU版本的二元运算
     */
    private double[] cpuBinaryOperate(double[] x1, double[] x2, BinaryOperation operation) {
     ...
     * 分块处理大数据的二元运算
     */
    private double[] chunkedBinaryOperate(double[] x1, double[] x2, BinaryOperation operation) {
        int length = x1.length;
        ...
        for (int start = 0; start < length; start += MAX_CHUNK_SIZE) {
            ...
            // 创建分块数据
            double[] chunk1 = new double[chunkSize];
            double[] chunk2 = new double[chunkSize];
            System.arraycopy(x1, start, chunk1, 0, chunkSize);
            System.arraycopy(x2, start, chunk2, 0, chunkSize);
            
            // 处理分块
            double[] chunkResult = gpuBinaryOperate(chunk1, chunk2, operation);
     ...
     * GPU版本的二元运算
     */
    private double[] gpuBinaryOperate(double[] x1, double[] x2, BinaryOperation operation) {
        int length = x1.length;
        double[] result = new double[length];
    ...
    private double[] chunkedBinaryOperate(double[] x1, double x2, BinaryOperation operation) {
        ...
        for (int start = 0; start < length; start += MAX_CHUNK_SIZE) {
            ...
            // 处理分块
            double[] chunkResult = gpuBinaryOperate(chunk, x2, operation);
     ...
     * CPU版本的矩阵二元运算
     */
    private double[][] cpuMatrixBinaryOperate(double[][] x1, double[][] x2, BinaryOperation operation) {
        int rows = x1.length;
        int cols = x1[0].length;
     ...
     * 分块处理大矩阵的二元运算
     */
    private double[][] tiledMatrixBinaryOperate(double[][] x1, double[][] x2, BinaryOperation operation) {
        int rows = x1.length;
        int cols = x1[0].length;
        double[][] result = new double[rows][cols];
     ...
     * GPU版本的矩阵二元运算
     */
    private double[][] gpuMatrixBinaryOperate(double[][] x1, double[][] x2, BinaryOperation operation) {
        int rows = x1.length;
        int cols = x1[0].length;
        
        // 展平矩阵
        double[] flat1 = flattenMatrix(x1);
        double[] flat2 = flattenMatrix(x2);
        
        // 使用向量运算
        double[] flatResult = gpuBinaryOperate(flat1, flat2, operation);
    ...
    private double[][] cpuMatrixScalarOperate(double[][] x1, double x2, BinaryOperation operation) {
        int rows = x1.length;
        int cols = x1[0].length;
        double[][] result = new double[rows][cols];
    ...
    private double[] cpuUniversalOperate(double[] x, UniversalOperation operation, double additionalParam) {
    ...
    private double[][] cpuMatrixUniversalOperate(double[][] x, UniversalOperation operation, double additionalParam) {
        int rows = x.length;
        int cols = x[0].length;
        double[][] result = new double[rows][cols];
            ...
            for (int j = 0; j < cols; j++) {
                switch (operation) {
                    ...
                    case ABS:
                        ...
                        break;
                    case POW:
                        result[i][j] = Math.pow(x[i][j], additionalParam);
                        break;
                    default:
                        throw new IllegalArgumentException("不支持的操作: " + operation);
                }
    ...
    public double reduceOperate(double[] x, ReduceOperation operation) {
        switch (operation) {
            ...
            case MAX:
                ...
                for (double val : x) if (val > max) max = val;
                return max;
            case MIN:
                double min = Double.POSITIVE_INFINITY;
                for (double val : x) if (val < min) min = val;
                return min;
            default:
                throw new IllegalArgumentException("不支持的操作: " + operation);
        }
    ...
    public double reduceOperate(double[][] x, ReduceOperation operation) {
        switch (operation) {
            ...
            case MAX:
                double max = Double.NEGATIVE_INFINITY;
                for (double[] row : x) {
                    for (double val : row) if (val > max) max = val;
                }
                return max;
            case MIN:
                double min = Double.POSITIVE_INFINITY;
                for (double[] row : x) {
                    for (double val : row) if (val < min) min = val;
                }
                return min;
            default:
                throw new IllegalArgumentException("不支持的操作: " + operation);
    ...
    public double[][] transpose(double[] rowVector) {
        double[][] result = new double[rowVector.length][1];
    ...
    public double[] diff(double[] array, int stride) {
        ...
        double[] result = new double[array.length - stride];
