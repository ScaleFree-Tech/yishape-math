 ...
 * @author lteb2
 */
public class SIMDDoubleComputer implements IDoubleVectorComputer {
    ...
    private static VectorSpecies<Double> selectOptimalSpecies() {
        ...
        if (preferred.length() >= 8) {
            return preferred;
        } else if (DoubleVector.SPECIES_512.length() > 0) {
            return DoubleVector.SPECIES_512;
        } else if (DoubleVector.SPECIES_256.length() > 0) {
    ...
    public static String getPerformanceStats() {
        ...
        double vectorizationRate = totalOperations > 0 ? 
            (double) totalVectorizedOperations / totalOperations * 100 : 0;
...
//            System.out.println("Optimal Block Size: " + OPTIMAL_BLOCK_SIZE);
    ...
    private DoubleVector performBinaryOperation(DoubleVector a, DoubleVector b, BinaryOperation operation) {
        return switch (operation) {
            ...
            case REMAINDER -> {
                ...
                var intQuotient = quotient.convert(VectorOperators.D2I, 0);
                var truncated = intQuotient.convert(VectorOperators.I2D, 0);
    ...
    private DoubleVector performBinaryOperationWithScalar(DoubleVector a, DoubleVector scalarVector, 
        ...
        return switch (operation) {
            ...
            case REMAINDER -> {
                // 优化的取余计算
                DoubleVector quotient = a.div(scalarValue);
                var intQuotient = quotient.convert(VectorOperators.D2I, 0);
                DoubleVector truncated = (DoubleVector) intQuotient.convert(VectorOperators.I2D, 0);
    ...
    public double reduceOperate(double[][] x, ReduceOperation operation) {
        ...
        // 获取最优向量规格
        VectorSpecies<Double> species = DoubleVector.SPECIES_PREFERRED.length() >= 4 ? 
            DoubleVector.SPECIES_PREFERRED.loopBound(4) >= 4 ? DoubleVector.SPECIES_PREFERRED : 
    ...
    public double[][] transpose(double[][] matrix) {
        ...
        final VectorSpecies<Double> species = DoubleVector.SPECIES_PREFERRED.length() >= 4 ? 
            DoubleVector.SPECIES_PREFERRED.loopBound(4) >= 4 ? DoubleVector.SPECIES_PREFERRED : 
            DoubleVector.SPECIES_256 : DoubleVector.SPECIES_128;
        ...
        for (int i = 0; i < rows; i++) {
            ...
            for (; j < unrolledEnd; j += vectorLength * UNROLL_FACTOR) {
                ...
                DoubleVector vec0 = DoubleVector.fromArray(species, matrix[i], j);
                DoubleVector vec1 = DoubleVector.fromArray(species, matrix[i], j + vectorLength);
                DoubleVector vec2 = DoubleVector.fromArray(species, matrix[i], j + 2 * vectorLength);
                DoubleVector vec3 = DoubleVector.fromArray(species, matrix[i], j + 3 * vectorLength);
            ...
            for (; j < vectorizedEnd; j += vectorLength) {
                DoubleVector vec = DoubleVector.fromArray(species, matrix[i], j);
                // 将向量元素存储到转置矩阵的对应位置
                for (int k = 0; k < vectorLength; k++) {
    ...
    public double[][] transpose(double[] rowVector) {
        ...
        for (; i < unrolledEnd; i += vectorLength * UNROLL_FACTOR) {
            // Unroll 4 iterations to improve instruction-level parallelism
            DoubleVector vec0 = DoubleVector.fromArray(species, rowVector, i);
            DoubleVector vec1 = DoubleVector.fromArray(species, rowVector, i + vectorLength);
            DoubleVector vec2 = DoubleVector.fromArray(species, rowVector, i + 2 * vectorLength);
            DoubleVector vec3 = DoubleVector.fromArray(species, rowVector, i + 3 * vectorLength);
        ...
        for (; i < vectorizedEnd; i += vectorLength) {
            DoubleVector vec = DoubleVector.fromArray(species, rowVector, i);
     ...
     * 优化的矩阵块处理
     */
    private void processMatrixBlockOptimized(double[][] a, double[][] bT, double[][] c,
            int iStart, int iEnd, int jStart, int jEnd, int kStart, int kEnd) {
        
        for (int i = iStart; i < iEnd; i++) {
            for (int j = jStart; j < jEnd; j++) {
                // 使用优化的点积计算部分和
                double partialSum = computePartialDotProduct(a[i], bT[j], kStart, kEnd);
                c[i][j] += partialSum;
            }
        }
    }
     ...
     * 计算部分点积的优化实现
     */
    private double computePartialDotProduct(double[] x, double[] y, int start, int end) {
        ...
        double result = acc1.add(acc2).reduceLanes(VectorOperators.ADD);
     ...
     * 简单的部分点积计算
     */
    private double computePartialDotProductSimple(double[] x, double[] y, int start, int end) {
        double result = 0.0;
    ...
    private double[][] outerProductByColumn(double[] a, double[] b, double[][] result) {
        int m = a.length;
        int n = b.length;
        final VectorSpecies<Double> species = DoubleVector.SPECIES_PREFERRED.length() >= 4 ? 
            DoubleVector.SPECIES_PREFERRED.loopBound(4) >= 4 ? DoubleVector.SPECIES_PREFERRED : 
        ...
        for (int j = 0; j < n; j++) {
            DoubleVector bBroadcast = DoubleVector.broadcast(species, b[j]);
            int i = 0;
            ...
            for (; i < unrolledEnd; i += vectorLength * UNROLL_FACTOR) {
                // Unroll 4 iterations to improve instruction-level parallelism
                DoubleVector aVec0 = DoubleVector.fromArray(species, a, i);
                DoubleVector aVec1 = DoubleVector.fromArray(species, a, i + vectorLength);
                DoubleVector aVec2 = DoubleVector.fromArray(species, a, i + 2 * vectorLength);
                DoubleVector aVec3 = DoubleVector.fromArray(species, a, i + 3 * vectorLength);
                
                DoubleVector product0 = aVec0.mul(bBroadcast);
                DoubleVector product1 = aVec1.mul(bBroadcast);
                DoubleVector product2 = aVec2.mul(bBroadcast);
                DoubleVector product3 = aVec3.mul(bBroadcast);
            ...
            for (; i < vectorizedEnd; i += vectorLength) {
                ...
                DoubleVector product = aVec.mul(bBroadcast);
    ...
    public double[] sign(double[] array) {
        ...
        for (; i < unrolledEnd; i += vectorLength * UNROLL_FACTOR) {
            ...
            DoubleVector signVec0 = zero.blend(one, gtZero0).blend(minusOne, ltZero0);
            DoubleVector signVec1 = zero.blend(one, gtZero1).blend(minusOne, ltZero1);
            DoubleVector signVec2 = zero.blend(one, gtZero2).blend(minusOne, ltZero2);
            DoubleVector signVec3 = zero.blend(one, gtZero3).blend(minusOne, ltZero3);
        ...
        for (; i < vectorizedEnd; i += vectorLength) {
            ...
            DoubleVector signVec = zero.blend(one, gtZero).blend(minusOne, ltZero);
