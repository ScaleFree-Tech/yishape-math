...
import com.yishape.lab.math.linalg.SliceExpressionParser;
...
/**
 * 数据框类，用于处理结构化数据，支持从CSV文件读取数据并与IMatrix进行转换 DataFrame class for handling
 * structured data, supports reading from CSV files and conversion with IMatrix
 *
 * <p>
 * 本类提供了类似pandas DataFrame的功能，包括：</p>
 * <p>
 * This class provides functionality similar to pandas DataFrame, including:</p>
 * <ul>
...
public class DataFrame implements Serializable {
     ...
     * <p>
     * 创建一个空的DataFrame / Creates an empty DataFrame</p>
     */
    public DataFrame() {
     ...
     * <p>
     * 使用指定的列列表创建DataFrame / Creates DataFrame with specified column list</p>
     *
    ...
    public static DataFrame readCsv(String filePath) throws IOException {
        String separator = ",";
        boolean ifHasHead = true;
        return readCsv(filePath, separator, ifHasHead);
    }
    ...
    public static DataFrame readCsv(String filePath, String separator, boolean ifHasHead) throws IOException {
        ...
        try (FileReader reader = new FileReader(filePath); CSVParser parser = new CSVParser(reader, CSVFormat.DEFAULT.builder().setDelimiter(separator.charAt(0)).build())) {
            ...
            for (Column column : df.columns) {
                ...
                if (isNumeric && !doubleData.isEmpty()) {
                    column.setColumnType(ColumnType.Numeric);
                    column.setData(doubleData);
                }
            }
     ...
     * <p>
     * 根据起始位置、结束位置和步长对列进行切片</p>
     * <p>
     * Slices columns based on start position, end position and step</p>
     *
     ...
     * <p>
     * 根据起始位置和结束位置对列进行切片，步长默认为1</p>
     * <p>
     * Slices columns based on start and end positions with default step of
     * 1</p>
     *
     * @param start 起始位置（包含） / Start position (inclusive)
    ...
    /**
     * 列切片操作（从指定位置到末尾） / Column slicing operation (from specified position to
     * end)
     * <p>
     * 从指定位置开始到最后一列进行切片</p>
     * <p>
     * Slices from specified position to the last column</p>
     *
     * @param start 起始位置（包含） / Start position (inclusive)
     * @return 切片后的DataFrame / Sliced DataFrame
     */
     ...
     * <p>
     * 支持类似NumPy的切片语法，如"1:5:2"表示从1到5，步长为2</p>
     * <p>
     * Supports NumPy-like slicing syntax, e.g., "1:5:2" means from 1 to 5 with
     * step 2</p>
     *
     * @param rowExp 行切片表达式，如"1:5"或"1:5:2" / Row slicing expression, e.g., "1:5"
     * or "1:5:2"
     * @param colExp 列切片表达式，如"1:5"或"1:5:2" / Column slicing expression, e.g.,
     * "1:5" or "1:5:2"
     * @return 切片后的DataFrame / Sliced DataFrame
     * @throws IllegalArgumentException 如果表达式格式无效 / if expression format is
     * invalid
     */
     ...
     * <p>
     * 提取所有Float类型的列并合并为一个矩阵</p>
     * <p>
     * Extracts all Float type columns and combines them into a matrix</p>
     *
     * @return 包含所有Float列数据的矩阵 / Matrix containing all Float column data
     * @throws IllegalStateException 如果没有Float类型的列 / if no Float type columns
     * exist
     */
     ...
     * <p>
     * 支持负数索引，-1表示最后一列，-2表示倒数第二列，以此类推</p>
     * <p>
     * Supports negative indices, -1 means last column, -2 means second to last
     * column, and so on</p>
     *
     ...
     * 向DataFrame添加新列</p>
     * <p>
     * Adds a new column to the DataFrame</p>
     *
     * @param column 要添加的列 / Column to add
     * @throws IllegalArgumentException 如果列数据长度与现有行数不匹配 / if column data length
     * doesn't match existing row count
     */
     ...
     * 返回DataFrame的列数</p>
     * <p>
     * Returns the number of columns in the DataFrame</p>
     *
     ...
     * 返回DataFrame的行数</p>
     * <p>
     * Returns the number of rows in the DataFrame</p>
     *
     ...
     * 返回[行数, 列数]的数组</p>
     * <p>
     * Returns array of [row count, column count]</p>
     *
     ...
     * 判断DataFrame是否没有数据</p>
     * <p>
     * Determines if DataFrame has no data</p>
     *
     ...
     * 返回所有列的名称列表</p>
     * <p>
     * Returns list of all column names</p>
     *
     ...
     * 根据列名查找并返回对应的列</p>
     * <p>
     * Finds and returns column by name</p>
     *
     ...
     * 返回所有列的类型列表</p>
     * <p>
     * Returns list of all column types</p>
     *
     ...
     * 清空所有列和行数据</p>
     * <p>
     * Clears all columns and row data</p>
     */
    public void clear() {
        columns.clear();
        rowCount = 0;
     ...
     * 创建当前DataFrame的深拷贝</p>
     * <p>
     * Creates a deep copy of current DataFrame</p>
     *
     ...
     * 返回列列表的副本</p>
     * <p>
     * Returns a copy of the columns list</p>
     *
     ...
     * 设置列列表并更新行数</p>
     * <p>
     * Sets columns list and updates row count</p>
     *
     * @param columns 新的列列表 / New columns list
     * @throws IllegalArgumentException 如果列数据长度不一致 / if column data lengths are
     * inconsistent
     */
