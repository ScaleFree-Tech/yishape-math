...
public interface IMatrix<T extends Number> {
     ...
     * </p>
     */
    public default void set(int row, int col, T value) {
    ...
    public static IMatrix<Double> unitLowerTriMatrix(int m, double[][] values) {
        // 创建一个m×m的单位矩阵
        IMatrix<Double> matrix = IMatrix.eye(m);
    ...
    public static IMatrix<Double> lowerTriMatrix(int m, double[][] values) {
        // 创建一个m×m的零矩阵
        IMatrix<Double> matrix = IMatrix.zeros(m, m);
    ...
    public default IMatrix<Float> toFloatMatrix() {
        ...
        if (this instanceof IMatrix<?> && ((IMatrix<?>) this).get(0, 0) instanceof Float) {
            @SuppressWarnings("unchecked")
            IMatrix<Float> floatMatrix = (IMatrix<Float>) this;
            return floatMatrix;
        }
    ...
    public default IMatrix<Double> toDoubleMatrix() {
        ...
        if (this instanceof IMatrix<?> && ((IMatrix<?>) this).get(0, 0) instanceof Double) {
            @SuppressWarnings("unchecked")
    ...
    public default IMatrix<T> permuteRows(int[] rowIndices) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                result[i][j] = this.get(rowIndices[i], j).doubleValue();
            }
        }
        
        // Convert to appropriate type and return
        if (this instanceof IMatrix<?>) {
            IMatrix<?> matrix = this;
            if (matrix.get(0, 0) instanceof Double) {
                return (IMatrix<T>) IMatrix.of(result);
            } else if (matrix.get(0, 0) instanceof Float) {
    ...
    public default IMatrix<T> permuteColumns(int[] colIndices) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                result[i][j] = this.get(i, colIndices[j]).doubleValue();
            }
        }
     ...
     * <p>
     * 求解下三角线性系统 LX = B，其中L是当前下三角矩阵，B是右端矩阵。
     * Solves the lower triangular linear system LX = B, where L is the current 
     * lower triangular matrix and B is the right-hand side matrix.
     * </p>
     *
     * @param B 右端矩阵 / Right-hand side matrix
     * @return 解矩阵 X / Solution matrix X
     ...
     * <p>
     * 求解上三角线性系统 UX = B，其中U是当前上三角矩阵，B是右端矩阵。
     * Solves the upper triangular linear system UX = B, where U is the current 
     * upper triangular matrix and B is the right-hand side matrix.
     * </p>
     *
     * @param B 右端矩阵 / Right-hand side matrix
     * @return 解矩阵 X / Solution matrix X
    ...
    /**
     * 计算矩阵的诱导L1范数（最大列和范数） / Compute matrix induced L1 norm (maximum column sum norm)
     * <p>
     * 计算矩阵的诱导L1范数，即所有列绝对值之和的最大值。
     * Computes the induced L1 norm of the matrix, which is the maximum absolute column sum.
     * </p>
     *
     * @return 诱导L1范数 / Induced L1 norm
     */
    public default T normL1() {
        // Compute maximum absolute column sum
        T maxSum = null;
        for (int j = 0; j < this.cols(); j++) {
            T colSum = this.getColumn(j).norm1();
            if (maxSum == null || ((Number) colSum).doubleValue() > ((Number) maxSum).doubleValue()) {
                maxSum = colSum;
            }
        }
        return maxSum;
     ...
     * <p>
     * 计算矩阵的L2范数，即最大奇异值。对于对称矩阵，这是绝对特征值的最大值。
     * Computes the L2 norm of the matrix, which is the largest singular value. 
     * For symmetric matrices, this is the largest absolute eigenvalue.
     * </p>
     *
     * @return L2范数 / L2 norm
     */
    public default T norm2() {
    ...
    /**
     * 计算矩阵的诱导无穷范数（最大行和范数） / Compute matrix induced infinity norm (maximum row sum norm)
     * <p>
     * 计算矩阵的诱导无穷范数，即所有行绝对值之和的最大值。
     * Computes the induced infinity norm of the matrix, which is the maximum absolute row sum.
     * </p>
     *
     * @return 诱导无穷范数 / Induced infinity norm
     */
    public default T normInf() {
        // Compute maximum absolute row sum
        T maxSum = null;
        for (int i = 0; i < this.rows(); i++) {
            T rowSum = this.getRow(i).norm1();
            if (maxSum == null || ((Number) rowSum).doubleValue() > ((Number) maxSum).doubleValue()) {
                maxSum = rowSum;
            }
        }
        return maxSum;
