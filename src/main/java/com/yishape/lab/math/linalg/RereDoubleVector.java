 ...
 * {@code
 * // 创建向量 / Create vector
 * double[] data = {1.0, 2.0f, 3.0f, 4.0f};
...
public class RereDoubleVector implements IDoubleVector,Serializable {
    ...
    /**
     * 避免同类向量间运算时的防御性拷贝。若 other 也是 RereDoubleVector，直接返回其内部 data；
     * 否则回退到 toDoubleArray() 做拷贝。
     */
    static double[] otherData(IVector<Double> other) {
        return (other instanceof RereDoubleVector) ? ((RereDoubleVector) other).data : other.toDoubleArray();
    }
    ...
    @Override
    public IDoubleVector dot(IVector<Double> vec) {
    ...
    @Override
    public  IDoubleVector axpy(double alpha, IVector<Double> x) {
        if (this.size() != x.size()) {
     ...
     */
    @Override
    public  IDoubleVector mmul(IMatrix<Double> matrix) {
        if (matrix == null) {
        ...
        // Avoid double copy: wrap vector data directly as 1xN row matrix,
        // and access matrix internal data without defensive copy.
        double[][] x1 = new double[][]{this.data};
        double[][] x2 = (matrix instanceof RereDoubleMatrix rdm) ? rdm.getData() : matrix.toDoubleArray();
     ...
     */
    @Override
    public IDoubleVector std(int ddof) {
        return IDoubleVector.of(Math.sqrt(this.varValue(ddof)));
    }
     ...
     */
    @Override
    public IDoubleVector var(int ddof) {
        if (this.length() <= ddof) {
            throw new IllegalArgumentException(
                "Vector length (" + this.length() + ") must be greater than ddof (" + ddof + ") to compute variance");
        }
        double result = this.subScalar(this.meanValue()).pow(2d).sumValue() / (double) (this.length() - ddof);
        return IDoubleVector.of(result);
    }
    ...
    public  IDoubleVector slice(int start, int end, int step) {
        ...
        if (step > 0) {
            ...
            if (start >= end) return IDoubleVector.of(new double[0]);
            ...
            if (start < 0) return IDoubleVector.of(new double[0]);
            if (end >= data.length) end = data.length;
            if (end < -1) end = -1;
            if (start <= end) return IDoubleVector.of(new double[0]);
            int absStep = -step;
    ...
    public  IDoubleVector slice(String sliceExpression) {
        ...
        double[] v = new double[indices.length];
    ...
    public  IDoubleVector fancyGet(int[] positions) {
        double[] v = IndexExpressionParser.fancyGetData(data, positions);
        return IDoubleVector.of(v);
    }

    @Override
    public void fancySet(int[] positions, Double[] values) {
        IndexExpressionParser.fancySetData(data, positions,
            java.util.Arrays.stream(values).mapToDouble(Double::doubleValue).toArray());
    }

    @Override
    public void fancySetScalar(int[] positions, Double value) {
        double[] expanded = new double[positions.length];
        java.util.Arrays.fill(expanded, value);
        IndexExpressionParser.fancySetData(data, positions, expanded);
    }

    @Override
    public void booleanSet(boolean[] booleanIndex, Double[] values) {
    ...
    @Override
    public void booleanSetScalar(boolean[] booleanIndex, Double value) {
        if (booleanIndex.length != data.length) {
            throw new IllegalArgumentException("布尔索引数组长度与向量长度不匹配: " + booleanIndex.length + " != " + data.length
                + " / Boolean index array length doesn't match vector length: " + booleanIndex.length + " != " + data.length);
        }
    ...
    public  IDoubleVector booleanGet(boolean[] booleanIndex) {
        double[] result = IndexExpressionParser.booleanGetData(data, booleanIndex);
        return IDoubleVector.of(result);
    }
     ...
     */
    @Override
    public IDoubleVector ptp() {
        return IDoubleVector.of(this.maxValue() - this.minValue());
    }
     ...
     * <p>
     * 使用double精度计算每个元素的sigmoid值（1/(1+e^(-x))）
     * Computes sigmoid (1/(1+e^(-x))) for each element in double precision
     * </p>
     *
     ...
     * <p>
     * 使用double精度计算每个元素的ReLU值（max(0, x)）
     * Computes ReLU (max(0, x)) for each element in double precision
     * </p>
     *
    ...
    public double mode() {
        ...
        // Sort then linear scan — avoids HashMap autoboxing overhead
        double[] sorted = this.data.clone();
    ...
    public double corr(IVector<Double> other) {
        ...
        // 计算标准差
        double stdX = this.stdValue();
        double stdY = other.stdValue();
    ...
    public double cov(IVector<Double> other) {
        ...
        // 计算协方差：使用中心化向量的内积（Bessel 修正，除以 n-1，与 RereDoubleMatrix.covarianceFromCentered 一致）
        IVector<Double> centeredX = this.subScalar(this.meanValue());
        IVector<Double> centeredY = other.subScalar(other.meanValue());
        double covariance = centeredX.innerProductValue(centeredY) / (len - 1);

        return covariance;
    }

    @Override
    public  IDoubleVector map(Function<Double, Double> fun) {
        if (fun == null) {
    ...
    public  IDoubleMatrix hessianMatrix() {
        ...
        IDoubleMatrix result = IDoubleMatrix.zeros(n, n);
