...
public class RereFloatMatrix implements IFloatMatrix,Serializable {
    ...
    public IMatrix<Float> sub(Float scalar) {
        var res = this.computer.binaryOperate(data, scalar, IFloatVectorComputer.BinaryOperation.SUBTRACT);
     ...
     */
    @Override
    public IMatrix<Float> sub(IMatrix<Float> other) {
        var res = this.computer.binaryOperate(data, other.toFloatArray(), IFloatVectorComputer.BinaryOperation.SUBTRACT);
        return IFloatMatrix.of(res);
    }
    ...
    public IMatrix<Float> multiplyScalar(Float scalar) {
        var res = this.computer.binaryOperate(data, scalar, IFloatVectorComputer.BinaryOperation.MULTIPLY);
        return IFloatMatrix.of(res);
    }
     ...
     */
    @Override
    public IMatrix<Float> add(IMatrix<Float> other) {
        var res = this.computer.binaryOperate(data, other.toFloatArray(), IFloatVectorComputer.BinaryOperation.ADD);
        return IFloatMatrix.of(res);
    }
     ...
     */
    @Override
    public IMatrix<Float> divide(IMatrix<Float> other) {
        var res = this.computer.binaryOperate(data, other.toFloatArray(), IFloatVectorComputer.BinaryOperation.DIVIDE);
        return IFloatMatrix.of(res);
    }
    ...
    public Tuple2<IVector<Float>, IMatrix<Float>> eigen() {
        var tp = Decomps.createEigen().decompose(this.toDoubleMatrix());
    ...
    public Tuple3<IMatrix<Float>, IVector<Float>, IMatrix<Float>> svd() {
        var tp = Decomps.createSVD().decompose(this.toDoubleMatrix());
    ...
    @Override
    public Float det() {
        return (float) DeterminantSolver.compute(this.toDoubleMatrix());
    }
    ...
    @Override
    public Float cond() {
        return (float) ConditionNumberSolver.compute(this.toDoubleMatrix());
    }
    ...
    public IMatrix<Float> slice(String rowSlice, String colSlice) {
        ...
        if (resultRows <= 0 || resultCols <= 0) {
            // 返回空矩阵而不是抛出异常
            // 但是要保持正确的维度：0行但保持列数，或者0列但保持行数
            if (resultRows <= 0 && resultCols > 0) {
                // 创建一个特殊的空矩阵，保持列数信息
                RereFloatMatrix emptyMatrix = new RereFloatMatrix(0, resultCols);
                return emptyMatrix;
            } else if (resultCols <= 0 && resultRows > 0) {
                // 创建一个特殊的空矩阵，保持行数信息
                RereFloatMatrix emptyMatrix = new RereFloatMatrix(resultRows, 0);
                return emptyMatrix;
            } else {
                return IFloatMatrix.of(new float[0][0]);
            }
     ...
     * 使用示例 / Usage Example:
     * <pre>{@code
     * RereFloatMatrix matrix = new RereFloatMatrix(new float[][]{{1, 2}, {3, 4}});
     * IMatrix<Float> squared = matrix.apply(x -> x * x);  // 结果: [[1, 4], [9, 16]]
     * IMatrix<Float> abs = matrix.apply(Math::abs);      // 结果: [[1, 2], [3, 4]]
     * }</pre>
     * </p>
     *
     * @param fun 要应用的函数，接受Float类型参数并返回Float类型结果 Function to apply, accepts
     * Float parameter and returns Float result
    ...
    public IMatrix<Float> multiply(IMatrix<Float> other) {
        var res = this.computer.binaryOperate(data, other.toFloatArray(), IFloatVectorComputer.BinaryOperation.MULTIPLY);
        return IFloatMatrix.of(res);
    }
    ...
    public boolean[][] eq(IMatrix<Float> other) {
        ...
        return this.computer.logicalCompare(data, other.toFloatArray(), IFloatVectorComputer.LogicalCompare.EQUALS);
    }
    ...
    public boolean[][] lt(IMatrix<Float> other) {
        ...
        return this.computer.logicalCompare(data, other.toFloatArray(), IFloatVectorComputer.LogicalCompare.LESS_THAN);
    }
    ...
    public boolean[][] gt(IMatrix<Float> other) {
        ...
        return this.computer.logicalCompare(data, other.toFloatArray(), IFloatVectorComputer.LogicalCompare.GREATER_THAN);
    }
    ...
    public boolean[][] ge(IMatrix<Float> other) {
        ...
        return this.computer.logicalCompare(data, other.toFloatArray(), IFloatVectorComputer.LogicalCompare.GREATER_THAN_OR_EQUALS);
    }
    ...
    public boolean[][] le(IMatrix<Float> other) {
        ...
        return this.computer.logicalCompare(data, other.toFloatArray(), IFloatVectorComputer.LogicalCompare.LESS_THAN_OR_EQUALS);
    }
     ...
     */
    @Override
    public IMatrix<Float> forwardSolve(IMatrix<Float> B) {
        if (B == null) {
     ...
     */
    @Override
    public IMatrix<Float> backwardSolve(IMatrix<Float> B) {
        if (B == null) {
            throw new IllegalArgumentException("Right-hand side matrix cannot be null");
        }
