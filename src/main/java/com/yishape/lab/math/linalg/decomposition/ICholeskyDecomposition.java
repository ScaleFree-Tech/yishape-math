...
public interface ICholeskyDecomposition extends IMatrixDecomposition<IMatrix<Double>> {
     ...
     * with L in the lower triangle. Only supported when the input is a
     * {@link com.yishape.lab.math.linalg.RereDoubleMatrix} (falls back to
     * {@link #decompose(IMatrix)} otherwise).
     *
     * @param matrix will be MUTATED to store L (lower triangle)
     * @return L matrix (view over the mutated input)
     * @throws NonSymmetricMatrixException if the matrix is not symmetric
     * @throws NonPositiveDefiniteMatrixException if the matrix is not positive definite
     */
    IMatrix<Double> decomposeInPlace(IMatrix<Double> matrix);
