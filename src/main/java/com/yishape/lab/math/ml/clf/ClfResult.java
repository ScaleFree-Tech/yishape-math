 ...
 * @since 1.0
 */
public abstract class ClfResult implements Serializable, IRichReport {
     ...
     * 例如 {@link com.yishape.lab.math.ml.reg.RereLinearRegression#getFeatureWeights()}、
     * {@link com.yishape.lab.math.ml.clf.svm.RereLinearSVM#getFeatureWeights()}。
     *
    ...
    public String toBriefReport() {
        return String.format("%s | accuracy=%.4f | classes=%d",
                getModelTypeDescription(),
                trainAccuracy >= 0 ? trainAccuracy : Double.NaN,
                numClasses);
    }
