...
public class RFTreeNode {
    ...
    Map<String, Object> toParams() {
        ...
        if (isLeaf) {
            ...
            m.put("featureIndex", featureIndex);
            m.put("threshold", threshold);
            m.put("gain", gain);
            m.put("leftChild", leftChild.toParams());
            m.put("rightChild", rightChild.toParams());
        }
        return m;
    }

    @SuppressWarnings("unchecked")
    static RFTreeNode fromParams(Map<String, Object> m) {
        boolean isLeaf = (Boolean) m.get("isLeaf");
        int depth = ((Number) m.get("depth")).intValue();
        int sampleCount = ((Number) m.get("sampleCount")).intValue();
        if (isLeaf) {
            String leafLabel = (String) m.get("leafLabel");
            Map<String, Integer> countsStr = (Map<String, Integer>) m.get("classCounts");
            Map<Integer, Integer> classCounts = new LinkedHashMap<>();
            if (countsStr != null) {
                for (Map.Entry<String, Integer> e : countsStr.entrySet()) {
                    classCounts.put(Integer.parseInt(e.getKey()), e.getValue());
                }
            }
            return new RFTreeNode(leafLabel, depth, sampleCount, classCounts);
        } else {
            int featureIndex = ((Number) m.get("featureIndex")).intValue();
            double threshold = ((Number) m.get("threshold")).doubleValue();
            double gain = ((Number) m.get("gain")).doubleValue();
            RFTreeNode left = fromParams((Map<String, Object>) m.get("leftChild"));
            RFTreeNode right = fromParams((Map<String, Object>) m.get("rightChild"));
            return new RFTreeNode(featureIndex, threshold, left, right, depth, sampleCount, gain, null);
        }
    }
}