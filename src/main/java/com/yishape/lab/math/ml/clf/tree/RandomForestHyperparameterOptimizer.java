...
public class RandomForestHyperparameterOptimizer implements IObjectiveFunction {
    ...
    public OptimizationResult optimizeWithLBFGS(int maxIterations) {
        ...
        // 创建LBFGS优化器
        IOptimizer lbfgs = Opts.lbfgs(1e-6, maxIterations);
    ...
    public double computeObjective(IVector x) {
        ...
        int nEstimators = (int) (bounds.minEstimators + 
                                x.get(0) * (bounds.maxEstimators - bounds.minEstimators));
        int maxDepth = (int) (bounds.minDepth + 
                             x.get(1) * (bounds.maxDepth - bounds.minDepth));
        int minSamplesSplit = (int) (bounds.minSamplesSplit + 
                                    x.get(2) * (bounds.maxSamplesSplit - bounds.minSamplesSplit));
        int minSamplesLeaf = (int) (bounds.minSamplesLeaf + 
                                   x.get(3) * (bounds.maxSamplesLeaf - bounds.minSamplesLeaf));
        int maxFeatures = (int) (trainFeatures.cols() * 
                                (bounds.minMaxFeaturesRatio + 
                                 x.get(4) * (bounds.maxMaxFeaturesRatio - bounds.minMaxFeaturesRatio)));
    ...
    private IVector computeNumericalGradient(IVector x) {
        ...
        for (int i = 0; i < x.size(); i++) {
            ...
            xPlusEpsilon.set(i, x.get(i) + epsilon);
    ...
    private IVector clampParameters(IVector params) {
        ...
        for (int i = 0; i < params.size(); i++) {
            clampedParams[i] = Math.max(0.0, Math.min(1.0, params.get(i)));
        }
    ...
    private OptimizationResult convertToOptimizationResult(IVector params, double score) {
        int nEstimators = (int) (bounds.minEstimators + 
                                params.get(0) * (bounds.maxEstimators - bounds.minEstimators));
        int maxDepth = (int) (bounds.minDepth + 
                             params.get(1) * (bounds.maxDepth - bounds.minDepth));
        int minSamplesSplit = (int) (bounds.minSamplesSplit + 
                                    params.get(2) * (bounds.maxSamplesSplit - bounds.minSamplesSplit));
        int minSamplesLeaf = (int) (bounds.minSamplesLeaf + 
                                   params.get(3) * (bounds.maxSamplesLeaf - bounds.minSamplesLeaf));
        int maxFeatures = (int) (trainFeatures.cols() * 
                                (bounds.minMaxFeaturesRatio + 
                                 params.get(4) * (bounds.maxMaxFeaturesRatio - bounds.minMaxFeaturesRatio)));
