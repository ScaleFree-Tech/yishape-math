...
public class XGBoostLossFunction  implements Serializable{
    ...
    private double computeMulticlassSoftmaxLoss(IMatrix predictions, IMatrix labels) {
        ...
        for (int i = 0; i < numSamples; i++) {
            ...
            // 使用向量运算计算交叉熵损失
            // 先对概率应用log函数，确保数值稳定性
            IVector logProbs = probs.apply(p -> Math.log(Math.max(EPS, p)));
            
            // 使用向量内积计算损失：-sum(label * log(prob))
            double sampleLoss = -labelRow.dotValue(logProbs);
    ...
    private IMatrix computeMulticlassSoftmaxGradients(IMatrix predictions, IMatrix labels) {
        ...
        for (int i = 0; i < numSamples; i++) {
            ...
            for (int j = 0; j < numClasses; j++) {
                gradients.set(i, j, gradient.get(j));
            }
        }
    ...
    private IMatrix computeMulticlassSoftmaxHessians(IMatrix predictions, IMatrix labels) {
        ...
        for (int i = 0; i < numSamples; i++) {
            ...
            // 应用数值稳定性约束
            hessianRow = hessianRow.apply(h -> Math.max(EPS, h));
            
            // 将海塞矩阵向量设置到结果矩阵中
            for (int j = 0; j < numClasses; j++) {
                hessians.set(i, j, hessianRow.get(j));
            }
        }
    ...
    private IMatrix predictMulticlassProba(IMatrix predictions) {
        ...
        for (int i = 0; i < numSamples; i++) {
            ...
            for (int j = 0; j < numClasses; j++) {
                probabilities.set(i, j, probs.get(j));
            }
        }
