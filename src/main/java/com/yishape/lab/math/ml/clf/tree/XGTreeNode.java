...
public class XGTreeNode {
    ...
    static XGTreeNode fromParams(Map<String, Object> m) {
        ...
        if (isLeaf) {
            double leafValue = ((Number) m.get("leafValue")).doubleValue();
            return new XGTreeNode(leafValue, depth, sampleCount);
        } else {
            ...
            XGTreeNode left = fromParams((Map<String, Object>) m.get("leftChild"));
            XGTreeNode right = fromParams((Map<String, Object>) m.get("rightChild"));
            return new XGTreeNode(featureIndex, threshold, left, right, depth, sampleCount, gain);
        }
    }
}