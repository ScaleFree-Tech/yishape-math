...
//    IOptimizer optimizer = new RereSteepestDescent();
     ...
     * @param labels 标签数组
     * @param k 折数
     * @param lambda1Candidates L1正则化候选值
     * @param lambda2Candidates L2正则化候选值
     * @return 最佳参数组合和对应的平均准确率
     */
    public CrossValidationResult crossValidate(IMatrix features, String[] labels, int k,
            double[] lambda1Candidates, double[] lambda2Candidates) {
        if (features == null || labels == null || lambda1Candidates == null || lambda2Candidates == null) {
            throw new IllegalArgumentException("输入参数不能为null");
        }

        if (k < 2) {
            throw new IllegalArgumentException("折数k必须至少为2");
        }

        int numSamples = features.getRowNum();
        if (numSamples < k) {
            throw new IllegalArgumentException("样本数不能少于折数");
        }

        // 存储最佳结果
        double bestAccuracy = 0.0;
        double bestLambda1 = lambda1Candidates[0];
        double bestLambda2 = lambda2Candidates[0];

        // 尝试不同的正则化参数组合
        for (double lambda1 : lambda1Candidates) {
            for (double lambda2 : lambda2Candidates) {
                double totalAccuracy = 0.0;

                // k折交叉验证
                for (int fold = 0; fold < k; fold++) {
                    // 分割数据
                    int testStart = fold * numSamples / k;
                    int testEnd = (fold + 1) * numSamples / k;

                    // 构建训练集和测试集
                    int trainSize = numSamples - (testEnd - testStart);
                    int testSize = testEnd - testStart;

                    double[][] trainFeaturesData = new double[trainSize][features.getColNum()];
                    String[] trainLabels = new String[trainSize];
                    double[][] testFeaturesData = new double[testSize][features.getColNum()];
                    ...
                    for (int i = 0; i < numSamples; i++) {
                        if (i >= testStart && i < testEnd) {
                            // 测试集
                            for (int j = 0; j < features.getColNum(); j++) {
                                testFeaturesData[testIdx][j] = (double) features.get(i, j);
                            }
                            ...
                            // 训练集
                            for (int j = 0; j < features.getColNum(); j++) {
                                trainFeaturesData[trainIdx][j] = (double) features.get(i, j);
                            }
                    ...
                    RereLogisticRegression model = new RereLogisticRegression(
                            this.learningRate, this.maxIterations, this.tolerance, lambda1, lambda2, this.randomSeed);
                    model.setStandardizeFeatures(this.standardizeFeatures);

                    IMatrix trainFeatures = Linalg.matrix(trainFeaturesData);
                    model.fit(trainFeatures, trainLabels);

                    // 在测试集上评估
                    IMatrix testFeatures = Linalg.matrix(testFeaturesData);
                    String[] predictions = model.predictBatch(testFeatures);

                    // 计算准确率
                    int correct = 0;
                    for (int i = 0; i < testSize; i++) {
                        if (predictions[i].equals(testLabels[i])) {
                            correct++;
                        }
                    }
                ...
                // 计算平均准确率
                double avgAccuracy = totalAccuracy / k;
                ...
                System.out.println(String.format("λ₁=%.4f, λ₂=%.4f: 平均准确率=%.4f",
                        lambda1, lambda2, avgAccuracy));
            }
        }

        System.out.println(String.format("最佳参数: λ₁=%.4f, λ₂=%.4f, 准确率=%.4f",
                bestLambda1, bestLambda2, bestAccuracy));

        return new CrossValidationResult(bestLambda1, bestLambda2, bestAccuracy);
    }

    /**
     * 批量预测结果类，包含预测标签和概率
     */
    public static class BatchPredictionResult {
    ...
    public static class CrossValidationResult {
        ...
        private final double bestLambda1;
        private final double bestLambda2;
        private final double bestAccuracy;

        public CrossValidationResult(double bestLambda1, double bestLambda2, double bestAccuracy) {
            this.bestLambda1 = bestLambda1;
            this.bestLambda2 = bestLambda2;
            this.bestAccuracy = bestAccuracy;
        }

        public double getBestLambda1() {
            return bestLambda1;
        }

        public double getBestLambda2() {
            return bestLambda2;
        }
