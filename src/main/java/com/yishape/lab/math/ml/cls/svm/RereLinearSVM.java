package com.yishape.lab.math.ml.cls.svm;
 ...
 * </p>
 */
public class RereLinearSVM implements IClassifier, ISerializableModel {

    private static final Logger log = LoggerFactory.getLogger(RereLinearSVM.class);
     ...
     * 命名与 {@link com.yishape.lab.math.ml.lr.RereLinearRegression#getFeatureWeights()} 对齐。
    ...
    public ClassificationResult fit(IMatrix feature, String[] labels) {
        ...
        trained = true;
        LinearSvmResult result = new LinearSvmResult();
        result.setCParam(C);
    ...
    private static void computeMeanStd(IMatrix X, double[] mean, double[] std) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < d; j++) {
                double t = X.get(i, j).doubleValue() - mean[j];
                std[j] += t * t;
            }
        }
        for (int j = 0; j < d; j++) {
    ...
    private static IMatrix applyStandardize(IMatrix X, double[] mean, double[] std) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < d; j++) {
                buf[i][j] = (X.get(i, j).doubleValue() - mean[j]) / std[j];
            }
        }
    ...
    private IVector transform(IVector x) {
        ...
        for (int j = 0; j < numFeatures; j++) {
            z[j] = (x.get(j).doubleValue() - mean[j]) / std[j];
        }
    ...
    private double decisionOvR(IVector xz, int k) {
        double s = bOvR[k];
        for (int j = 0; j < numFeatures; j++) {
            s += wOvR[k][j] * xz.get(j).doubleValue();
        }
        return s;
    }
    ...
    private static final class SquaredHingeSvmProblem implements IObjectiveFunction, IGradientFunction {
        ...
        public IVector computeGradient(IVector thetaVec) {
            ...
            for (int i = 0; i < m; i++) {
                ...
                for (int j = 0; j < d; j++) {
                    g[j] += coeff * X.get(i, j).doubleValue();
                }
                g[d] += coeff;
            }
