...
public class XGTree implements Serializable {
    ...
    /** 训练矩阵列数（全局分箱） */
    private transient int globalNumCols;

    /** 各列特征全局 [min,max]，长度 = globalNumCols */
    private transient double[] featMin;

    private transient double[] featMax;

    /** HIST + {@link XGBoostHistogramBinning#QUANTILE_WEIGHTED_SKETCH} 时非 null，形状 {@code [col][maxBin+1]} */
    private transient double[][] histQuantileEdges;

    /** 直方图分箱策略（仅 HIST） */
    private XGBoostHistogramBinning histogramBinning = XGBoostHistogramBinning.UNIFORM;

    /** 直方图临时缓冲 */
    private transient double[] histGradBuf;

    private transient double[] histHessBuf;

    private transient int[] histCountBuf;
    ...
    private void computeGlobalBinning(IMatrix features) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < globalNumCols; j++) {
                double v = features.get(i, j).doubleValue();
                if (v < featMin[j]) {
                    featMin[j] = v;
                }
                if (v > featMax[j]) {
    ...
    private SplitResult findBestSplitExact(IMatrix features, IVector gradients, IVector hessians,
        ...
        for (int fi : feats) {
            ...
            Arrays.sort(order, Comparator.comparingDouble(
                    i -> features.get(sampleIndices[i], fcol).doubleValue()));
            ...
            for (int i = 0; i < n - 1; i++) {
                ...
                leftGradSum += gradients.get(row).doubleValue();
                leftHessSum += hessians.get(row).doubleValue();

                double v0 = features.get(row, fi).doubleValue();
                double v1 = features.get(sampleIndices[order[i + 1]], fi).doubleValue();
    ...
    private SplitResult findBestSplitHist(IMatrix features, IVector gradients, IVector hessians,
        ...
        for (int fi : feats) {
            ...
            for (int idx : sampleIndices) {
                double v = features.get(idx, fi).doubleValue();
                int b = useQuantile ? binForQuantile(v, histQuantileEdges[fi]) : binUniform(v, mn, width);
                histGradBuf[b] += gradients.get(idx).doubleValue();
                histHessBuf[b] += hessians.get(idx).doubleValue();
                histCountBuf[b]++;
            }
