...
public final class XgbWeightedQuantileSketch implements Serializable {
    ...
    private static void buildColumn(IMatrix features, IVector hessians, int[] rows, int col, int maxBin, double[] e) {
        ...
        Arrays.sort(ord, Comparator.comparingDouble(i -> features.get(rows[i], col).doubleValue()));

        List<Double> vx = new ArrayList<>();
        List<Double> hw = new ArrayList<>();
        for (int i = 0; i < n; ) {
            double val = features.get(rows[ord[i]], col).doubleValue();
            double sw = 0.0;
            int j = i;
            while (j < n && Double.compare(features.get(rows[ord[j]], col).doubleValue(), val) == 0) {
                sw += Math.max(EPS, hessians.get(rows[ord[j]]).doubleValue());
                j++;
            }
            vx.add(val);
            hw.add(sw);
