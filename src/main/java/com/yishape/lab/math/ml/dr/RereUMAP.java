 ...
 * @author lteb2
 */
public class RereUMAP implements ITransform<Double>, ISerializableModel {

    private static final YishapeLogger log = YishapeLogger.getLogger(RereUMAP.class);

        private IMatrix<Double> feature;
    private int nComponents = -1;

    // UMAP算法超参数 / UMAP algorithm hyperparameters
    private int nNeighbors = 15;        // k近邻数量 / Number of k-nearest neighbors
    private double minDist = 0.1f;       // 最小距离参数 / Minimum distance parameter
    private int nEpochs = 500;          // 最大迭代次数 / Maximum iterations
    private double learningRate = 1.0f;  // 学习率 / Learning rate
    private double spread = 1.0f;        // 散布参数 / Spread parameter
    private double localConnectivity = 1.0f; // 局部连通性 / Local connectivity
    private double repulsionStrength = 1.0f; // 排斥强度 / Repulsion strength
    private int negativeSampleRate = 5; // 负样本采样率 / Negative sampling rate
    private double initialAlpha = 1.0f;  // 初始学习率 / Initial learning rate
    private double momentum = 0.5;       // 动量 / Momentum
    ...
    public IMatrix transform(IMatrix feature) {
        ...
        log.debug("开始UMAP降维，数据形状: " + Arrays.toString(feature.shape()) +
                          "，目标维度: " + nComponents);
        ...
        int[][] knnIndices = computeKNearestNeighbors((IMatrix)feature);
        double[][] knnDistances = computeKNNDistances((IMatrix)feature, knnIndices);
        ...
        IMatrix embedding = initializeEmbedding(n, nComponents);
     ...
     * @return 当前实例
     */
    public RereUMAP setNComponents(int nComponents) {
    ...
    private IMatrix optimizeEmbedding(IMatrix weights, IMatrix embedding, int[][] knnIndices) {
        ...
        for (int epoch = 0; epoch < nEpochs; epoch++) {
            ...
            // 更新嵌入
            // 使用向量化操作更新嵌入，替代手动循环
            IMatrix gradUpdate = (IMatrix)grad.multiplyByScalar(alpha);
            ...
            headEmbedding = (IMatrix)headEmbedding.add(diff.multiplyByScalar(momentum));
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < n; j++) {
                if (i != j) {
                    ...
                    IVector gradUpdate = (IVector)diff.multiplyByScalar(4.0 * coeff);
    ...
    private void updateNegativeGradient(IMatrix embedding, int i, int neg, double alpha) {
        ...
        if (distance > 0) {
            ...
            IVector gradUpdate = (IVector)diff.multiplyByScalar(alpha * grad_coeff);
    ...
    public Map<String, Object> toParams() {
        ...
        p.put("nNeighbors", nNeighbors);
        p.put("minDist", minDist);
        p.put("nEpochs", nEpochs);
        p.put("learningRate", learningRate);
        p.put("spread", spread);
        p.put("localConnectivity", localConnectivity);
        p.put("repulsionStrength", repulsionStrength);
        p.put("negativeSampleRate", negativeSampleRate);
        p.put("initialAlpha", initialAlpha);
    ...
    public void fromParams(Map<String, Object> p) {
        ...
        this.nNeighbors = ((Number) p.get("nNeighbors")).intValue();
        this.minDist = ((Number) p.get("minDist")).doubleValue();
        this.nEpochs = ((Number) p.get("nEpochs")).intValue();
        this.learningRate = ((Number) p.get("learningRate")).doubleValue();
        this.spread = ((Number) p.get("spread")).doubleValue();
        this.localConnectivity = ((Number) p.get("localConnectivity")).doubleValue();
        this.repulsionStrength = ((Number) p.get("repulsionStrength")).doubleValue();
        this.negativeSampleRate = ((Number) p.get("negativeSampleRate")).intValue();
        this.initialAlpha = ((Number) p.get("initialAlpha")).doubleValue();
