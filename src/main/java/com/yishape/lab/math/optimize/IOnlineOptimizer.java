...
public interface IOnlineOptimizer  extends Serializable{
    ...
    default <T> IVector step(T[] samples, BiFunction<IVector, T, Double> lossFunction) {
        ...
        IVector averageGradient = totalGradient.multiplyByScalar(1.0 / samples.length);
    ...
    default <T> IVector computeNumericalGradient(IVector params, T sample, BiFunction<IVector, T, Double> lossFunction) {
        double epsilon = 1e-6;
        IVector gradient = params.copy().multiplyByScalar(0.0);
        
        // 对每个参数计算偏导数
        for (int i = 0; i < params.size(); i++) {
            // 保存原始参数值
            double originalValue = params.get(i);
    ...
    /**
     * Export optimizer internal state buffers (momentum, velocity, etc.) for checkpointing.
     * Returns empty map by default — override in subclasses that maintain state.
     */
    default java.util.Map<String, double[]> optimizerStateDict() {
        return java.util.Map.of();
    }

    /**
     * Load optimizer internal state from a state dictionary.
     * No-op by default — override in subclasses that maintain state.
     */
    default void loadOptimizerStateDict(java.util.Map<String, double[]> stateDict) {
    }
}