...
public interface ILinProgSolver {
    ...
    /**
     * 求解线性规划问题
     * 本方法的存在是必要的，因为有些求解器（如内点法）能够在不添加松驰变量的情况下求解，大大节约变量，提高速度。因此提供专门的&lt;=入口
     *
     * @param c 目标函数系数（最小化问题）
     * @param A_ub 小于等于约束矩阵系数
     * @param b_ub 小于等于约束值（不等式右方）
    ...
    public static ILinProgSolver of() {
        return of(LpSolverType.Highs);
    }
     ...
     * @return
     */
    public static ILinProgSolver of(LpSolverType solver) {
        if (null == solver) {
            return null;
        } else {
            var model = switch (solver) {
                case LpSolverType.Highs ->
                    new HighsLinProgSolver();
                case LpSolverType.Simplex ->
                    new RereSimplexLinProgSolver();
                case LpSolverType.InnerPoints ->
                    new InteriorPointLinProgSolver();
                default ->
                    new HighsLinProgSolver();
            };
