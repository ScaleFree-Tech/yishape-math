...
public class PivotSelectionStrategy {
    ...
    public Integer selectEnteringVariableDantzig(RereSimplexTableau tableau) {
        // Phase I uses row 0; Phase II uses the last objective row.
        // getNumObjectiveFunctions() is 2 during Phase I (W+Z) or Big-M,
        // and 1 after dropPhase1Objective or in standard Phase II.
        int objectiveRow = (tableau.getCurrentPhase() == 1) ? 0 : (tableau.getNumObjectiveFunctions() - 1);

        Integer enteringVar = null;

        if (tableau.getCurrentPhase() == 1) {
            // Phase I: minimization - select most negative coefficient on non-artificial vars
    ...
    public Integer selectLeavingVariable(RereSimplexTableau tableau, int enteringVar) {
        ...
        for (int i = tableau.getNumObjectiveFunctions(); i < tableau.getHeight(); i++) {
            ...
            if (RerePrecision.compareTo(entry, 0d, cutOff) > 0) {
                // rhs must be non-negative for feasible solutions; using abs
                // masks negative rhs and allows infeasible pivots.
    ...
    public void performPivotOperation(RereSimplexTableau tableau, int leavingRow, int enteringVar) {
        // getNumObjectiveFunctions() correctly reports the number of objective rows
        // in ALL tableau states: 2 during Phase I or Big-M, 1 after dropPhase1Objective
        // or in standard Phase II. Using this instead of a dimension-based heuristic
        // prevents pivot row misalignment after Phase I→Phase II transition.
        int pivotRow = leavingRow + tableau.getNumObjectiveFunctions();
