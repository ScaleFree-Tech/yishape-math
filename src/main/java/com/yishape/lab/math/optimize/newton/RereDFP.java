...
public class RereDFP implements IOptimizer {
    ...
    public OptResult optimize(IVector initX, IObjectiveFunction objFun, IGradientFunction grdFun) {
        ...
        // Line search instance (created once outside loop)
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            ...
            IVector searchDirection = H.mmul(grad).multiplyByScalar(-1.0);

            // Line search to determine step size
            double stepSize = lineSearch.search(x, searchDirection, objFun, grdFun, grad);
    ...
    private IMatrix<Double> updateInverseHessian(IMatrix<Double> H, IVector s, IVector y) {
        ...
        double sTy = s.innerProductValue(y);
        ...
        // Compute (s * s^T) / (s^T * y)
        IMatrix<Double> term1 = sOuter.multiplyByScalar(1.0 / sTy);
        ...
        // Compute y^T * H * y
        double yTHy = y.innerProductValue(Hy);

        // Avoid division by zero or near-zero
        if (Math.abs(yTHy) < 1e-15) {
            return H;
        }

        // Compute (H * y * y^T * H) / (y^T * H * y)
        IMatrix<Double> term2 = HyOuter.multiplyByScalar(1.0 / yTHy);
