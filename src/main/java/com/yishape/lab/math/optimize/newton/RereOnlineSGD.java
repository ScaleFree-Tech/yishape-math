...
public class RereOnlineSGD implements IOnlineOptimizer {
    ...
    private static final YishapeLogger log = YishapeLogger.getLogger(RereOnlineSGD.class);
    ...
    public void initialize(IVector initialParams) {
        ...
        // 初始化动量缓存
        if (momentum > 0.0) {
            this.momentumBuffer = initialParams.copy().multiplyByScalar(0.0);
        }
    ...
    public IVector step(IVector gradient) {
        ...
        double[] newMomentumBuf = (momentum > 0.0) ? new double[n] : null;
        double[] momArr = (momentum > 0.0 && momentumBuffer != null) ? ((com.yishape.lab.math.linalg.IDoubleVector) momentumBuffer).getData() : null;
        ...
        for (int i = 0; i < n; i++) {
            ...
            double updateI;
            if (mom > 0.0 && momArr != null) {
                double newMomI = mom * momArr[i] + gi;
                newMomentumBuf[i] = newMomI;
                updateI = newMomI;
            } else {
                updateI = gi;
            }
            newP[i] = p[i] - lr * updateI;
        }

        currentParams = com.yishape.lab.math.linalg.IDoubleVector.of(newP);
        if (momentum > 0.0 && newMomentumBuf != null) {
            momentumBuffer = com.yishape.lab.math.linalg.IDoubleVector.of(newMomentumBuf);
        }
    ...
    public void setCurrentParams(IVector params) {
        ...
        if (initialized && momentum > 0.0) {
            this.momentumBuffer = params.copy().multiplyByScalar(0.0);
        }
    ...
    public RereOnlineSGD setMomentum(double momentum) {
        ...
        if (initialized) {
            if (momentum > 0.0 && this.momentumBuffer == null) {
                this.momentumBuffer = currentParams.copy().multiplyByScalar(0.0);
     ...
     * @return this / 返回自身以支持链式调用 / Returns this for method chaining
     */
    public RereOnlineSGD setSkipGradientValidation(boolean skipGradientValidation) {
    ...
    public Map<String, double[]> optimizerStateDict() {
        ...
        if (momentum > 0.0 && momentumBuffer != null) {
            state.put("momentum_buffer", toPrimitiveArray(momentumBuffer));
        }
        return state;
    }
    ...
    public void loadOptimizerStateDict(Map<String, double[]> stateDict) {
        ...
        if (momData != null && momentumBuffer != null) {
            setVectorData(momentumBuffer, momData);
        }
    }
