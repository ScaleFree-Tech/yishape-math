...
public interface IPlot extends Serializable {
    ...
    /**
     * Auto-detect best legend position and draw legend (Fluent API, seaborn-style).
     * Equivalent to {@code legend(LegendPositions.BEST)}.
     * 自动计算最佳位置并绘制图例（流式API，Seaborn 风格），等价于 {@code legend(LegendPositions.BEST)}。
     *
     * @return Current instance for method chaining 当前实例，支持链式调用
     */
    IPlot legend();
    ...
    /**
     * Set legend position (Fluent API).
     * Supported values: "top-right", "top-left", "bottom-right", "bottom-left",
     * "right", "left", "top", "bottom", "best".
     * 设置图例位置（流式API）。
     *
     * @param position Position string 位置字符串
     * @return Current instance for method chaining 当前实例，支持链式调用
     */
    IPlot legend(String position);

    /**
     * Show or hide the legend (Fluent API).
     * 显示或隐藏图例（流式API）。
     *
     * @param show {@code true} to show, {@code false} to hide
     * @return Current instance for method chaining 当前实例，支持链式调用
     */
    IPlot legend(boolean show);
    ...
    /**
     * Persists the current figure as PDF (typically one page with a raster chart; vector support depends on backend).
     * 保存为 PDF（常见为单页嵌入栅格图；是否矢量取决于具体实现）。
     *
     * @param filename output path
     * @return this instance for chaining
     */
    IPlot saveAsPdf(String filename);
