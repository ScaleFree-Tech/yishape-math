...
public final class MeshGridHelper {
    ...
    public static <T extends Number> Tuple3<IVector<T>, IVector<T>, IVector<T>> meshgrid3RavelIj(
        ...
        int planeSize = nx * ny;
        int totalSize = planeSize * nz;
        IVector<T> allX = IVector.zeros(totalSize, type);
        IVector<T> allY = IVector.zeros(totalSize, type);
        IVector<T> allZ = IVector.zeros(totalSize, type);
        for (int k = 0; k < nz; k++) {
            Tuple2<IMatrix<T>, IMatrix<T>> xy = meshgridIj(x, y);
            IVector<T> fx = xy._1.flatten();
            IVector<T> fy = xy._2.flatten();
            int offset = k * planeSize;
            for (int idx = 0; idx < planeSize; idx++) {
                allX.set(offset + idx, fx.get(idx));
                allY.set(offset + idx, fy.get(idx));
                allZ.set(offset + idx, z.get(k));
            }
        }
        return new Tuple3<>(allX, allY, allZ);
    }
