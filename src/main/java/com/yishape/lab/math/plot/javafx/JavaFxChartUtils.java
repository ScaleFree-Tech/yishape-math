...
public class JavaFxChartUtils {
    ...
    public static void drawAxesCartesian(GraphicsContext gc, JavaFxChartRenderer.ChartConfig config,
        ...
        // 是否画网格仅由图窗工具栏 / ChartConfig 决定；主题的 showGrid 仅作主题缺省样式，不覆盖用户勾选
    ...
    private static void drawCartesianAxisLabels(GraphicsContext gc, JavaFxChartRenderer.ChartConfig config,
        ...
        drawCartesianAxisTitles(gc, config, themeManager);
    }

    /**
     * 绘制 X/Y 轴标题（{@link JavaFxChartRenderer.ChartConfig#xlabel} / {@link JavaFxChartRenderer.ChartConfig#ylabel}），
     * 供 {@link #drawAxesCartesian} 及未走笛卡尔轴绘制的图类型（如柱状图、热力图）复用。
     */
    public static void drawCartesianAxisTitles(GraphicsContext gc, JavaFxChartRenderer.ChartConfig config,
                                              JavaFxThemeManager themeManager) {
        ...
        if (xn != null && !xn.isEmpty()) {
            gc.setTextAlign(TextAlignment.CENTER);
            gc.fillText(xn, config.width / 2.0, config.height - 10);
        }
    ...
    public static void drawLegend(GraphicsContext gc, List<JavaFxChartRenderer.SeriesData> dataList,
        ...
        try {
            ...
            if (LegendPositions.BEST.equals(pos)) {
                ...
                for (JavaFxChartRenderer.SeriesData s : dataList) {
                    if (s.x != null && s.y != null) {
                        int len = Math.min(s.x.length(), s.y.length());
                        for (int i = 0; i < len; i++) {
                            pts.add(new double[]{s.x.get(i), s.y.get(i)});
                        }
                    }
                }
                pos = LegendPositions.best(pts,
                    config.paddingLeft, config.width - config.paddingRight,
                    config.paddingTop, config.height - config.paddingBottom);
            }

            double boxOuterW = legendWidth + 16;
            double boxOuterH = legendHeight + 16;
            double legendX, legendY;
            double midX = config.width / 2.0;
            double midY = config.height / 2.0;

            switch (pos) {
                case LegendPositions.TOP_LEFT -> {
                    legendX = config.paddingLeft + 10 + config.legendOffsetX;
                    legendY = config.paddingTop + 30 + config.legendOffsetY;
                }
                case LegendPositions.BOTTOM_RIGHT -> {
                    legendX = config.width - config.paddingRight - legendWidth + 20 + config.legendOffsetX;
                    legendY = config.height - config.paddingBottom - legendHeight - 10 + config.legendOffsetY;
                }
                ...
                case LegendPositions.RIGHT -> {
                    legendX = config.width - config.paddingRight + 5 + config.legendOffsetX;
                    legendY = midY - legendHeight / 2 + config.legendOffsetY;
                }
                case LegendPositions.LEFT -> {
                    legendX = config.paddingLeft - legendWidth - 15 + config.legendOffsetX;
                    legendY = midY - legendHeight / 2 + config.legendOffsetY;
                }
                case LegendPositions.TOP -> {
                    legendX = midX - legendWidth / 2 + config.legendOffsetX;
                    legendY = config.paddingTop - legendHeight - 10 + config.legendOffsetY;
                }
                case LegendPositions.BOTTOM -> {
                    legendX = midX - legendWidth / 2 + config.legendOffsetX;
                    legendY = config.height - config.paddingBottom + 10 + config.legendOffsetY;
                }
                default -> { // TOP_RIGHT
                    legendX = config.width - config.paddingRight - legendWidth + 20 + config.legendOffsetX;
                    legendY = config.paddingTop + 30 + config.legendOffsetY;
            ...
            double limLeft = 2;
            double limTop = 2;
            double limRight = Math.max(limLeft + 1, config.width - boxOuterW - 2);
            double limBot = Math.max(limTop + 1, config.height - boxOuterH - 2);
            legendX = Math.max(limLeft, Math.min(legendX, limRight));
            legendY = Math.max(limTop, Math.min(legendY, limBot));
            ...
            if (config.hitTestHandler != null) {
                double bx0 = legendX - 8;
                double by0 = legendY - 8;
                double bx1 = legendX + legendWidth + 8;
                double by1 = legendY + legendHeight + 8;
                config.hitTestHandler.setLegendPickBounds(bx0, by0, bx1, by1);
            }
        } finally {
            gc.restore();
        }
