...
public class JavaFxInteractionHandler {
    ...
    /** 与 {@link #setLegendTargetChartConfig} 同步：用于拖动图例偏移 */
    private JavaFxChartRenderer.ChartConfig legendTargetConfig;
    private boolean legendPickActive;
    private double legendMinX;
    private double legendMinY;
    private double legendMaxX;
    private double legendMaxY;
    private boolean legendDragging;
    private double legendPressLX;
    private double legendPressLY;
    private double legendStartOffX;
    private double legendStartOffY;
    ...
    private void handleMousePressed(MouseEvent event) {
        ...
        Point2D logical = screenToLogical(event.getX(), event.getY());
        if (legendPickActive && legendTargetConfig != null && containsLegendPick(logical)) {
            legendDragging = true;
            legendPressLX = logical.getX();
            legendPressLY = logical.getY();
            legendStartOffX = legendTargetConfig.legendOffsetX;
            legendStartOffY = legendTargetConfig.legendOffsetY;
        } else {
    ...
    public void clearLegendPickBounds() {
        legendPickActive = false;
    }

    public void setLegendPickBounds(double minX, double minY, double maxX, double maxY) {
        this.legendMinX = minX;
        this.legendMinY = minY;
        this.legendMaxX = maxX;
        this.legendMaxY = maxY;
        this.legendPickActive = true;
    }

    public void setLegendTargetChartConfig(JavaFxChartRenderer.ChartConfig chartConfig) {
        this.legendTargetConfig = chartConfig;
    }

    private boolean containsLegendPick(Point2D logical) {
        double x = logical.getX();
        double y = logical.getY();
        return x >= legendMinX && x <= legendMaxX && y >= legendMinY && y <= legendMaxY;
    }
    ...
    private void handleMouseDragged(MouseEvent event) {
        if (legendDragging && legendTargetConfig != null) {
            Point2D logical = screenToLogical(event.getX(), event.getY());
            double dx = logical.getX() - legendPressLX;
            double dy = logical.getY() - legendPressLY;
            legendTargetConfig.legendOffsetX = legendStartOffX + dx;
            legendTargetConfig.legendOffsetY = legendStartOffY + dy;
            requestRepaint();
            return;
        }
        if (!enablePan) {
