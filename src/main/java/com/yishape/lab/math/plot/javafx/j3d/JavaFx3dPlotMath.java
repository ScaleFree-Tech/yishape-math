...
public final class JavaFx3dPlotMath {
    ...
    public static double minVec(IVector<?> v) {
        ...
        for (int i = 0; i < v.length(); i++) {
            m = Math.min(m, v.get(i).doubleValue());
        }
        return m;
    }
    ...
    public static double maxVec(IVector<?> v) {
        ...
        for (int i = 0; i < v.length(); i++) {
            m = Math.max(m, v.get(i).doubleValue());
        }
        return m;
    }
    ...
    public static double[] idwVel(double px, double py, double pz,
        ...
        int n = xs.length();
        for (int i = 0; i < n; i++) {
            double dx = px - xs.get(i).doubleValue();
            double dy = py - ys.get(i).doubleValue();
            double dz = pz - zs.get(i).doubleValue();
            double d2 = dx * dx + dy * dy + dz * dz + 1e-9;
            double wgt = 1.0 / d2;
            sx += wgt * us.get(i).doubleValue();
            sy += wgt * vs.get(i).doubleValue();
            sz += wgt * ws.get(i).doubleValue();
            sw += wgt;
        }
    ...
    public static double zMinMatrix(IMatrix<?> z) {
        ...
        for (int i = 0; i < z.rows(); i++) {
            for (int j = 0; j < z.cols(); j++) {
                m = Math.min(m, z.get(i, j).doubleValue());
            }
        }
        return m;
    }
    ...
    public static double zMaxMatrix(IMatrix<?> z) {
        ...
        for (int i = 0; i < z.rows(); i++) {
            for (int j = 0; j < z.cols(); j++) {
                m = Math.max(m, z.get(i, j).doubleValue());
            }
        }
        return m;
    }
