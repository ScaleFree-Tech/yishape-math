...
public final class JavaFx3dSurfaceContour {
    ...
    private static void marchSquareSegments(DataCylinder cylinder,
        ...
        for (int i = 0; i < nx - 1; i++) {
            for (int j = 0; j < ny - 1; j++) {
                double z00 = z.get(i, j);
                double z10 = z.get(i + 1, j);
                double z01 = z.get(i, j + 1);
                double z11 = z.get(i + 1, j + 1);
                edgeIso(cylinder, xv.get(i), yv.get(j), z00, xv.get(i + 1), yv.get(j), z10, level, zPlane, mat);
                edgeIso(cylinder, xv.get(i + 1), yv.get(j), z10, xv.get(i + 1), yv.get(j + 1), z11, level, zPlane, mat);
                edgeIso(cylinder, xv.get(i + 1), yv.get(j + 1), z11, xv.get(i), yv.get(j + 1), z01, level, zPlane, mat);
                edgeIso(cylinder, xv.get(i), yv.get(j + 1), z01, xv.get(i), yv.get(j), z00, level, zPlane, mat);
            }
        }
    }
