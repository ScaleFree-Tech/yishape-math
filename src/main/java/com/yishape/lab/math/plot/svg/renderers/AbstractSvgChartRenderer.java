...
public abstract class AbstractSvgChartRenderer {
    ...
    public String renderMulti(List<SeriesData> seriesList, ChartConfig config, String theme) {
        ...
        renderLegend(sb, seriesList, config, themeManager);
        buildFooter(sb);
        return sb.toString();
    }
    ...
    /**
     * Render legend as inline SVG elements. Position is controlled by {@code config.legendPosition}.
     */
    protected void renderLegend(StringBuilder sb, List<SeriesData> seriesList,
                                ChartConfig config, JavaFxThemeManager theme) {
        if (!config.showLegend || seriesList.size() <= 1) return;
        // Per-slice legends are handled inline by pie/themeRiver renderers
        String type = seriesList.get(0).type;
        if ("pie".equals(type) || "themeRiver".equals(type)) return;
        int n = seriesList.size();
        double itemH = 25;
        double legendNameW = 100;
        for (SeriesData s : seriesList) {
            if (s.name != null) {
                legendNameW = Math.max(legendNameW, Math.min(300, s.name.length() * 7.5));
            }
        }
        double lw = legendNameW + 32;  // swatch + gap + text
        double lh = n * itemH + 20;
        double boxW = lw + 16;
        double boxH = lh + 16;

        // Resolve position
        String pos = config.legendPosition != null ? config.legendPosition : LegendPositions.TOP_RIGHT;
        ...
        switch (pos) {
            case LegendPositions.TOP_LEFT -> {
                lx = config.paddingLeft + 10; ly = config.paddingTop + 30;
            }
            case LegendPositions.BOTTOM_RIGHT -> {
                lx = config.width - config.paddingRight - lw + 20; ly = config.height - config.paddingBottom - lh - 10;
            }
            case LegendPositions.BOTTOM_LEFT -> {
                lx = config.paddingLeft + 10; ly = config.height - config.paddingBottom - lh - 10;
            }
            case LegendPositions.RIGHT -> {
                lx = config.width - config.paddingRight + 5; ly = cy - lh / 2;
            }
            case LegendPositions.LEFT -> {
                lx = config.paddingLeft - lw - 15; ly = cy - lh / 2;
            }
            case LegendPositions.TOP -> {
                lx = cx - lw / 2; ly = config.paddingTop - lh - 10;
            }
            case LegendPositions.BOTTOM -> {
                lx = cx - lw / 2; ly = config.height - config.paddingBottom + 10;
            }
            default -> { // TOP_RIGHT
                lx = config.width - config.paddingRight - lw + 20; ly = config.paddingTop + 30;
            }
        }
        // Clamp to canvas
        lx = Math.max(2, Math.min(lx, Math.max(3, config.width - boxW - 2)));
        ly = Math.max(2, Math.min(ly, Math.max(3, config.height - boxH - 2)));

        String bg = backgroundColor;
        sb.append("<g class=\"legend\">\n");
        sb.append("<rect x=\"").append((int)(lx - 8)).append("\" y=\"").append((int)(ly - 8))
          .append("\" width=\"").append((int)boxW).append("\" height=\"").append((int)boxH)
          .append("\" rx=\"6\" fill=\"").append(bg).append("\" fill-opacity=\"0.94\"")
          .append(" stroke=\"").append(axisColor).append("\" stroke-width=\"0.9\"/>\n");

        for (int i = 0; i < n; i++) {
            SeriesData s = seriesList.get(i);
            String color = s.style != null && s.style.getColor() != null
                ? s.style.getColor() : colorPalette[i % colorPalette.length];
            double yRow = ly + i * itemH;
            sb.append("<rect x=\"").append((int)lx).append("\" y=\"").append((int)(yRow + 5))
              .append("\" width=\"14\" height=\"14\" fill=\"").append(color)
              .append("\" stroke=\"").append(axisColor).append("\" stroke-width=\"0.8\" rx=\"2\"/>\n");
            sb.append("<text x=\"").append((int)(lx + 22)).append("\" y=\"").append((int)(yRow + 14))
              .append("\" class=\"tick-label\" font-size=\"11\">")
              .append(escXml(s.name != null ? s.name : "Series " + (i + 1)))
              .append("</text>\n");
        }
        sb.append("</g>\n");
    }
    ...
    /**
     * 仅 Y 轴数值刻度 + 底/左轴线 + 水平网格（与 JavaFX 箱线/小提琴 {@code drawYAxisOnly} 一致），不画 X 轴刻度数字。
     * 轴标题请再调用 {@link #appendAxisTitleLabels}，避免与假 X 区间 0..1 的刻度混用。
     */
    protected void drawYAxisCartesianFrame(StringBuilder sb, ChartConfig config, double yMin, double yMax) {
        double cLeft = config.paddingLeft;
        double cRight = config.width - config.paddingRight;
        double cTop = config.paddingTop;
        double cBottom = config.height - config.paddingBottom;
        ...
        for (int i = 0; i <= yTicks; i++) {
            ...
            sb.append("<text x=\"").append((int) (cLeft - 6)).append("\" y=\"").append((int) (y + 4))
              .append("\" text-anchor=\"end\" class=\"tick-label\">")
              .append(formatTickLabel(value)).append("</text>\n");
            sb.append("<line x1=\"").append((int) cLeft).append("\" y1=\"").append((int) y)
              .append("\" x2=\"").append((int) (cLeft - 4)).append("\" y2=\"").append((int) y)
              .append("\" stroke=\"").append(axisColor).append("\" stroke-width=\"0.8\"/>\n");
        }
    ...
    /**
     * X/Y 轴标题（与 {@link #drawAxes} 末尾及 JavaFX {@code drawCartesianAxisTitles} 对齐），画在内容之后以便不被遮挡。
     */
    protected void appendAxisTitleLabels(StringBuilder sb, ChartConfig config) {
        String xn = config.xlabel;
        String yn = config.ylabel;
        if ((xn == null || xn.isEmpty()) && (yn == null || yn.isEmpty())) {
            return;
        }
        double cLeft = config.paddingLeft;
        double cRight = config.width - config.paddingRight;
        ...
        if (xn != null && !xn.isEmpty()) {
            sb.append("<text x=\"").append((int) (cLeft + cWidth / 2)).append("\" y=\"")
              .append(config.height - 8).append("\" text-anchor=\"middle\" class=\"axis-label\">")
              .append(escXml(xn)).append("</text>\n");
        }
        if (yn != null && !yn.isEmpty()) {
            int cy = (int) (cTop + cHeight / 2);
            sb.append("<text x=\"14\" y=\"").append(cy)
              .append("\" text-anchor=\"middle\" class=\"axis-label\" ")
              .append("transform=\"rotate(-90, 14, ").append(cy).append(")\">")
              .append(escXml(yn)).append("</text>\n");
        }
    }
