...
public class SvgCandlestickRenderer extends AbstractSvgChartRenderer {
    ...
    protected void renderSvgContent(StringBuilder sb, SeriesData series,
        ...
        for (int i = 0; i < n; i++) {
            ...
            double bodyH = Math.max(3.0, Math.abs(closeY - openY));
            sb.append("<rect x=\"").append((int) x).append("\" y=\"").append((int) Math.round(bodyTop))
              .append("\" width=\"").append((int) Math.round(candleWidth)).append("\" height=\"").append((int) Math.round(bodyH))
    ...
    private double[] niceAxis(double dataMin, double dataMax, int targetDivisions) {
        ...
        double step = nf * pow;
        double axisLo = Math.floor(dataMin / step) * step;
        double axisHi = Math.ceil(dataMax / step) * step;
