...
public class SvgJointplotRenderer extends AbstractSvgChartRenderer {
    ...
    protected void renderSvgContent(StringBuilder sb, SeriesData series,
        ...
        for (int i = 0; i < xData.length; i++) {
            ...
            double py = scatterY + pad + (yData[i] - yMinP) / yRangeP * (scatterH - 2 * pad);
        ...
        for (double v : xData) {
            ...
            int bin = Math.max(0, Math.min(bins - 1, (int) binIdx));
            xCounts[bin]++;
        }
        double xMaxCount = 0;
        ...
        for (double v : yData) {
            ...
            int bin = Math.max(0, Math.min(binsY - 1, (int) binIdx));
            yCounts[bin]++;
        }
        double yMaxCount = 0;
