...
public class RereHilbert {
    ...
    public static IVector<Double> instantaneousFrequency(IVector<Double> signal, double samplingRate) {
        ...
        for (int i = 1; i < signal.length(); i++) {
            double prev = phase.get(i - 1);
            double curr = phase.get(i);
            if (Double.isNaN(prev) || Double.isNaN(curr)) {
                phaseDiff.set(i, Double.NaN);
                continue;
            }
        ...
        // 索引0前向填充 / Forward-fill index 0
        if (signal.length() > 1) {
            phaseDiff.set(0, phaseDiff.get(1));
        }

        // 计算瞬时频率 / Calculate instantaneous frequency
        return phaseDiff.multiplyByScalar(samplingRate / (2 * Math.PI));
    }
    ...
    public static IVector<Double> instantaneousEntropy(IVector<Double> signal, int windowSize) {
        ...
        for (int i = 0; i < signal.length(); i++) {
            ...
            double totalPower = windowPower.sumValue();
            
            if (totalPower > 0) {
                // 归一化功率 / Normalize power
                IVector<Double> normalizedPower = windowPower.multiplyByScalar(1.0 / totalPower);
    ...
    public static IVector<Double> instantaneousBandwidth(IVector<Double> signal, double samplingRate) {
        ...
        for (int i = 1; i < signal.length(); i++) {
            double prev = amplitude.get(i - 1);
            double curr = amplitude.get(i);
            if (Double.isNaN(prev) || Double.isNaN(curr)) {
                amplitudeDerivative.set(i, Double.NaN);
                continue;
            }
            double deriv = (curr - prev) * samplingRate;
            amplitudeDerivative.set(i, Math.abs(deriv));
        }
        // 索引0前向填充 / Forward-fill index 0
        if (signal.length() > 1) {
            amplitudeDerivative.set(0, amplitudeDerivative.get(1));
        }
        ...
        for (int i = 0; i < signal.length(); i++) {
            ...
            double deriv = amplitudeDerivative.get(i);
            if (Double.isNaN(amp) || Double.isNaN(deriv)) {
                bandwidth.set(i, Double.NaN);
            } else if (amp > 1e-15) {
                bandwidth.set(i, deriv / amp);
            } else {
                bandwidth.set(i, 0.0);
            }
        }
