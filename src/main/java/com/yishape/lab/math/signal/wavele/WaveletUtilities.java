...
public class WaveletUtilities {
    ...
    public static IVector<Double> analyzeWaveletEnergy(WaveletCoefficients coeffs) {
        ...
        energy.set(0, coeffs.approximation.multiply(coeffs.approximation).sumValue());

        // 细节系数能量 / Detail coefficient energy
        for (int i = 0; i < coeffs.levels; i++) {
            energy.set(i + 1, coeffs.details[i].multiply(coeffs.details[i]).sumValue());
        }
    ...
    private static java.util.List<Double> extractCoefficientFeatures(IVector<Double> coeffs, String prefix) {
        ...
        features.add(coeffs.meanValue());
        features.add(coeffs.stdValue());
        features.add(coeffs.maxValue());
        features.add(coeffs.minValue());
        features.add(coeffs.sumValue());
    ...
    private static java.util.List<Double> extractEnergyFeatures(WaveletCoefficients coeffs) {
        ...
        // 总能量 / Total energy
        double totalEnergy = coeffs.approximation.multiply(coeffs.approximation).sumValue();
        for (int i = 0; i < coeffs.levels; i++) {
            totalEnergy += coeffs.details[i].multiply(coeffs.details[i]).sumValue();
        }
        features.add(totalEnergy);
        
        // 相对能量 / Relative energy
        features.add(coeffs.approximation.multiply(coeffs.approximation).sumValue() / totalEnergy);
    ...
    private static double calculateEntropy(IVector<Double> signal) {
        ...
        double sum = signal.multiply(signal).sumValue();
        if (sum == 0) {
            return 0.0;
        }
