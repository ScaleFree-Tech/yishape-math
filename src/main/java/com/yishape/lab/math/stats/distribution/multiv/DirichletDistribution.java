...
public class DirichletDistribution implements IMultivariateDistribution<Double> {
    ...
    public double klDivergence(IMultivariateDistribution<Double> other) {
        ...
        for (int i = 0; i < alpha.size(); i++) {
            kl += logGamma(q.alpha.get(i)) - logGamma(alpha.get(i));
            kl += (alpha.get(i) - q.alpha.get(i))
                    * (digamma(alpha.get(i)) - digamma(alphaSum));
        }
        return kl;
    }
    ...
    public double entropy() {
        ...
        for (int i = 0; i < alpha.size(); i++) {
            logB -= logGamma(alpha.get(i));
        }
        double h = logB + (alphaSum - alpha.size()) * digamma(alphaSum);
        for (int i = 0; i < alpha.size(); i++) {
            h -= (alpha.get(i) - 1.0) * digamma(alpha.get(i));
        }
        return h;
    }
    ...
    public IVector<Double> sampleVector() {
        int k = alpha.size();
        IVector gammaVariates = Linalg.zeros(k);

        // 从Gamma分布中采样
        double sum = 0.0;
        for (int i = 0; i < k; i++) {
            GammaDistribution gamma = new GammaDistribution(alpha.get(i), 1.0);
        ...
        // 归一化得到Dirichlet样本
        IVector result = Linalg.zeros(k);
        for (int i = 0; i < k; i++) {
            result.set(i, gammaVariates.get(i) / sum);
        }
    ...
    public double pdfVector(IVector x) {
        ...
        for (int i = 0; i < x.size(); i++) {
            ...
            logPdf += (alpha.get(i) - 1.0) * Math.log(xi);
        }
    ...
    public double logPdf(IVector x) {
        ...
        for (int i = 0; i < x.size(); i++) {
            logPdf += (alpha.get(i) - 1.0) * Math.log(x.get(i));
        }
    ...
    public IVector<Double> meanVector() {
        ...
        for (int i = 0; i < alpha.size(); i++) {
            mean.set(i, alpha.get(i) / alphaSum);
        }
    ...
    public double mode() {
        ...
        try {
            if (alpha.size() > 0) {
                ...
                if (denominator > 0) {
                    return (alpha.get(0) - 1.0) / denominator;
                }
            }
        } catch (Exception e) {
    ...
    public double pdf(double x) {
        ...
        return Math.pow(x, alpha.get(0) - 1) * Math.pow(1 - x, alphaSum - alpha.get(0));
    }
    ...
    public IVector modeVector() {
        ...
        for (int i = 0; i < alpha.size(); i++) {
            mode.set(i, (alpha.get(i) - 1.0) / denominator);
        }
