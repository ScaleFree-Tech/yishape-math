...
final class MultivariateDistributionMath {
    ...
    static double wishartElementCovariance(double nu, IMatrix v, int i, int j, int k, int l) {
        return nu * (v.get(i, k).doubleValue() * v.get(j, l).doubleValue()
                + v.get(i, l).doubleValue() * v.get(j, k).doubleValue());
    }
    ...
    static double inverseWishartElementCovariance(double nu, int p, IMatrix psi, int i, int j, int k, int l) {
        ...
        double term1 = 2 * psi.get(i, j).doubleValue() * psi.get(k, l).doubleValue();
        double term2 = (nu - p - 1) * (psi.get(i, k).doubleValue() * psi.get(j, l).doubleValue()
                + psi.get(i, l).doubleValue() * psi.get(j, k).doubleValue());
