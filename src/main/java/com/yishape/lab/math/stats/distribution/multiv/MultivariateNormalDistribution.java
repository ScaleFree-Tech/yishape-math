...
public class MultivariateNormalDistribution implements IMultivariateDistribution<Double> {
    ...
    public IVector<Double> sample() {
        ...
        // 使用 Cholesky 分解；若矩阵数值病态，回退到 eigendecomposition
        IMatrix<Double> transform;
        try {
            transform = covariance.cholesky();
        } catch (Exception e) {
            // 岭正则化：Σ + ridge*I，其中 ridge 与迹成比例
            try {
                double ridge = 1e-10 * Math.abs(covariance.trace()) / dimension;
                IMatrix<Double> reg = covariance.add(Linalg.eye(dimension).multiplyByScalar(ridge));
                transform = reg.cholesky();
            } catch (Exception e2) {
                // 回退：Σ = QΛQᵀ，取 Σ½ = Q√Λ，clamp 负特征值为 0
                var eig = covariance.eigen();
                IVector<Double> lam = eig.getFirst();
                IMatrix<Double> q = eig.getSecond();
                IMatrix<Double> sqrtLam = Linalg.zeros(dimension, dimension);
                for (int i = 0; i < dimension; i++) {
                    sqrtLam.set(i, i, Math.sqrt(Math.max(0.0, lam.get(i))));
                }
                transform = q.mmul(sqrtLam);
            }
        }
        IVector<Double> transformedSample = transform.mmul(standardVector);
    ...
    public double wassersteinDistance(IMultivariateDistribution<Double> other) {
        ...
        double meanTerm = meanDiff.dotValue(meanDiff);
    ...
    public IMultivariateDistribution<Double> conjugateUpdate(IVector<Double> observations) {
        ...
        IVector<Double> newMean = mean.add(observations.multiplyByScalar(0.1));
    ...
    public double marginalLikelihood(IVector<Double> observations) {
        ...
        // This is a simplified placeholder implementation
        double logLikelihood = -0.5 * observations.dotValue(observations);
