...
public class WishartDistribution implements IMultivariateDistribution<Double> {
    ...
    public IMatrix<Double> getCovariance() {
        ...
        if (vectorDim >= 1024) {
            YishapeLogger.getLogger(WishartDistribution.class)
                    .warn("Large vec(W) covariance allocation: vectorDim={}, ~{} MB",
                            vectorDim, (vectorDim * vectorDim * 8L) >> 20);
        }
    ...
    private IMatrix sampleBartlett() {
        ...
        IMatrix LT = scaleMatrixCholesky.transpose();
    ...
    private IMatrix<Double> meanMatrixTyped() {
        ...
        for (int i = 0; i < matrixOrder; i++) {
            for (int j = 0; j < matrixOrder; j++) {
                mean.set(i, j, degreesOfFreedom * scaleMatrix.get(i, j));
            }
        }
        return mean;
    }
    ...
    public double var() {
        ...
        for (int i = 0; i < matrixOrder; i++) {
            double diagElement = scaleMatrix.get(i, i);
            trace += diagElement;
    ...
    public IMatrix varianceMatrix() {
        ...
        for (int i = 0; i < matrixOrder; i++) {
            for (int j = 0; j < matrixOrder; j++) {
                double vij = scaleMatrix.get(i, j);
                double vii = scaleMatrix.get(i, i);
                double vjj = scaleMatrix.get(j, j);
