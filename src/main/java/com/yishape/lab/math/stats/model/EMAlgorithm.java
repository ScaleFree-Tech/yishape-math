...
public class EMAlgorithm {
    ...
    private static final YishapeLogger log = YishapeLogger.getLogger(EMAlgorithm.class);
    ...
    /**
     * 在专用分解线程池中执行任务，避免竞争公共 ForkJoinPool。
     */
    private void runInDecompositionPool(Runnable task) {
    ...
    private <T> T supplyInDecompositionPool(Callable<T> task) {
        ForkJoinPool pool = RereDoubleMatrix.getDecompositionPool();
        try {
    ...
    private IMatrix<Double> computeWeightedCovariance(List<IVector<Double>> data, IMatrix<Double> posteriors,
        ...
        IMatrix<Double> covarianceSum = Linalg.zeros(dimension, dimension);
    ...
    private double computeLogLikelihood(List<IVector<Double>> data, GaussianMixtureModel gmm) {
        ...
        if (enableParallel) {
            return supplyInDecompositionPool(() ->
    ...
    private void applyDampedUpdate(GaussianMixtureModel gmm, List<IVector<Double>> oldMeans, 
        ...
        for (int c = 0; c < numComponents; c++) {
            ...
            IVector<Double> dampedMean = oldMeans.get(c).multiplyByScalar(1.0 - DAMPING_FACTOR)
                                                      .add(currentMean.multiplyByScalar(DAMPING_FACTOR));
            
            // 阻尼更新协方差矩阵
            IMatrix<Double> currentCov = gmm.getComponent(c).getCovariance();
            IMatrix<Double> dampedCov = oldCovariances.get(c).multiplyByScalar(1.0 - DAMPING_FACTOR)
                                                            .add(currentCov.multiplyByScalar(DAMPING_FACTOR));
