 ...
 * <p>
 * 提供时间序列滤波功能，包括移动平均、指数平滑、卡尔曼滤波、小波滤波等。 使用项目现有的signal包和linalg包功能进行滤波处理。
 * </p>
 * <p>
 * Provides time series filtering functionality including moving average,
 * exponential smoothing, Kalman filtering, wavelet filtering, etc. Uses
 * existing signal and linalg package functionality for filtering processing.
 * </p>
 *
 * @author lterb2
 * @version 1.0
...
public class TimeSeriesFiltering {
     ...
     * <p>
     * 对时间序列进行移动平均滤波，用于平滑噪声。 Apply moving average filter to time series for
     * noise smoothing.
     * </p>
     *
     ...
     * <p>
     * 对时间序列进行指数平滑滤波，适用于趋势数据。 Apply exponential smoothing filter to time series,
     * suitable for trend data.
     * </p>
     *
     * @param timeSeries 输入时间序列 / Input time series
     * @param variableIndex 变量索引 / Variable index
     ...
     * <p>
     * 对时间序列进行高斯滤波，提供平滑的滤波效果。 Apply Gaussian filter to time series for smooth
     * filtering effect.
     * </p>
     *
     * @param timeSeries 输入时间序列 / Input time series
     ...
     * 中值滤波 / Median Filter
     * <p>
     * 对时间序列进行中值滤波，有效去除脉冲噪声。 Apply median filter to time series for effective
     ...
     * <p>
     * 对时间序列进行低通滤波，去除高频噪声。 Apply low pass filter to time series for high
     ...
     * <p>
     * 对时间序列进行高通滤波，去除低频趋势。 Apply high pass filter to time series for low
     * frequency trend removal.
     * </p>
     *
     * @param timeSeries 输入时间序列 / Input time series
     * @param variableIndex 变量索引 / Variable index
     ...
     * <p>
     * 对时间序列进行带通滤波，保留特定频率范围。 Apply band pass filter to time series for specific
     * frequency range retention.
     * </p>
     *
     * @param timeSeries 输入时间序列 / Input time series
     * @param variableIndex 变量索引 / Variable index
     ...
     * <p>
     * 对时间序列进行自适应滤波，根据信号特性自动调整滤波参数。 Apply adaptive filter to time series,
     * automatically adjusting filter parameters based on signal
     * characteristics.
     * </p>
     *
     * @param timeSeries 输入时间序列 / Input time series
