...
public class TimeSeriesUtils {
    ...
    public static double calculateCoefficientOfVariation(IVector<Double> data) {
        ...
        return data.stdValue() / Math.abs(mean);
    }
    ...
    public static IVector<Double> calculatePartialAutocorrelation(IVector<Double> data, int maxLag) {
        ...
        for (int i = 0; i <= maxLag; i++) r[i] = autocorr.get(i);

        // Levinson-Durbin algorithm for Yule-Walker equations
        double[] a = new double[maxLag + 1];
        a[0] = 1.0;
        double[] aNew = new double[maxLag + 1];
        double E = r[0];
    ...
    public static TrendResult analyzeTrend(IVector<Double> data) {
        ...
        IVector<Double> trend = x.multiplyByScalar(slope).addScalar(intercept);
        
        // 计算去趋势数据 / Calculate detrended data
        IVector<Double> detrended = data.sub(trend);
        
        // 计算R² / Calculate R-squared
        double ssRes = detrended.multiply(detrended).sumValue();
        double ssTot = data.subScalar(data.meanValue()).multiply(data.subScalar(data.meanValue())).sumValue();
    ...
    public static double detectSeasonalStrength(IVector<Double> data, int period) {
        ...
        double seasonalVar = seasonal.varValue();
        double dataVar = data.varValue();
    ...
    public static IVector<Double> movingAverageForecast(IVector<Double> data, int steps, int windowSize) {
        ...
        double lastValue = data.slice(data.length() - windowSize, data.length()).meanValue();
    ...
    public static boolean checkStationarity(IVector<Double> data) {
        ...
        double var1 = data.slice(0, half).varValue();
        double var2 = data.slice(half, n).varValue();
    ...
    public static IVector<Double> standardize(IVector<Double> data) {
        ...
        return data.subScalar(mean).multiplyByScalar(1.0 / std);
    }
    ...
    public static IVector<Double> normalize(IVector<Double> data) {
        ...
        return data.subScalar(min).multiplyByScalar(1.0 / (max - min));
    }
    ...
    public static IVector<Double> calculateFeatures(IVector<Double> data) {
        double[] features = {
            data.meanValue(),                    // 均值 / Mean
            data.stdValue(),                     // 标准差 / Standard deviation
            data.minValue(),                     // 最小值 / Minimum
            data.maxValue(),                     // 最大值 / Maximum
            ...
            data.maxValue() - data.minValue()         // 极差 / Range
        };
