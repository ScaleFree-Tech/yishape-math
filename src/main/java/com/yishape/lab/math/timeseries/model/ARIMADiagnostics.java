...
public class ARIMADiagnostics implements ITimeSeriesDiagnostics {
    ...
    public IVector<Double> getStandardizedResiduals() {
        double mean = residuals.meanValue();
        double std = residuals.stdValue();
    ...
    public double[] getGoodnessOfFitMetrics() {
        // 计算R² / Calculate R²
        double ssRes = residuals.multiply(residuals).sumValue();
        double ssTot = originalData.subScalar(originalData.meanValue()).multiply(
                      originalData.subScalar(originalData.meanValue())).sumValue();
    ...
    private double calculateADFStatistic(IVector<Double> data) {
        ...
        double numerator = diff.meanValue();
        double denominator = diff.stdValue();
