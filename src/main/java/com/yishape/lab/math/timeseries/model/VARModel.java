...
public class VARModel {
    ...
    private final IMatrix<Double> originalData; // 原始训练数据 / Original training data
    ...
    public VARModel(int p, int k, IMatrix<Double> coefficients, IMatrix<Double> coefMatrix, IVector<Double> constant,
                   ...
                   double aic, double bic, String[] variableNames, IMatrix<Double> originalData) {
        this.p = p;
        this.k = k;
    ...
    public static VARModel fit(IMatrix<Double> data, int p, String[] variableNames) {
        ...
        IMatrix<Double> covariance = residuals.transpose().mmul(residuals).multiplyByScalar(1.0 / (observations - regressors));
        ...
        return new VARModel(p, k, coefficients, coefMatrix, constant, residuals, covariance,
                          logLikelihood, aic, bic, variableNames, data);
    }
    ...
    public VARForecastResult forecast(int steps) {
        ...
        for (int i = 0; i < p; i++) {
            for (int j = 0; j < k; j++) {
                lastObservations.set(i, j, originalData.get(n - p + i, j));
            }
        }
    ...
    public VarianceDecompositionResult varianceDecomposition(int steps) {
        ...
        // Cholesky分解以获得正交化脉冲响应 / Cholesky decomposition for orthogonalized IRF
        IMatrix<Double> P;
        try {
            P = covariance.cholesky();
        } catch (Exception e) {
            // 如果Cholesky失败，使用对角矩阵的平方根作为回退 / Fallback to sqrt of diagonal
            P = Linalg.zeros(k, k);
            for (int i = 0; i < k; i++) {
                P.set(i, i, Math.sqrt(Math.abs(covariance.get(i, i))));
            }
        }

        // 计算正交化脉冲响应的累积平方和 / Accumulate squared orthogonalized IRF
        IMatrix<Double> decomposition = Linalg.zeros(k, k);
        for (int t = 0; t < steps; t++) {
            IMatrix<Double> Theta = responseMatrices.get(t).mmul(P);
            for (int i = 0; i < k; i++) {
                for (int j = 0; j < k; j++) {
                    double theta_ij = Theta.get(i, j);
                    decomposition.set(i, j, decomposition.get(i, j) + theta_ij * theta_ij);
                }
            }
        }

        // 标准化：每行除以其总和 / Normalize each row by its total
        for (int i = 0; i < k; i++) {
            double total = 0.0;
            for (int j = 0; j < k; j++) {
    ...
    public GrangerCausalityResult grangerCausalityTest(int causeVar, int effectVar) {
        ...
        if (originalData == null) {
            throw new IllegalStateException("模型未包含原始数据，无法进行Granger因果检验");
        }

        int n = originalData.getRowNum();
        int observations = n - p;
        int regressors = k * p + 1; // 无约束模型参数总数 / total params in unrestricted

        // 构建效应变量的观测向量Y / Build observation vector Y for effect variable
        IVector<Double> Y = Linalg.zeros(observations);
        for (int t = 0; t < observations; t++) {
            Y.set(t, originalData.get(t + p, effectVar));
        }

        // 计算无约束模型的残差平方和RSS_u / Compute unrestricted RSS
        double rssU = 0.0;
        for (int t = 0; t < observations; t++) {
            double predicted = constant.get(effectVar);
            for (int lag = 1; lag <= p; lag++) {
                for (int var = 0; var < k; var++) {
                    int col = (lag - 1) * k + var;
                    predicted += coefMatrix.get(effectVar, col) * originalData.get(t + p - lag, var);
                }
            }
            double residual = Y.get(t) - predicted;
            rssU += residual * residual;
        }

        // 构建有约束模型（排除causeVar的所有滞后项）/ Build restricted model (exclude causeVar lags)
        int restrictedRegressors = regressors - p; // 去掉causeVar的p个滞后项
        IMatrix<Double> Xr = Linalg.zeros(observations, restrictedRegressors);
        for (int t = 0; t < observations; t++) {
            Xr.set(t, 0, 1.0); // 常数项 / Constant
            int col = 1;
            for (int lag = 1; lag <= p; lag++) {
                for (int var = 0; var < k; var++) {
                    if (var == causeVar) continue; // 排除原因变量的滞后项
                    Xr.set(t, col, originalData.get(t + p - lag, var));
                    col++;
                }
            }
        }

        // 有约束模型的最小二乘估计 / Restricted model OLS
        IMatrix<Double> XrTXr = Xr.transpose().mmul(Xr);
        IVector<Double> XrTY = Xr.transpose().mmul(Y);
        IVector<Double> restrictedCoeffs = XrTXr.pinv().mmul(XrTY);
        IVector<Double> restrictedFitted = Xr.mmul(restrictedCoeffs);
        double rssR = 0.0;
        for (int t = 0; t < observations; t++) {
            double residual = Y.get(t) - restrictedFitted.get(t);
            rssR += residual * residual;
        }

        // 计算F统计量 / Calculate F statistic
        double fStatistic = ((rssR - rssU) / p) / (rssU / (observations - regressors));

        // 计算p值（使用F分布） / Calculate p-value using F distribution
        double pValue = calculateFPValue(fStatistic, p, observations - regressors);
     ...
     */
    public static VARModel autoFit(IMatrix<Double> data, int maxP, TimeSeriesModelFactory.SelectionCriterion criterion, String[] variableNames) {
        VARModel bestModel = null;
        double bestCriterion = Double.POSITIVE_INFINITY;

        for (int p = 1; p <= maxP; p++) {
            try {
                VARModel model = fit(data, p, variableNames); // fit() already stores originalData
    ...
    private static double calculateLogLikelihood(IMatrix<Double> residuals, IMatrix<Double> covariance, int n) {
        try {
            int k = covariance.getColNum();
            IMatrix<Double> invCov = covariance.pinv();
            double det = Math.abs(covariance.det());
            ...
            for (int i = 0; i < n; i++) {
                ...
                double quadratic = temp.innerProductValue(residual);  // 行向量与行向量的内积
                logLikelihood += -0.5 * (k * Math.log(2 * Math.PI) + Math.log(det) + quadratic);
            }
    ...
    private static double calculateFPValue(double fStatistic, int df1, int df2) {
        ...
        try {
            com.yishape.lab.math.stats.distribution.FDistribution fDist =
                new com.yishape.lab.math.stats.distribution.FDistribution(df1, df2);
