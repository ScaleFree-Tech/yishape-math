...
public class SetAllVariablesTest {
    public static void main(String[] args) {
        ...
        try {
            ...
            Tuple2<Double, IVector> result1 = solver1.solveWithNonNegativeEqualConstraints(c1, A_eq1, b_eq1);
            if (result1 != null) {
                System.out.println("   Solution: " + result1.getSecond());
                // Check if all variables are integers
                boolean allIntegers = true;
                for (int i = 0; i < result1.getSecond().length(); i++) {
                    double value = (Double) result1.getSecond().get(i);
        ...
        try {
            ...
            Tuple2<Double, IVector> result2 = solver2.solve(c2, A_ub2, b_ub2);
            if (result2 != null) {
                System.out.println("   Solution: " + result2.getSecond());
                // Check if all variables are binary (0 or 1)
                boolean allBinary = true;
                for (int i = 0; i < result2.getSecond().length(); i++) {
                    double value = (Double) result2.getSecond().get(i);
