package com.reremouse.lab.math.stats.distributions;

import com.reremouse.lab.math.stats.distribution.multiv.WishartDistribution;
...
public class WishartDistributionTest {
    ...
    void testMean() {
        ...
        IMatrix<Double> expectedMean = scaleMatrix.multiplyScalar(degreesOfFreedom);
    ...
    void testVariance() {
        ...
        for (int i = 0; i < variance.rows(); i++) {
            for (int j = 0; j < variance.cols(); j++) {
                assertEquals(variance.get(i, j).doubleValue(), variance.get(j, i).doubleValue(), tolerance);
            }
        }
    }
