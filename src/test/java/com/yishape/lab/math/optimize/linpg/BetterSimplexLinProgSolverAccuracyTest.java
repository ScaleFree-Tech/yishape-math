...
public class BetterSimplexLinProgSolverAccuracyTest {
    ...
    void testStandardMinimizationProblem() {
        ...
        if (result.isConverged()) {
            ...
            double x1 = fullSolution.get(0).doubleValue();
            double x2 = fullSolution.get(1).doubleValue();
    ...
    void testThreeVariableProblem() {
        ...
        if (result.isConverged()) {
            ...
            double x1 = solution.get(0).doubleValue();
            double x2 = solution.get(1).doubleValue();
            double x3 = solution.get(2).doubleValue();
    ...
    private boolean verifySolution(IVector solution, IMatrix A, IVector b) {
        ...
        for (int i = 0; i < solution.length(); i++) {
            if (solution.get(i).doubleValue() < -TOLERANCE) {
                return false;
            }
        }
        ...
        for (int i = 0; i < b.length(); i++) {
            double diff = Math.abs(constraintValues.get(i).doubleValue() - b.get(i).doubleValue());
            if (diff > TOLERANCE) {
                return false;
            }
        }
    ...
    private double calculateObjective(IVector c, IVector solution) {
        ...
        for (int i = 0; i < c.length(); i++) {
            objective += c.get(i).doubleValue() * solution.get(i).doubleValue();
        }
        return objective;
    }
}