...
public class BetterSimplexLinProgSolverAdditionalTest {
    ...
    public void testDegenerateProblem() {
        ...
        assertEquals(0.0, result.getOptimalPoint().get(0), TOLERANCE);
        assertEquals(0.0, result.getOptimalPoint().get(1), TOLERANCE);
    ...
    public void testLargerProblem() {
        ...
        for (int i = 0; i < solution.size(); i++) {
            assertTrue(solution.get(i) >= -TOLERANCE, 
                "解的第" + (i+1) + "个分量应该非负");
        }
    }
    ...
    public void testZeroObjectiveCoefficients() {
        ...
        // 验证约束满足
        IVector solution = result.getOptimalPoint();
        double constraintValue = solution.get(0) + solution.get(1);
