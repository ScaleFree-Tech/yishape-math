...
public class ConstraintViolationDebugTest {
    ...
    void debugStandardMaximizationProblem() {
        ...
        for (int i = 0; i < A_eq.rows(); i++) {
            ...
            for (int j = 0; j < A_eq.cols(); j++) {
                System.out.printf("%.1f ", A_eq.get(i, j).doubleValue());
            }
            System.out.println();
        }
    ...
    void debugThreeVariableProblem() {
        ...
        if (result.isConverged()) {
            ...
            double s1 = solution.get(3).doubleValue();
            double s2 = solution.get(4).doubleValue();
            double s3 = solution.get(5).doubleValue();
