...
public class SignalGenerationTest {
    ...
    public void testChirpSignal() {
        ...
        if (Math.abs(chirpSignal.maxValue()) > 1.0 + 1e-10) {
            throw new RuntimeException("Signal should be bounded, but max value was " + chirpSignal.maxValue());
        }
        
        if (Math.abs(chirpSignal.minValue()) < -1.0 - 1e-10) {
            throw new RuntimeException("Signal should be bounded, but min value was " + chirpSignal.minValue());
        }
    ...
    public void testPulseSignal() {
        ...
        // 检查信号的基本特性 / Check basic signal characteristics
        // 信号应该是有界的 / Signal should be bounded
        if (Math.abs(pulseSignal.maxValue()) > 1.0 + 1e-10) {
            throw new RuntimeException("Signal should be bounded, but max value was " + pulseSignal.maxValue());
        }
        
        if (pulseSignal.minValue() < -1e-10) {
            throw new RuntimeException("Signal should be non-negative, but min value was " + pulseSignal.minValue());
        }
