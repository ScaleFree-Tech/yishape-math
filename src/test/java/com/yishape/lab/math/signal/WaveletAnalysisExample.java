...
import com.yishape.lab.math.signal.wavele.WaveletUtilities;
import com.yishape.lab.math.signal.wavele.WaveletFilters;
 ...
 * @since 1.0
 */
public class WaveletAnalysisExample {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("  均值: " + signal.meanValue());
        System.out.println("  标准差: " + signal.stdValue());
        System.out.println("  峰值: " + signal.maxValue());
        ...
        IVector<Double> scales = Linalg.range(1, 65).multiplyByScalar(0.5);
        ...
        System.out.println("  Rigrsure方法 - 均值: " + denoisedRigrsure.meanValue() + 
                          ", 标准差: " + denoisedRigrsure.stdValue());
        System.out.println("  SURE方法 - 均值: " + denoisedSure.meanValue() + 
                          ", 标准差: " + denoisedSure.stdValue());
        System.out.println("  自适应方法 - 均值: " + adaptiveDenoised.meanValue() + 
                          ", 标准差: " + adaptiveDenoised.stdValue());
        ...
        for (double ratio : compressionRatios) {
            ...
            double compressionRatio = (double) signal.length() / compressed.length();
            
            System.out.println("  压缩比 " + (int)(ratio * 100) + "%:");
            System.out.println("    压缩误差: " + compressionError);
            System.out.println("    实际压缩比: " + compressionRatio);
        }
        ...
        System.out.println("相干性分析:");
        System.out.println("  相干性矩阵大小: " + coherence.rows() + " x " + coherence.cols());
        
        // 计算平均相干性 / Calculate average coherence
        double avgCoherence = 0;
        for (int i = 0; i < coherence.rows(); i++) {
            for (int j = 0; j < coherence.cols(); j++) {
                avgCoherence += coherence.get(i, j);
            }
        }
        avgCoherence /= (coherence.rows() * coherence.cols());
        System.out.println("  平均相干性: " + avgCoherence);
        
        // 小波去趋势和平滑 / Wavelet Detrending and Smoothing
        System.out.println("\n=== 小波去趋势和平滑 / Wavelet Detrending and Smoothing ===");
        
        IVector<Double> detrended = WaveletUtilities.waveletDetrending(
                signal, WaveletAnalysis.WaveletType.DAUBECHIES, 4, 4);
        IVector<Double> smoothed = WaveletUtilities.waveletSmoothing(
                signal, WaveletAnalysis.WaveletType.DAUBECHIES, 4, 4);
        
        System.out.println("去趋势结果:");
        System.out.println("  原始信号均值: " + signal.meanValue());
        System.out.println("  去趋势后均值: " + detrended.meanValue());
        
        System.out.println("平滑结果:");
        System.out.println("  原始信号标准差: " + signal.stdValue());
        System.out.println("  平滑后标准差: " + smoothed.stdValue());
    ...
    private static double calculateSNR(IVector<Double> original, IVector<Double> noisy) {
        ...
        double signalPower = original.multiply(original).meanValue();
        double noisePower = noise.multiply(noise).meanValue();
