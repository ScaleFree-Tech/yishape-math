...
public class StatsCorrCovTest {
    ...
    public void testCorrZero() {
        ...
        // 实现：任一侧标准差为 0 时抛 ArithmeticException（与 RereFloatVector.corr 一致）
        assertThrows(ArithmeticException.class, () -> Stats.corr(v1, v2),
                "与常数向量求相关系数应视为不可用 / corr with zero std should throw");
    }
    ...
    public void testCovWithEmptyVector() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            Stats.cov(v1, v2);
        }, "长度不匹配（含空向量一侧）应抛出 IllegalArgumentException");
    }
}
