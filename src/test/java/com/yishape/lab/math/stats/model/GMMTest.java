...
public class GMMTest {
    ...
    private static void testMultivariateGaussian() {
        ...
        // 创建测试数据
        IVector<Double> mean = Linalg.vector(new double[]{1.0, 2.0});
        IMatrix<Double> covariance = Linalg.matrix(new double[][]{
            {1.0, 0.5},
            {0.5, 2.0}
        });
        ...
        // 测试概率密度函数
        IVector<Double> testPoint = Linalg.vector(new double[]{1.5, 2.5});
    ...
    private static List<IVector<Double>> generateTestData() {
        ...
        for (int i = 0; i < 100; i++) {
            if (random.nextDouble() < 0.5) {
                ...
                data.add(Linalg.vector(new double[]{x1, y1}));
            } else {
                // 第二个分量：均值[4, 4]，标准差1.0
                double x2 = random.nextGaussian() * 1.0 + 4.0;
                double y2 = random.nextGaussian() * 1.0 + 4.0;
                data.add(Linalg.vector(new double[]{x2, y2}));
            }
        }
