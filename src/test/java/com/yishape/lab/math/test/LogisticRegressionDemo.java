...
public class LogisticRegressionDemo {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // 获取训练结果
            LRResult lrResult = lr.getResult();
            
            // 显示训练结果
            System.out.println("训练结果：");
            System.out.println("  最终损失: " + lrResult.getLoss());
            
            // 手动格式化权重向量输出，避免依赖问题
            IVector<Float> weights = lrResult.getWeights();
            ...
            // 手动格式化偏置项输出
            IVector<Float> bias = lrResult.getBias();
            ...
            for (int i = 0; i < newSamples.length; i++) {
                IVector<Double> newSample = Linalg.vector(newSamples[i]);
